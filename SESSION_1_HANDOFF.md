...
- **Current task** Session-2 T3.4-phase-3-staged landed (agent-authored cherry-pick) : **@staged comptime-check structural walker** in cssl-hir — `check_staged_consistency(&HirModule, &Interner)` validates stage-class annotations (CompTime / Runtime / Polymorphic) + detects call-site mismatches + flags cyclic dependency graphs via DFS. Mirrors T3-D11 AD-legality + T3-D12 IFC walker API. 3 diagnostic codes (STG0001/2/3) + StageRegistry + StageClass + StageEntry. 25 new tests covering missing-class / accepted-classes / mismatches / acyclic / cyclic / 3-fn cycles. 1074 tests / 0 failed / 0 clippy. Remaining phase-2d : vector-SDF + scene-SDF monomorphization + T6-phase-2c HirExprKind widening + macro-hygiene + let-generalization + full AnalyticExpr→Term SMT translator + Arc A770 bit-exact verification.
