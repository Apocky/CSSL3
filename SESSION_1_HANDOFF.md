...
- **Current task** Session-4 T11-D41 landed : **CALL-SITE REWRITING — whole-module generic-fn compilation end-to-end**. Previously the walker produced specialized `id_i32` but `main`'s body still had `func.call @id` (unspecialized name) — JIT-compiling `main` would fail. T11-D41 closes this : (a) body_lower stamps every `func.call` op with an `hir_id` attribute, (b) new `rewrite_generic_call_sites(module, report.call_site_names) -> u32` walks all ops and rewrites callees to mangled names. For source `fn id<T>(x:T) -> T { x }; fn main() -> i32 { id::<i32>(5) }` : pre-rewrite MIR has `main { func.call @id (5) }` — won't JIT ; post-rewrite has `main { func.call @id_i32 (5) }` + specialized `id_i32` fn. Whole module compiles. 4 new rewriter tests + updated `lower_call` signature + re-export. **Closes the D38..D41 generic-fn MVP full arc** (API + syntax + discovery + rewriting). 1499 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Next** : natural follow-ups are end-to-end JIT integration test (compile `main` itself after rewriting, not just specializations), bwd-mode AD on generic fns, bare-call type-inference, multi-segment callees, generic struct/enum/impl monomorphization (needed for `struct Vec<T>`), or pivot to heap-allocation primitives.
