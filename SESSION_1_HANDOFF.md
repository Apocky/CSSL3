...
- **Current task** Session-4 T11-D26 landed : **Multi-fn JIT modules + inter-fn calls — user-defined CSSLv3 fns can call each other at runtime**. JIT `compile` gains a pre-scan pass that finds `func.call` ops with user-defined callees (not intrinsics), looks them up in `fn_table` (populated by prior `compile()` calls), and uses cranelift's `declare_func_in_func` to produce per-caller `FuncRef`s. `lower_intrinsic_call` fallback emits `builder.ins().call(func_ref, &args)` for registered user-defined callees or `UnsupportedMirOp` otherwise. `is_intrinsic_callee` helper centralizes the intrinsic-name set. 4 new tests : `inter_fn_call_jit_roundtrip` (hand-built double+caller : `caller(5) == 11`), `inter_fn_call_unknown_callee_errors`, `full_chain_source_inter_fn_call_runtime` (source-driven : `fn helper(x) { x * x }` + `fn scene(x) { helper(x) }` : `scene(3) == 9`, `scene(-4) == 16`). 1366 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Multi-fn scene SDFs are now unblocked** — ordering discipline : callee must be compiled before caller in the same JitModule (cyclic recursion deferred to a two-phase API). Remaining open arcs : T11-D27 JIT multi-return (for multi-param bwd variants like `sphere_sdf(p, r)` returning both d_p and d_r) + mutual recursion via two-phase declare-all-then-define-all + full scene-SDF composition gate + libm-backed transcendentals + stage-1 self-host.
