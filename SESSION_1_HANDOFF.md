...
- **Current task** Session-4 T11-D35 landed : **VEC3 RUNTIME GRADIENT — `sphere_sdf(p : vec3<f32>, r : f32) -> f32 { length(p) - r }` compiles end-to-end and gradient matches `normalize(p)` at JIT-executed runtime**. Body-lower scalarizes vec3 params into 3 consecutive scalar f32 MIR params ; `length(p)` inlines to `sqrt(Σ pᵢ²)` via 7 scalar ops (3 mulf + 2 addf + 1 sqrt call). AD walker + JIT stay scalar-only — zero changes needed downstream. At `p = (3, 0, 4)`, `r = 1` : JIT primal = 4.0 ✓ ; JIT fwd gradient = (0.6, 0.0, 0.8, -1.0) = (normalize(p), -1) within 1e-3 ✓ ; cross-checked by central-difference on the JIT-compiled primal. Closes the T11-D31 loop — `MirType::Vec` remains as a canonical intent marker but scalarization happens at HIR→MIR. Second slice of the user-directed "2 → 1 → 4 → 3" next-arc sequence complete. 1418 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Next** : IFC wire-through — Jif-DLM declassification + integrity-endorsement enforcement on real programs (currently structural-walker only).
