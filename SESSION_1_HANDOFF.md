...
- **Current task** Session-3 T7-D8 landed : **proof-cert emission + R18 SMT signing**. `SignedProofCert { case_name, query_text, verdict, solver_kind, content_hash, signature, verifying_key, format }` wraps the SMT `(query, verdict, solver)` triple with BLAKE3 + Ed25519. `GradientCase::sign_proof_cert(&dyn Solver, &SigningKey)` produces the cert ; `verify_signed_proof_cert` emits 4-step ProofCertVerdict ; `append_to_audit_chain` integrates with R18 AuditChain as `smt-proof-cert` tagged entry. 10 new tests (format-stability + determinism + missing-solver-None + fixed-unsat-valid + tampered-query + wrong-key + sat-still-valid + chain-append + summary + verdict-shape). **Full R18 killer-app attestation stack now complete** : gate-seal + per-case SMT-cert + chain-of-custody. 1150 tests / 0 failed / 0 clippy. Remaining : vector-SDF + T11-phase-2b + Arc A770.
