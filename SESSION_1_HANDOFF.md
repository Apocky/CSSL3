...
- **Current task** Session-4 T11-D40 landed : **AUTO-MONOMORPHIZATION WALKER — generic-fn → machine-code loop closes end-to-end with no manual intervention**. T11-D38 provided the specialization API ; T11-D39 carried turbofish type-args through CST → HIR ; T11-D40 is the discovery pass. `cssl-mir/src/auto_monomorph.rs` (new ~470 LOC) : `auto_monomorphize(hir, interner, source) -> AutoMonomorphReport` walks every HIR fn body, finds turbofish Calls, dedupes by (callee, type-arg-signature), emits one specialized `MirFunc` per unique tuple. Walker covers 20+ HirExprKind variants (Call/Binary/Unary/Field/Index/Block/If/Match/Return/Break/Cast/Paren/Tuple/Array/Assign/For/While/Loop/Range/Pipeline/Try/TryDefault/Run). Integration test proves the full flow : source `fn id<T> + id::<i32>(5) + id::<f32>(2.5)` → walker → 2 MirFuncs → one JIT module → `id_i32(5)=5` / `id_i32(-42)=-42` / `id_f32(2.5)≈2.5` / `id_f32(-1.25)≈-1.25`. **First fully automatic generic-fn machine-code execution — no manual `specialize_generic_fn` call required.** Completes the generic-fn-MVP arc D38+D39+D40. 1495 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Next** : natural follow-ups are call-site rewriting in MIR bodies (so `main → id_i32` references emit the mangled name directly), generic struct/enum/impl monomorphization (needed for `struct Vec<T>`), or pivot to heap-allocation primitives. Generic-FN half of the P1 stdlib-core story is fully landed.
