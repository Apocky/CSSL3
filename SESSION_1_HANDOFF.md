...
- **Current task** Session-4 T11-D45 landed : **GENERIC STRUCT MONOMORPHIZATION MVP (decl-only)**. After the D38..D44 generic-fn arc closed end-to-end (source → HIR → auto_monomorph → rewrite → drop → JIT produces working main()), D45 extends specialization to structs. A background recon agent investigated scope + recommended "option (a) : declaration-only specialization" — parallel API to `specialize_generic_fn` for `HirStruct` items, no runtime construction (needs heap-alloc + MIR struct-value representation, both explicitly deferred). `pub fn specialize_generic_struct(interner, hir_struct, subst) -> HirStruct` + `mangle_struct_specialization_name` handle Named / Tuple / Unit bodies + nested type-args (`Box<T>` → `Box<i32>` via recursion). Capability landed : `struct Pair<T, U> { first: T, second: U }` + T↦i32, U↦f32 → `HirStruct { body: Named([first: i32, second: f32]), generics: empty }` mangled as `Pair_i32_f32`. **First generic-struct decl specialization in the compiler.** Session-4 totals : 13 commits (D33..D45) ; 1513 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Arc status** : Generic-FN callable-from-source ✓ (D38..D44) + Generic-struct decl-level ✓ (D45). **Next natural slices for P1** : auto-discovery for struct-specialization (parallel to auto_monomorphize) ; impl<T> monomorphization (walks HirImpl.fns + substitutes self_ty) ; struct-expr lowering in body_lower emitting specialized type tags ; MirType::Struct value-level representation ; heap-allocation primitives (hard blocker for Vec<T>) ; generic enums.
