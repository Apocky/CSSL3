...
- **Current task** Session-4 T11-D27 landed : **Multi-param bwd via single-adjoint extraction — reverse-mode AD runs source-to-runtime for multi-param primals**. `extract_bwd_single_adjoint(bwd, adjoint_index)` post-processes a multi-result bwd variant to extract one adjoint per call (the result at `adjoint_index` + a rewritten `cssl.diff.bwd_return` returning only that operand). New `call_f32_f32_f32_to_f32` JIT helper for canonical `(a, b, d_y) → d_x` per-param bwd shape. 2 new tests : `full_chain_source_bwd_mul_per_param_adjoints` — CSSLv3 `@differentiable fn mul(a, b) { a*b }` → extract `mul_bwd_d0` + `mul_bwd_d1` → verify `∂/∂a=b`, `∂/∂b=a` exactly at (3, 5) + chain-rule at (2, 7, 0.5) + central-diff cross-check 3 samples ; `full_chain_source_bwd_two_params_affine` — `lin2(a,b)=a+a+b` → ∂/∂a=2, ∂/∂b=1 across 3 samples. 1368 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Multi-param reverse-mode AD now runs source-to-runtime.** Full F1 correctness chain verified end-to-end for 2-param scalar primals via Fwd-mode (tangent-only) + Bwd-mode (per-param extraction). T11-D24..D27 quadrilogy complete — rigorous runtime-execution coverage across intrinsics + inter-fn + fwd + bwd. Remaining open arcs : native JIT multi-return (out-param ABI) + mutual recursion + full scene-SDF composition (`min(sphere_sdf(p,r0), sphere_sdf(p,r1))`) + libm transcendentals + stage-1 self-host.
