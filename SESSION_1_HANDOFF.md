...
- **Current task** Session-4 T11-D20 landed : **STAGE-0.5 — toolchain bump 1.75 → 1.85 + real Cranelift JIT activation + first CSSLv3-derived program executing**. `rust-toolchain.toml` pin moved to 1.85.0 (Apocky-approved via "✓ bump →" per R16 protocol). Cranelift 0.115 (codegen + frontend + module + jit + native) activated in cssl-cgen-cpu-cranelift. `jit.rs` fully rewritten (~700 LOC) : real `cranelift_jit::JITModule`, `FunctionBuilder`-based MIR → CLIF IR lowering, host-default `CallConv` (critical on Windows for `extern "C"` fn-ptr correctness), `finalize_definitions` + `get_finalized_function` for code-addr retrieval, signature-validated `JitFn::call_*` methods with safety-documented `std::mem::transmute`. **`add_i32_roundtrip_3_plus_4_equals_7 ... ok`** : the canonical stage-0.5 test — MIR `fn add(v0: i32, v1: i32) -> i32 { v0 + v1 }` JIT-compiled + executed + returns 7. Plus 15 more JIT tests : negative inputs, i64 big-integer, f32 mul, const fn `answer()->42`, guard tests. `#![forbid(unsafe_code)]` → `deny` with narrowly-scoped `#![allow]` on jit.rs ; each of 4 transmute call-sites carries a SAFETY comment. 9 new workspace clippy allowances for 1.85-surfaced lints on pre-existing docstrings/code. 1344 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **CSSLv3-derived programs now execute at runtime.** This is the stage-0 → stage-0.5 jump. Remaining open arcs : JIT-executable cmpf/select/func.call + scf.if/for control-flow + memref load/store + scene-SDF runtime-gradient verification (closes killer-app loop at runtime, not just emission-layer).
