...
- **Current task** Session-4 T11-D8 landed : **RefinedGen<G, P> rejection-sampling generator** — bridges §§ 20 refinement-types to the property-framework. `RefinedGen { inner, predicate, max_attempts }` wraps any inner generator ; `generate()` loops up to `max_attempts` drawing from inner until `predicate(&v)` holds ; `shrink()` filters inner.shrink() candidates through predicate so all shrink-results are refinement-valid. `{x : i64 | x > 0}` becomes `RefinedGen::new(IntGen { min: 0, max: _ }, |x| *x > 0)`. 6 new tests (respects-predicate, shrinks-to-valid-only, returns-last-when-unsatisfiable, custom max_attempts, refined-float positive-only, end-to-end run_property). 1258 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Generator suite now complete** : scalar (IntGen/BoolGen/FloatGen) + structural (TripleGen/VecGen) + refinement (RefinedGen). **Nine of ten oracle modes still live** (@power + @thermal + @hot_reload remain intentional-defer hw/OS-gated). Remaining open arcs : Hypothesis-style integrated shrinking + smart-refinement-compilation (eliminate rejection-sampling) + WeightedGen/OneOfGen sum-types + Faà-di-Bruno + real vec3 AnalyticExpr + scene-SDF monomorphization + Arc A770 FFI + stage3 rebuild-pipeline.
