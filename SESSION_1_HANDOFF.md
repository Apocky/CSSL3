...
- **Current task** Session-4 T11-D7 landed : **refinement-guided generators + calculus-rule metamorphic checks**. `property.rs` gains `FloatGen { min, max }` (half-open-interval f64 generator, shrink-toward-zero + halved-magnitude), `TripleGen<G>` (3-tuple generator with component-at-a-time shrinker — keeps failing-dimension info), `VecGen<G> { inner, max_len }` (variable-length vec with half-truncation + drop-last shrinker). `metamorphic.rs` gains `check_leibniz` (verifies `(f·g)' = f'·g + f·g'` via central-differences at `h = max(1e-5, |x|·1e-6)`), `check_chain_rule` (verifies `(f∘g)' = f'(g)·g'`), `check_lipschitz` (verifies `|f(x) - f(y)| ≤ k·|x - y|` — the 1-Lipschitz SDF invariant per `specs/05`). 20 new tests (12 property + 8 metamorphic). 1252 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Nine of ten oracle modes now live** + both @property + @metamorphic substrates substantially enriched. Remaining stubs : `@power` + `@thermal` + `@hot_reload` (OS/hw-specific deps intentional-defer). Remaining open arcs : RefinedGen<R> rejection-sampling + Hypothesis-style integrated shrinking + Faà-di-Bruno higher-order + real vec3 AnalyticExpr variant + scene-SDF monomorphization + Arc A770 FFI (MSVC-gated) + stage3 rebuild-pipeline for real R16-attestation.
