...
- **Current task** Session-4 T11-D19 landed : **JIT API surface + toolchain-bump gate documented**. New `jit.rs` module (~200 LOC + 8 tests) with the exact API the real JIT will expose : `JitModule::compile(&MirFunc) -> Result<JitFn, JitError>`, `JitFn::call_i64_i64_to_i64 / call_f32_f32_to_f32`, `JitError::NotActivated` (current return-path, mentions toolchain-bump) + `UnsupportedFeature` + `LoweringFailed` + `UnknownFunction`. `compile()` already validates MIR shape (rejects multi-result fns) + records handle ; only missing internal swap is real Cranelift FunctionBuilder + JITBuilder in place of stub-record. **Blocker**: cranelift 0.115 + transitive indexmap-2.14 require edition2024 = Rust 1.85+ ; toolchain pinned at 1.75.0 per R16 anchor. Attempted downgrade to cranelift 0.103 : same transitive block. Decision deferred to Apocky — a new DECISIONS entry documenting the rust-toolchain bump from 1.75 → ≥1.85 unblocks T11-D19-full : add cranelift-jit dep + implement compile() via FunctionBuilder + add add(3, 4) == 7 roundtrip + flip is_activated() → true. 1358 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **JIT API surface frozen** : when toolchain bumps, follow-on commit is pure internal swap, no public API churn. Remaining open arcs : toolchain bump decision (Apocky-gated) + cranelift activation + scf.if/for control-flow CLIF lowering + scene-SDF runtime gradient verification via JIT + central-diff comparison.
