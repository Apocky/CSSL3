...
- **Current task** Session-4 T11-D14 landed : **AD walker dispatch for min/max/abs/sign** — `walker::op_to_primitive` gains `arith.minimumf` + `arith.minf` → Primitive::Min, symmetric for max, `math.absf` / `math.abs` → Abs, `math.copysign` → Sign (MLIR proxy). `specialize_transcendental` gains callee-name matches : `min` / `math.min` / `fmin` → Min, symmetric for max/abs/sign with aliases. 2 new test-functions with 15 combined assertions. AD pipeline is now end-to-end consistent for piecewise primitives : HIR call-expr → body-lower `func.call` with `callee="min"` → MIR op recognized as Primitive::Min → rule-table dispatches Fwd/Bwd → substitute emits placeholder w/ recipe. 1315 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. Remaining open arcs : real branchful adjoint emission (arith.cmpf + arith.select) + scene-SDF-shaped end-to-end MIR gate + real vec3 MIR lowering + integrated shrinking + Arc A770 FFI + stage3 rebuild-pipeline.
