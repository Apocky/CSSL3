...
- **Current task** Session-4 T11-D15 landed : **real branchful adjoint emission for Min/Max/Abs/Sign** — `substitute.rs` replaces `cssl.diff.{fwd,bwd}_placeholder` with real `arith.cmpf + arith.select + arith.constant + arith.negf` chains. Min Fwd : `d_y = select(cmp "ole" a b, d_a, d_b)`. Max Fwd : symmetric with `"oge"`. Abs Fwd : const 0 + cmpf "oge" x 0 + negf d_x + select → `d_y = select(x ≥ 0, d_x, -d_x)`. Sign Fwd : const 0.0 (zero gradient a.e.). Bwd mirror : cmpf + 2 selects + 2 addfs route d_y to winning branch ; Sign Bwd is no-op. 8 new tests verify emission shape : fwd_min has `predicate=ole` + tangent-select ; fwd_abs has full const+cmpf+negf+select chain ; fwd_sign has zero-const ; bwd_min has ≥ 2 adjoint selects ; bwd_sign is no-op ; regression-guard : `min` no longer emits placeholder. 1323 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **AD primitives + emission now end-to-end executable MIR** : HIR `min(a, b)` call → body_lower func.call(callee=min) → walker recognizes Primitive::Min → substitute emits real branchful tangent + adjoint → backend lowers to SSE BLENDPS / SPIR-V OpSelect. Remaining open arcs : body_lower integration test (full @differentiable fn with min call) + real vec3 MIR lowering + integrated shrinking + Arc A770 FFI + stage3 rebuild-pipeline.
