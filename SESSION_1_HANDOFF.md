...
- **Current task** Session-4 T11-D11 landed : **AnalyticExpr::Abs + Sign + smooth_min + cusp-detection** — `AnalyticExpr` gains `Abs(Box<AE>)` + `Sign(Box<AE>)` unary variants wired through simplify/evaluate/to_term/to_smt/collect_vars ; `analytic_vec3.rs` gains `smooth_min(a, b, k) = -log(exp(-k·a) + exp(-k·b))/k` (differentiable everywhere ; rounded-edge scene-SDF per `specs/05 § APPENDIX-SMOOTH`) + `is_near_cusp(a, b, env, ε)` sampler-guard. Smooth_min central-diff at cusp x=0 equals exactly 0.5 (midpoint of sharp-min subgradient) — verified numerically. 11 new tests (Abs/Sign evaluate + constant-fold + SMT output ; smooth_min symmetric + converges to min at high k + differentiable at cusp ; is_near_cusp detects close + treats NaN as cusp-adjacent). 1298 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **AnalyticExpr primitive-set now complete** for every scene-SDF operator per specs/05 (Min/Max/Abs/Sign/smooth_min/length/dot/normalize all first-class). Remaining open arcs : smooth_max + tri-min/max n-ary + MIR Min/Max/Abs/Sign primitives with subgradient AD rules + real vec3 MIR lowering + integrated shrinking + Arc A770 FFI + stage3 rebuild-pipeline.
