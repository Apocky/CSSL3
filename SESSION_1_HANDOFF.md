...
- **Current task** Session-4 T11-D24 landed : **JIT intrinsic func.call dispatch + scene-SDF min/max/abs source-driven gradient verification**. `lower_op_to_cl` now dispatches `func.call` for known math intrinsics : `min/max/abs/sqrt/neg` map to cranelift `fmin/fmax/fabs/sqrt/fneg` natively ; transcendentals (sin/cos/exp/log) rejected with `UnsupportedMirOp` pending libm externs ; user-defined callees rejected pending T11-D26 inter-fn calls. `body_lower::lower_call` now infers concrete scalar result-types for known-intrinsic callees (operand[0]'s type) rather than always emitting `Opaque("!cssl.call_result.*")` — this fixes walker-emitted successor ops (e.g., `arith.constant 0.0` in `emit_abs_fwd`) which previously inherited the opaque type and failed JIT lowering. 3 new full-chain tests : `full_chain_source_scene_sdf_min_runtime_gradient` (exact pick-the-winner verdicts at `(3,5)` + `(8,2)` + central-diff at 5 sample points), `full_chain_source_scene_sdf_max_runtime_gradient`, `full_chain_source_scene_sdf_abs_runtime_gradient` (∂|a|/∂a = sign(a) verified for positive + negative). 1359 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Piecewise-linear scene-SDF primitives (min, max, abs) now run source-to-runtime with verified gradients.** Remaining open arcs : Bwd-mode full-chain integration (T11-D25) + multi-fn scene SDFs + JIT native multi-return + libm-backed transcendentals + stage-1 self-host.
