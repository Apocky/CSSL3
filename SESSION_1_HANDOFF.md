...
- **Current task** Session-4 T11-D23 landed : **FULL CHAIN — CSSLv3 source → walker → JIT → gradient-verified**. New `cssl-examples/src/jit_chain.rs` (~300 LOC + 4 tests) with `pipeline_source_to_ad_mir(source)` running lex → parse → HIR → MIR → walker to produce a full MirModule with `_fwd`/`_bwd` variants, `extract_tangent_only_variant(fwd)` post-processing walker output to strip the multi-result primal so the JIT can execute tangent-only, and the full-chain test `full_chain_source_to_jit_sphere_sdf_gradient` which takes the raw CSSLv3 source `@differentiable fn sphere_sdf(p : f32, r : f32) -> f32 { p - r }` all the way to JIT-executed gradient verification — tangent returns exactly 1.0 for ∂/∂p and -1.0 for ∂/∂r, matches central-differences at 4 sample points. Chain-rule verified via `fmul_gradient` : ∂(a*b)/∂a = b, ∂(a*b)/∂b = a. Two architectural fixes : (1) walker `substitute_fwd` now appends tangent to `func.return` operands making multi-result fwd variants body-consistent with their signature ; (2) JIT `compile` wires block-params by iterating `entry_block.args` rather than assuming sequential ValueIds 0..n (walker uses non-sequential IDs after `synthesize_tangent_params` interleaving). 1356 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **The AD walker's runtime output is now directly executable.** No hand-built MIR shortcut needed. Remaining open arcs : walker-emit scene-SDF `min(a, b)` source-driven test (composes T11-D21 + T11-D23 — architecture complete) + Bwd-mode integration + multi-fn scene SDFs with inter-fn JIT calls + JIT multi-return (remove tangent-only stripping) + stage-1 self-host.
...
- **T12-D2** : T12-phase-2a killer-app end-to-end F1-chain integration-test — `cssl_examples::F1ChainOutcome` + `run_f1_chain(name, src)` + `run_f1_chain_all()` that wires lex→parse→HIR→AD-legality→refinement-obligations→MIR-lowering→AD-walker→predicate-translator into single call. 9-counter outcome + 8 integration-tests validate the F1 chain composes on real CSSLv3 source (sdf_shader + audio_callback + hello_triangle). **F1-correctness chain now has test-driven invariants** — every future stage-touching commit must preserve run_f1_chain_all outcomes. Phase-2c : T7-phase-2b dual-substitution + T9-phase-2b Lipschitz + T12-phase-2c vertical_slice.cssl
- **T11-D2** : T11-phase-2a real BLAKE3 + Ed25519 replacing stubs — `ContentHash::hash` real BLAKE3 + `SigningKey::{generate, from_seed, verifying_key_bytes, verify}` Ed25519 wrapper + `Signature::sign(&SigningKey, bytes)` + `AuditChain::with_signing_key(key)` optional-signer. `verify_chain` now detects tampered-messages via Ed25519 verification. Stubs retained for tests. Cargo.lock pins : blake3 1.5.4 / ed25519-dalek 2.1.1 / base64ct 1.6.0 / cpufeatures 0.2.17 for 1.75 MSRV compat per T1-D4. **R18 audit-chain now cryptographically real** — third-party verifiable. Phase-2b deferred : OTLP transport + cross-thread atomic-ring + L0-sysman sampling + WAL/LMDB backends + @hot_reload_preserve + oracle-bodies
- **T3-D12** : T3.4-phase-3-IFC — `cssl_hir::ifc` with `IfcLabel { confidentiality, integrity }` + full lattice algebra (⊑ / ⊔ / ⊓) + 9 built-in PRIME_DIRECTIVE principals (HarmTarget/Surveiller/Coercer/Weaponizer/System/Kernel/User/Public/Anthropic-Audit) + `IfcDiagnostic` w/ 3 codes (IFC0001 MissingLabel / IFC0002 MissingDeclassPolicy / IFC0003 UnauthorizedDowngrade) + `check_ifc(&HirModule, &Interner) -> IfcReport` structural walker + `IfcLabelRegistry`. Same walker pattern as AD-legality (T3-D11). Phase-3-IFC-b deferred : full type-level `secret<T, L>` propagation + branch-cond IFC + declass-policy resolution + covert-channel mitigations + IfcLoweringPass MIR integration
- **T6-D4** : T6-phase-2b HIR-body-lowering expanded — 15 additional HirExprKind variants : For/While/Loop/Match (scf.for/while/loop/match) + Field (cssl.field) + Index (memref.load) + Assign / compound-assign (cssl.assign[_add/_sub/_mul/_div]) + Cast (arith.bitcast) + Tuple (cssl.tuple) + Array (cssl.array_list/cssl.array_repeat) + Struct (cssl.struct) + Pipeline (cssl.pipeline) + TryDefault (cssl.try_default) + Try (cssl.try) + Range (cssl.range/cssl.range_inclusive) + Run (transparent pass-through). Real-lowering coverage now ~25 of 31 variants. Phase-2c remaining : Lambda / Perform / With / Region / Compound / SectionRef + real-literal-value + type-propagation + scf.break labels
- **T9-D3** : T9-phase-2b Lipschitz arithmetic-interval encoding — `parse_lipschitz_bound` (bare-int / decimal / `k = N` forms) + `ObligationKind::Lipschitz` translates to (set-logic QF_LRA) + (declare-fun x/y ()) + uninterpreted f + (assert (not (<= (abs (- (f x) (f y))) (* k (abs (- x y)))))). **Zero `UnsupportedKind` fallbacks remaining** — every obligation-kind produces a concrete SMT query. Phase-2c : inline Lipschitz-decomposition (Sum/Product/Composition rules) + multi-dim + automatic bound-inference
- **T7-D4** : T7-phase-2b real dual-substitution — new `cssl_autodiff::substitute` module (~1200 LOC) with `TangentMap` + `apply_fwd(primal, rules) → (fwd_variant, TangentMap, SubstitutionReport)` + `apply_bwd(primal, rules) → (bwd_variant, TangentMap, SubstitutionReport)`. Fwd emits real tangent-carrying `arith.{addf,subf,mulf,divf,negf}` + `func.call` ops per primitive chain-rule (FMul fwd = 2 mulfs + 1 addf ; FDiv fwd = 2 mulfs + 1 subf + 1 mulf + 1 divf ; Sqrt fwd = constant 2.0 + mulf + divf ; etc) ; bwd reverse-iterates emitting adjoint-accumulation ops + `cssl.diff.bwd_return` terminator carrying adjoint-outs for each primal float-param. Variant signatures synthesized : fwd `[a, d_a, b, d_b, ...] → [y, d_y]` ; bwd `[a, b, d_y] → [d_a, d_b]`. `walker.rs` rewired to delegate to apply_fwd/apply_bwd ; `AdWalkerReport` extended with tangent_ops_emitted + tangent_params_added. 21 new unit-tests (10 per-primitive fwd + 3 bwd + 8 structural/helper). **`sphere_sdf_fwd`/`sphere_sdf_bwd` now contain real `arith.*` ops ready for Z3 unsat-verdict verification** — killer-app gate (T7-phase-2c) unblocked. Phase-2c : tape-buffer for scf.* control-flow + `@checkpoint` + GPU-AD tape-location + multi-result tangent-tuple + bit-exact-vs-analytic SMT gate
- **T7-D5** : T7-phase-2c KILLER-APP GATE — **11/11 cases pass ✓** structural gradient equivalence. New `cssl_examples::ad_gate` module (~1100 LOC) with `AnalyticExpr` (Const/Var/Neg/Add/Sub/Mul/Div/Sqrt/Sin/Cos/Exp/Log algebra + simplify + evaluate + to_smt) + `MirAdjointInterpreter` (walks bwd-variant body + reconstructs `AnalyticExpr` per adjoint-out) + `verify_gradient_case(name, primal, param_names, analytic_gradients)` + `run_killer_app_gate() → KillerAppGateReport`. Canonical gate covers FAdd/FSub/FMul/FDiv/FNeg + Sqrt/Sin/Cos/Exp/Log + sphere-sdf scalar surrogate + chain-rule `(x-r)²` with sampling-based equivalence across 11-point deterministic env set (NaN-both-sides = skip, one-NaN = mismatch). `substitute.rs` augmented with zero-init adjoint emission at bwd-start + inline zero-init for intermediate-value adjoints + self-reference serialization (a-update before prev_d_b read) — correctly handles `x*x` and chain-rule intermediates. SMT-LIB query text emission (`GradientCase::smt_query_text`) ready for Z3/CVC5 stretch-path. 24 new tests. **PUBLISHABLE F1-correctness claim** reproducible via `cargo test -p cssl-examples ad_gate`. Phase-2d : vector-SDF + scene-SDF union/min + Z3-subprocess dispatch + R18 AuditChain signing + Arc A770 bit-exact driver-matrix verification
- **T7-D6** : T7-phase-2d-R18 — **R18 AuditChain signing of killer-app gate report**. `SignedKillerAppGateReport` wraps `KillerAppGateReport` with `canonical_payload: Vec<u8>` + `content_hash: ContentHash` (BLAKE3) + `signature: Signature` (Ed25519) + `verifying_key: [u8; 32]` + `format: String` (= `"CSSLv3-R18-KILLER-APP-GATE-v1"`). `canonical_report_bytes` : line-oriented UTF-8 serializer with stable field-ordering (third-party-auditor reconstructs exact byte-sequence from plain-text report). `sign_gate_report(report, &SigningKey)` produces signed bundle ; `verify_signed_gate_report(signed, expected_vk) → AttestationVerdict` returns 4-step verdict (format_matches / payload_hash_matches / signature_verifies / gate_is_green) with `is_fully_valid()` (all 4) + `cryptographically_valid()` (first 3) threshold helpers. `cssl_telemetry::verify_detached` added + `SigningKey` / `ContentHash` / `Signature` re-exported from lib.rs. 11 new attestation tests (roundtrip / wrong-key / tampered-payload / tampered-format / tampered-signature / deterministic-signing / cryptographically-valid-accepts-failed-gate). Test count : 1027 → 1038 (+11). Publishable F1-correctness claim now cryptographically sealed — publish verifying-key + signed-report, auditor re-hashes + verifies independently. Phase-2d-remaining : vector-SDF + scene-SDF monomorphization + Z3-subprocess dispatch + T6-phase-2c HirExprKind widening + @staged check + Arc A770 bit-exact verification
- **T9-D4** : T9-phase-2c-partial Solver::check_text bridge — `run_cli_text(kind, smtlib, args) → Result<Verdict>` free fn pipes raw SMT-LIB text through Z3/CVC5 subprocess + `Solver::check_text` default trait method dispatches through it. `default_args_for(kind)` helper for canonical solver args. `cssl-examples` picks up `cssl-smt` dep + `ad_gate` gains `SmtVerification { case_name, verdict, solver_kind }` + `SmtVerificationReport { verifications, unavailable, unsat/sat/unknown_count }` with `all_decided_cases_proved()` + `summary()`. `GradientCase::run_smt_verification(&dyn Solver)` emits query-text, dispatches, wraps `Option<SmtVerification>` (None on BinaryMissing / subprocess-fail). `KillerAppGateReport::run_smt_verification(&dyn Solver)` aggregates across cases. 11 new tests — MissingBinarySolver + FixedVerdictSolver stubs exercise availability/unsat/sat paths + real Z3CliSolver dispatch is resilient. **Killer-app gate can now verify equivalence via 3 orthogonal paths : simplify + sampling + SMT unsat-verdict** — all three must agree for the F1-correctness claim to land. Test count : 1038 → 1049 (+11). Phase-2d : full AnalyticExpr→Term translator + Z3-timeout config + proof-cert emission + R18 signing of SMT verdict
- **T3-D13** : T3.4-phase-3-staged (agent-authored, cherry-picked to main) — `cssl_hir::staged_check::check_staged_consistency(&HirModule, &Interner) → StagedReport` structural walker mirroring T3-D11 AD-legality + T3-D12 IFC. `StageClass { CompTime / Runtime / Polymorphic }` + `StageEntry { def, class, param_count, span }` + `StageRegistry` (HIR-self-contained ; no cssl-staging dep). `StagedCode { STG0001 MissingStageClass, STG0002 StageClassMismatch, STG0003 CyclicStagedDependency }` + `StagedReport { diagnostics, checked_fn_count, cyclic_edges }`. 4-pass walker : collect → class-validate → call-site-validate → cycle-detect via DFS. 25 new tests (missing-class / 3 accepted classes / mismatch / acyclic / self-recursion / 3-fn cycle / non-staged-skip / registry / report). **All three T3.4-phase-3 walkers (AD-legality + IFC + @staged) now share unified `check_<concern>(&HirModule, &Interner) → Report` API**. Test count : 1049 → 1074 (+25). Phase-3-remaining : macro-hygiene + let-generalization slices
- **T6-D5** : T6-phase-2c HirExprKind completion (agent-authored, cherry-picked to main) — 6 remaining variants land (`lower_lambda` → `cssl.closure` / `lower_perform` → `cssl.effect.perform` / `lower_with` → `cssl.effect.handle` / `lower_region` → `cssl.region.enter` / `lower_compound` → `cssl.compound` / `lower_section_ref` → `cssl.section_ref`). Body-lowering coverage : 25/31 → 31/31 variants. **Real literal-value extraction** replaces `"stage0_int"` / `"stage0_float"` placeholders — `BodyLowerCtx` gains `source: Option<&SourceFile>` ; `lower_fn_body(&Interner, Option<&SourceFile>, &HirFn, &mut MirFunc)` now parses Int / Float / Bool / Str / Char literals from source-text spans, falls back to placeholders only when source absent or parse fails. `walker.rs` tests pass `None` (AD shape-only). `cssl-examples run_f1_chain` passes `Some(&file)` for real literals. Test count unchanged at 1074 — existing test infra indirectly covers via F1 chain ; dedicated per-variant unit tests deferred to follow-up. Phase-2d-remaining : closure-env capture + Xie+Leijen evidence + scf.br label-targeting + per-variant dedicated tests
- **T9-D5** : T9-phase-2d structured-SMT translator — `AnalyticExpr::to_term(&self) → cssl_smt::Term` recursive translator (Const integer-valued → `(/ n 1)` rational, fractional → `(/ num 10⁶)` approximation, Var / Neg / Add / Sub / Mul / Div → std operators, 5 transcendentals → `<fn>_uf` UF applications, NaN / ±∞ → sentinel vars). `f64_to_term` helper. `GradientCase::to_smt_query(&self) → Query` builds proper `Query` : theory `ALL` + free-var declarations + 5 transcendental UF declarations + single named assertion `gradient_equivalence_<sanitized>` holding the negated-equivalence term. `sanitize_label` replaces non-alphanumeric chars. `GradientCase::run_smt_verification_via_query(&self, &dyn Solver)` parallel dispatch path via `Solver::check` (complements existing text-form `run_smt_verification`). 13 new tests (to_term shape × 7 variants + to_smt_query declarations + label-sanitize + missing-solver None-path + FixedVerdict wrapping + killer-app round-trip + text ≡ query var-decl parity). **Killer-app gate now composable with full cssl_smt infrastructure** : labeled assertions enable unsat-core extraction + proof-cert hooks + incremental-solving. Test count : 1074 → 1087 (+13). Phase-2d-remaining : interpreted-transcendental axioms + Literal::Decimal encoding + HIR-Expr→Term for general obligation discharge + proof-cert emission + R18 signing
- **T7-D7** : T7-phase-2d-audit AuditChain integration — `SignedKillerAppGateReport::audit_tag()` → `"killer-app-gate"` stable tag + `audit_message()` → compact `"<format> hash=<64-hex> verdict=<n/m/{green|red}> vk=<64-hex>"` record + `append_to_audit_chain(&mut AuditChain, timestamp_s)` single-call integration. 6 new tests (tag-stability / message-format / failing-gate-reflects-red / single-append-verifies / multi-run-sequential-chain / signed-chain-integration). Gate-verdicts now land as tagged entries in R18 append-only chain alongside other audit events ; multi-run sequences show full verdict trajectory third-party-auditable. Composes with `AuditChain::with_signing_key` for per-entry Ed25519 signing. Test count : 1087 → 1093 (+6). Phase-2d-remaining : OTLP gRPC export + proof-cert embedding + cross-chain references
- **T6-D6** : T6-phase-2c coverage — dedicated per-variant tests (14 new). Covers Lambda (3 : cssl.closure emission + param_count attr + nested region), Perform (2 : cssl.effect.perform + effect_path/arg_count attrs), With (2 : cssl.effect.handle + body-region), Region (2 : cssl.region.enter + label attr), SectionRef (1 : infrastructure no-panic), + literal-value extraction (4 : Int/Float/Bool real values vs stage0_* no-source fallback). Closes the "indirect F1-chain coverage" defer from T6-D5. Test count : 1093 → 1107 (+14). cssl-mir specifically : 81 → 95. Regression safety : future refactors of the 6 lowerings trip named tests before reaching integration tests
- **T3-D14** : T3.4-phase-3-let-gen-foundation — `Scheme { ty_vars: Vec<TyVar>, row_vars: Vec<RowVar>, body: Ty }` rank-1 polymorphic wrapper + `Scheme::monomorphic(body)` + `is_monomorphic()` + `rank()` + `bound_ty_vars()` + `bound_row_vars()` inspectors + `Scheme::instantiate(&mut TyCtx) → Ty` HM "inst" (documented caller invariant : ctx.next_ty > max bound-var) + `generalize(env_free_ty, env_free_row, ty) → Scheme` HM "gen" (generic over HashSet hashers per clippy pedantic) + `free_ty_vars(&Ty) → Vec<TyVar>` + `free_row_vars(&Ty) → Vec<RowVar>` recursive walkers. Re-exported from cssl-hir/src/lib.rs. 14 new tests (primitives / tuples / fns dedupe / row-var collection / pure-row / monomorphic / identity-fn generalize / env-fixed vars / fresh-var allocation / two-instantiations-distinct / roundtrip). **Foundation-only** — no integration into `check_module` inference yet (T3-D15). Test count : 1107 → 1121 (+14). cssl-hir typing tests : 11 → 25
- **T3-D15** : T3.4-phase-3-let-gen integration — `TypeScope.bindings` stores `HashMap<Symbol, Scheme>` internally ; legacy `insert(Ty)` / `lookup() → Option<&Ty>` preserved via auto-monomorphic-wrap. `insert_scheme` / `lookup_scheme` / `schemes()` iterator added for polymorphic callers. `TypingEnv` : `insert_local_scheme` / `lookup_local_scheme` / `free_ty_vars` / `free_row_vars` helpers. `infer.rs::check_stmt::Let` now calls new `bind_pattern_let` helper (generalize-then-insert for simple Binding patterns ; falls through to monomorphic `bind_pattern` for destructuring). `synth_expr_kind::Path` uses `lookup_local_scheme` + `Scheme::instantiate(&mut tcx)` for fresh vars per use-site. Fixed staged_check.rs rustdoc ASCII-table → wrapped in `text` fence. 6 new tests (let-lambda / monomorphic / annotated / nested-shadow / fresh-vars-per-use / empty-env-free-vars). Classical let-polymorphism + backward-compat ≡ both demonstrated. Test count : 1121 → 1127 (+6). cssl-hir lib tests : 155. **HM let-generalization active in live inference flow**
...
- **§§ 01_BOOTSTRAP REPO-LAYOUT spec-delta** : workspace layout supersedes single-crate. Spec-corpus update pending Apocky approval per §§ HANDOFF_SESSION_1 REPORTING ("W! coordinate-with-Apocky before-editing-specs").
