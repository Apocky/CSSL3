...
- **Current task** Session-4 T11-D38 landed : **GENERIC MONOMORPHIZATION MVP — first generic-fn machine-code execution in the compiler**. `cssl-mir/src/monomorph.rs` (new ~470 LOC) provides the specialization core : `TypeSubst` (generic-param → concrete-type map), `substitute_hir_type` (recursive type-tree walker handling 10 variants), `mangle_specialization_name` (deterministic `{fn}_{args}` naming), `specialize_generic_fn` (clone + substitute + lower to MirFunc). Integration test proves end-to-end : `fn id<T>(x : T) -> T { x }` parses through HIR, specializes with T↦i32 AND T↦f32 producing `id_i32` + `id_f32` MirFuncs, both JIT-compile in one module, `id_i32(5)=5`, `id_i32(-42)=-42`, `id_f32(2.5)≈2.5`. Unblocks P1 stdlib-core at the core-machinery level — writing Vec<T>/HashMap<K,V> in CSSLv3 still needs turbofish call-site wiring + heap-alloc primitives + trait-dispatch, but the specialization API is present + validated. **Fourth slice of user-directed "2 → 1 → 4 → 3" sequence complete.** Arc status : D34 SPIR-V validation ✓ / D35 vec3 wire-through ✓ / D36 IFC flow-violation ✓ / D37 vec arc polish ✓ / D38 generic monomorphization MVP ✓. 1473 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Next** : checkpoint with user. Natural follow-ups : turbofish parsing (call-site type-args) → auto-monomorphization walker → generic struct/enum specialization → `struct Vec<T>` in CSSLv3 → `impl<T> Vec<T>` → first real stdlib-type.
