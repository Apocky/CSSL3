...
- **Current task** Session-2 T3-D14 landed : **Scheme + generalize/instantiate foundation** for Hindley-Milner let-generalization. `Scheme { ty_vars, row_vars, body }` rank-1 polymorphic wrapper + `Scheme::instantiate(&mut TyCtx) → Ty` HM "inst" step + `generalize(env_free_ty, env_free_row, ty) → Scheme` HM "gen" step + `free_ty_vars` / `free_row_vars` recursive walkers. 14 new tests cover primitives / tuples / fns / rows / rank-0 / rank-1 / env-fixed vars / fresh-var distinctness / roundtrip. Foundation-only commit — T3-D15 will integrate into `check_module` inference flow. 1121 tests / 0 failed / 0 clippy. Remaining phase-2d : let-gen integration (T3-D15) + vector-SDF + scene-SDF monomorphization + macro-hygiene + HIR-Expr→Term for general obligation discharge + Arc A770 bit-exact verification + OTLP export.
