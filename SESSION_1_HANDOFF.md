...
- **Current task** Session-4 T11-D9 landed : **Vec3 AnalyticExpr algebra** — new `compiler-rs/crates/cssl-examples/src/analytic_vec3.rs` module (~400 LOC + 20 tests). `AnalyticVec3Expr` enum with Const/Var/Neg/Add/Sub/ScalarMul/ScalarDiv/Normalize variants ; `to_scalar_components()` bridges to existing `AnalyticExpr` tree so every vec3 op reduces to three scalar expressions ; `length(v)` + `dot(a, b)` + `vec3_proj(v, comp)` free functions return `AnalyticExpr` so existing `simplify` / `evaluate` / `to_term` / `to_smt` / `equivalent_by_sampling` compose transparently. Killer-app primal `sphere_sdf_vec3(p, r) = length(p) - r` + gradients `sphere_sdf_grad_p(p, d_y) = normalize(p)·d_y` + `sphere_sdf_grad_r(d_y) = -d_y` now first-class expressions. 20 new tests covering : componentwise arithmetic, `length(3,4,0)=5`, dot product, proj extraction, sphere-SDF primal at `p=(3,4,0) r=2 → 3`, grad_p equals `(0.6, 0.8, 0.0)·d_y`, central-diff numerical agreement, to_scalar_components roundtrip. 1278 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. Vec3 algebra now ready for scene-SDF monomorphization + MIR vec3 lowering (T11-D10). Remaining open arcs : MIR Vec3F32 + Vec3{Add,Sub,Neg,ScalarMul,Dot,Length,Normalize} primitives + SceneSDFExpr with piecewise-min + AnalyticVec3Expr to_smt/to_term + full constant-folding + integrated shrinking + Arc A770 FFI + stage3 rebuild-pipeline.
