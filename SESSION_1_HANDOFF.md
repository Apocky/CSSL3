...
- **Current task** Session-4 T11-D36 landed : **IFC FLOW-VIOLATION DETECTION — the compiler rejects real non-interference violations**. Previously the IFC walker was structural-only (signature-level `@sensitive` / `@confidentiality` presence checks). T11-D36 adds a dataflow walker that traces `@sensitive` params through 15 HirExprKind variants (Binary, Unary, Call, If, Match, Let, Cast, Tuple, Array, …), propagates taint via union-based `combine_labels`, and emits `IFC0004 FlowViolation` when a sensitive param reaches the fn's return expression without `@confidentiality` label or `@declass + @requires` authority. `check_ifc_full(module, interner)` runs both walkers ; `check_ifc_flow` solo. 15 new tests covering clean baselines, simple leaks, declass authority, binary-op propagation, let-bindings, if-arms, literal returns, cast/unary preservation, combined diagnostics, signature-only skip. Closes `specs/THEOREMS.csl` T8 non-interference *structural-runtime* gap (mechanized proof still stage-1). Third slice of user-directed "2 → 1 → 4 → 3" sequence complete. 1433 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Next** : #4 P1 stdlib-core — generic monomorphization + `Vec<T>` + `HashMap<K,V>` implementable in CSSLv3 (multi-session work — first step on the self-host path).
