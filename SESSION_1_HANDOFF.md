...
- **Current task** Session-4 T11-D13 landed : **Primitive Min/Max/Abs/Sign + piecewise AD rules** — `cssl_autodiff::Primitive` enum gains 4 new variants (ALL bumped to 19) with Fwd/Bwd entries in `DiffRuleTable::canonical` : Min/Max pick-the-winner (`if x_0 <= x_1 { dx_0 } else { dx_1 }`), Abs uses `sign(x) * dx`, Sign is `dy = 0` by convention. `substitute.rs` extends the stage-0 placeholder fallback to cover them — `cssl.diff.{fwd,bwd}_placeholder` ops carry `primitive` + `recipe` attributes. Test counts updated : `all_nineteen_primitives`, `canonical_table` → 38 rules, `rules_table_pre_populated` → 38. 6 new tests : Min Fwd conditional-form ✓ ; Max symmetric ✓ ; Abs uses sign(x) ✓ ; Sign is zero-gradient ✓ ; both modes registered for each new primitive. 1313 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **AD walker now recognizes min/max/abs/sign at MIR level**. Remaining open arcs : real branchful adjoint bodies (replace fwd_placeholder with arith.cmpf + arith.select) + body_lower mapping math.min HIR call → Primitive::Min MIR op + real vec3 MIR lowering + integrated shrinking + Arc A770 FFI + stage3 rebuild-pipeline.
