...
- **Current task** Session-4 T11-D25 landed : **Bwd-mode full-chain integration — reverse-mode adjoints run source-to-runtime**. Two fixes : (1) walker's `substitute_bwd` now filters out primal `func.return` before appending bwd ops (the primal return was a mid-stream terminator causing "cannot add instruction to block already filled" panic) ; (2) JIT `lower_op_to_cl` recognizes `cssl.diff.bwd_return` as a terminator alongside `func.return`, emitting cranelift `return_()` identically. 3 new full-chain tests : `full_chain_source_bwd_sq_adjoint` (`fn sq(x) { x * x }` → d_x = 2x·d_y verified analytically + central-diff across 5 points), `full_chain_source_bwd_cube_adjoint` (x³ → 3x², yields 12 at x=2 and 27 at x=-3), `full_chain_source_bwd_affine_adjoint` (x+x+x → constant 3). 1362 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Both Fwd + Bwd modes run end-to-end from source.** For single-float-param primals, bwd variant `(x, d_y) -> d_x` directly uses `call_f32_f32_to_f32`. Remaining open arcs : T11-D26 multi-fn scene SDFs + inter-fn JIT calls + T11-D27 multi-result JIT (for multi-param bwd variants like `sphere_sdf(p, r)` returning both adjoints) + libm-backed transcendentals + stage-1 self-host.
