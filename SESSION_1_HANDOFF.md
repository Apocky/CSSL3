...
- **Current task** Session-4 T11-D3 landed : **live `@property` + `@metamorphic` oracle bodies** (no external deps). `property.rs` now runs a deterministic Lcg PRNG (Knuth-multiplier 6364136223846793005) driving a `Generator<T>` trait + `run_property` runner with greedy shrinker ; `IntGen` + `BoolGen` concrete impls with type-directed shrink-toward-origin. `metamorphic.rs` now exposes four generic algebraic-law runners : `check_commutative` + `check_associative` + `check_distributive` + `check_idempotent`, each returning `Outcome::Ok { samples_tested }` or `Outcome::Violation` with debug-formatted counter-sample. 21 new tests (12 property + 9 metamorphic) — replay-safety verified (same seed + same generator → identical stream), shrink-toward-small-odd verified, distributivity + commutativity + associativity verified for i64 arith, bool-and commutative. 1180 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. Remaining : refinement-guided generators + Leibniz + Lipschitz metamorphic specializations + Hypothesis-style integrated shrinking + real vec3 variant + scene-SDF monomorphization + Arc A770.
