...
- **Current task** Session-4 T11-D5 landed : **live `@audit_test` + `@r16_attestation` + `@bench` oracle bodies** (`cssl-telemetry` now cssl-testing dep). `audit.rs` wraps `AuditChain::verify_chain()` + filters by optional `domain_filter` + verifies required-event `(domain_prefix, kind_substring)` pairs are present. `r16_attestation.rs` adds `Attestation::canonical_bytes` / `build_signed` / `verify` / `content_hash` + `decide_attestation()` helper that maps `(expected, actual, version, commit, key)` → `Attested { signed-record } | Diverged | NoSigningKey`. `bench.rs` adds `run_bench_vs_baseline<F>` (timing-harness via `Instant::now()`) + `median_ns` (sort-based) + `classify(median, baseline, threshold)` pure-data regression-checker + `update_baseline()` writer with `<root>/<bench_id>/latest.txt` plain-integer format. 23 new tests (6 audit + 8 r16_attestation + 9 bench). 1226 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. **Eight of ten oracle modes now live** : `@property` `@metamorphic` `@replay` `@differential` `@golden` `@audit_test` `@r16_attestation` `@bench` ; remaining stubs : `@power` + `@thermal` + `@hot_reload` + `@fuzz` (all hw/OS/fuzzer-specific). Remaining open arcs : refinement-guided generators + Leibniz + Lipschitz metamorphic specializations + Hypothesis-style integrated shrinking + real vec3 AnalyticExpr variant + scene-SDF monomorphization + Arc A770 FFI (MSVC-gated) + stage3 rebuild-pipeline for real R16-attestation.
