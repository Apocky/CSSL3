...
- **Current task** Session-4 T11-D4 landed : **live `@replay` + `@differential` + `@golden` oracle bodies** (no external deps). `replay.rs` runs `config.n` replays with the same Lcg seed (bound from T11-D3), flagging `Divergence { replay_index, diff_bytes }` on first mismatch (hidden-state + counter-based impls correctly caught). `differential.rs` adds `check_two_impls<T, U, A, B, Eq>` comparator abstract over `Backend` labels, plus `ulp_diff_f32` (total-ordered sortable_u32 bit-distance ; NaN → u32::MAX ; +0.0/-0.0 → 1-ULP adjacency) + `ulp_tolerant_eq_f32(tolerance)` closure factory. `golden.rs` gains pure-byte-exact mode : `compare_bytes_to_golden` + `compare_bytes_against` + `compute_byte_metrics` + `update_golden` — length-mismatch counts toward diff ; SSIM/FLIP deferred to image-decode phase. 23 new tests (6 replay + 8 differential + 9 golden). 1203 tests / 0 failed / 0 clippy / 0 rustdoc / 0 xref. Five of ten oracle modes now live ; remaining stubs : `@audit` + `@r16_attestation` (thin wire-ups to telemetry / ad_gate) + `@bench` + `@power` + `@thermal` + `@hot_reload` + `@fuzz` (timing / hw / fuzzer-specific). Remaining open arcs : refinement-guided generators + Leibniz + Lipschitz metamorphic specializations + Hypothesis-style integrated shrinking + real vec3 variant + scene-SDF monomorphization + Arc A770.
