...
pub struct BodyLowerCtx<'a> {
    ...
    /// T11-D64 (S6-C5) : let-binding name → lowered ValueId + MirType.
    /// Populated by `HirStmtKind::Let` with a `Binding`-pattern name and
    /// consumed by the path-lookup helper (after [`Self::param_vars`]) + by
    /// the lambda free-var resolver. Stage-0 is a flat map ; later-
    /// shadowing overwrites by construction (single-pass lowerer ; no
        ...
        HirStmtKind::Let { value, pat, .. } => {
            if let Some(e) = value {
                if let Some((vid, ty)) = lower_expr(ctx, e) {
                    // § T11-D64 (S6-C5) : bind the let-pattern's name → its
                    //   lowered ValueId so subsequent path-refs resolve, AND
                    //   so closure free-var analysis can map a captured local
                    //   to its source SSA-value. Only `Binding`-patterns
...
/// Lower `|params| -> Ty { body }` into the `cssl.closure` op, complete with
/// env-capture analysis + heap-allocated env-pack (S6-C5 / T11-D64).
///
/// § PIPELINE
...
/// § T11-D64 (S6-C5) — free-var collector for a lambda body.
///
...
fn lower_path(ctx: &mut BodyLowerCtx<'_>, segments: &[Symbol], span: Span) -> (ValueId, MirType) {
    // Single-segment path : check param_vars first, then local_vars (T11-D64).
...
mod tests {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // § T11-D64 (S6-C5) — Closure env-capture lowering tests.
    //
    ...
    fn closure_op(f: &crate::MirFunc) -> &crate::MirOp {
        f.body
            .entry()
            .unwrap()
            .ops
    ...
    fn attr<'a>(op: &'a crate::MirOp, key: &str) -> Option<&'a str> {
        op.attributes
            .iter()
            .find(|(k, _)| k == key)
            .map(|(_, v)| v.as_str())
    ...
    fn closure_body_region_preserved() {
        ...
        // in regions[0] — must hold across S6-C5.
        let src = "fn f() { let y = 1; let g = |x : i32| { x + y }; g(0); }";
        let (f, _) = lower_one(src);
        let cop = closure_op(&f);
    ...
    // ─────────────────────────────────────────────────────────────────────
    // § Path-resolution against local_vars — S6-C5 unblocks let-binding refs.
    // ─────────────────────────────────────────────────────────────────────
