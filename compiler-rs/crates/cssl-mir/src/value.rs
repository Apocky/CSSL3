...
impl MirType {
    ...
    ///
    /// `MirType::Ptr` (T11-D57 / S6-B1) returns `None` at stage-0 — the host
    /// pointer-type's alignment isn't const-expressible from the MIR layer ;
    /// memref ops on raw pointers must carry an explicit `"alignment"` attr
    /// for now. A future refactor can introduce target-aware natural-align
    /// once cross-compile target-triple plumbing lands.
    #[must_use]
...
mod tests {
    ...
    fn mir_type_natural_alignment_non_scalars_is_none() {
        ...
        assert_eq!(MirType::Ptr.natural_alignment(), None);
    }
}
