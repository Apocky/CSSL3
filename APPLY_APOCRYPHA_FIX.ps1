...
$prefix = ((Resolve-Path $edge).Path.Substring($repo.Length)).TrimStart('\') -replace '\\','/'
Write-Host "worker checkout : $edge"
Write-Host "git repo        : $repo   (patch prefix: '$prefix')"

# -- 2. apply the patch ----------------------------------------------------------------
Push-Location $repo
try {
    git status --porcelain | Out-Null
    if ($prefix -eq 'cssl-edge') { git am --3way $Patch } else {
        # checkout where cssl-edge is the repo root itself
        git am --3way -p2 $Patch
    }
    if ($LASTEXITCODE -ne 0) { git am --abort; throw 'git am failed - apply manually from the patch' }
    Write-Host "patch applied   : $(git log --oneline -1)"
...
if (Test-Path -LiteralPath $RuntimeProfile) {
    ...
    if ($changed) {
        ...
        Push-Location $repo; git add -A; git commit -q -m "chore(apocrypha-worker): repin Qwen runtime profile ctx=$ContextTokens ($($new.Substring(0,12)))"; Pop-Location
...
# -- 7. push (Vercel builds the owner-route change from GitHub) ------------------------
if (-not $NoPush) { Push-Location $repo; git push; Pop-Location }
