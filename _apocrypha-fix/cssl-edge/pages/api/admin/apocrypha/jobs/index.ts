...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const request = {
      ...
      // The current question leads. Each faculty truncates this query to its own
      // ceiling (metaharness 256 bytes, mempalace 768), so anything placed ahead
      // of the question is cut off in its place - putting prior turns first made
      // the narrow faculties search on stale context and never see what was asked.
      retrieval_query: [
        prompt,
