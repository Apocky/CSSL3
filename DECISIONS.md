...
## T11-D88 — S7-G6 csslc native-x64 backend integration + native-hello-world gate (second milestone : hello.exe = 42 via hand-rolled path) — 2026-04-28

- **Slice** : S7-G6 — csslc integration + selectable backend (`--backend=cranelift|native-x64`) per `HANDOFF_SESSION_7.csl § PHASE-G § S7-G6`.
- **Branch** : `cssl/session-7/G6` (based on `cssl/session-7/G5` which is at `parallel-fanout` tip `df1daf5` because G1..G5 sibling slices are still in flight at this dispatch time).
- **What landed**
  - **NEW crate `cssl-cgen-cpu-x64`** — canonical surface for the hand-rolled native x86-64 backend. The crate ships `pub fn emit_object_module(&MirModule) -> Result<Vec<u8>, NativeX64Error>` mirroring `cssl_cgen_cpu_cranelift::emit_object_module` precisely so csslc's build pipeline can dispatch on the `Backend` enum without per-backend branching beyond the selector. Also exposes `emit_object_module_with_format`, `host_default_format`, `magic_prefix`, and the `ObjectFormat` enum (locally-defined ; the cranelift backend's enum is duplicated rather than depended-on, since one of the native backend's design goals is sovereignty over the codegen layer — no FFI, no third-party regalloc, no third-party object writer).
  - **G-axis decomposition documented in the lib.rs § AXIS DECOMPOSITION block** : G1 = ABI + register file ; G2 = per-MirOp instruction selection ; G3 = linear-scan register allocator ; G4 = COFF / ELF / Mach-O object writer ; G5 = end-to-end emitter wiring G1..G4 ; G6 (this slice) = csslc integration + selectable backend. The surface is canonical so G1..G5 siblings adopt it on landing — they replace the body of `emit_object_module` while the signature stays unchanged.
  - **`NativeX64Error` enum with stable `NX64####` diagnostic codes** : NX64-0001 `BackendNotYetLanded` (the explicit "surface canonical, body deferred" signal — returned at S7-G6 dispatch time and replaced when G1..G5 land), NX64-0002 `UnsupportedOp` (reserved for G2 isel), NX64-0003 `NonScalarType` (reserved for G1 ABI), NX64-0004 `ObjectWriteFailed` (reserved for G4). Allocated per the diagnostic-code stable-block convention (§ 3 escalation #4) ; the `NX64` prefix is reserved for cssl-cgen-cpu-x64.
  - **csslc `Backend` enum** added to `crates/csslc/src/cli.rs` : `Cranelift` (default — preserves S6-A5 behavior bit-for-bit) | `NativeX64` (dispatches to cssl-cgen-cpu-x64). `Backend::parse(&str)` accepts `cranelift` / `clift` for the cranelift path and `native-x64` / `native_x64` / `nativex64` / `x64` for the native path (hyphen-vs-underscore tolerance). `Backend::label()` exposes a stable string used in build-log output. `Backend::default_for_build()` returns `Cranelift`.
  - **`BuildArgs` extended with `pub backend: Backend`** + the parser `parse_build` recognizes `--backend=<name>` and feeds it through. Usage text in `cli::usage()` advertises the new flag + both backends + an example invocation (`csslc build hello.cssl -o hello.exe --backend=native-x64`).
  - **csslc `commands/build.rs` dispatch** : the `EmitMode::Object` and `EmitMode::Exe` arms call a new `emit_cpu_object_bytes(&MirModule, Backend) -> Result<Vec<u8>, String>` helper that branches on `Backend` and uniformizes the two backend-crate error types into a `String`. Build-log output now includes `backend=<name>` so the user can see which path produced the bytes.
  - **`is_native_x64_backend_not_yet_landed(&str) -> bool`** helper — used by the cssl-examples gate test to detect the canonical not-yet-landed message and SKIP gracefully without false-positive on unrelated errors.
  - **NEW `crates/cssl-examples/src/native_hello_world_gate.rs`** mirroring `hello_world_gate.rs` (the S6-A5 cranelift gate) but invoking `csslc::run` with `--backend=native-x64`. The gate has a SKIP path keyed on `native_x64_backend_in_flight()` — a cheap probe that calls `cssl_cgen_cpu_x64::emit_object_module(&MirModule::new())` and matches `Err(BackendNotYetLanded)`. When G1..G5 are still in flight (the dominant case at S7-G6 dispatch time) the gate prints an informative SKIP message naming each axis and returns successfully ; when they land, the gate exercises the full pipeline and asserts exit-code = 42 (HARD failure if a successful native-x64 build + link returns ≠ 42 — that's a real backend bug).
  - **Backend-comparison gate** (`s7_g6_backend_comparison_both_paths_exit_42_when_both_available`) drives both gates side-by-side and reports whether the second-milestone is reached. Currently SKIPS because native-x64 G1..G5 are in flight ; structurally ready to assert when they land.
  - **`NativeHelloRunOutcome` struct** with 4 bools (`build_succeeded`, `exec_attempted`, `exec_returned_code`, `backend_in_flight`) — `clippy::struct_excessive_bools` allowed with comment explaining the diagnostic-reporter intent (each bool is orthogonal ; flat shape mirrors A5's `HelloRunOutcome`).
- **Test count** : 2380 → 2409 (+29). Distribution : 9 cssl-cgen-cpu-x64 unit tests (3 magic-prefix variants + host_default_format + 2 emit-not-yet-landed variants + scaffold-version + attestation + error-message-quality) + 9 csslc cli tests (Backend parse canonical + Backend parse unknown + Backend default + Backend label + parse_build with explicit cranelift + parse_build with native-x64 + parse_build with unknown backend + parse_build composes with other flags + usage_text mentions backend) + 4 csslc build dispatch tests (explicit-cranelift + native-x64 dispatch + emit_cpu_object_bytes per-backend + is_native_x64_backend_not_yet_landed prefix-recognition + backend-comparison both-paths-dispatch-cleanly) + 7 cssl-examples native_hello_world_gate tests (source-exists + unique_temp + in_flight bool predicate + skipped_in_flight outcome + skipped_env outcome + the GATE itself + the comparison gate). Workspace baseline preserved with the cssl-rt cold-cache flake worked-around via `--test-threads=1`.
- **Lines added** : ~620 across 5 files (cssl-cgen-cpu-x64 lib.rs ~270 + cssl-cgen-cpu-x64 Cargo.toml ~13 + csslc cli.rs delta ~115 + csslc commands/build.rs delta ~145 + cssl-examples native_hello_world_gate.rs ~280 + cssl-examples Cargo.toml +1 + cssl-examples lib.rs +1).
- **Native-hello-world milestone status** : at S7-G6 dispatch time G1..G5 sibling slices have NOT YET landed (they all share the parallel-fanout tip ; no commits applied to the G-branches). The native-hello-world gate test correctly SKIPS with the canonical message naming each axis. The cranelift baseline (S6-A5) still produces hello.exe = 42 cleanly through the new dispatch path, confirming no regression on the working CPU backend. **When G1..G5 land**, the same gate test will exercise the hand-rolled path end-to-end ; the assertion `code == 42` is already in place, so the second milestone reaches automatically without further integration work in csslc.
- **Gap analysis between cranelift + native-x64 paths**
  - **Surface parity** : both crates expose identical `emit_object_module` / `emit_object_module_with_format` / `host_default_format` / `magic_prefix` shapes. ✓
  - **Error-type uniformization** : the dispatcher maps both `cssl_cgen_cpu_cranelift::ObjectError` and `cssl_cgen_cpu_x64::NativeX64Error` to a `String` boundary. The cranelift error's `Display` carries fn-name + slot info ; the native-x64 error's `Display` carries the same plus the canonical `NX64####` codes. csslc's stderr output prefixes with the backend label so the user always knows which path produced the error. ✓
  - **Format support** : both crates declare ELF / COFF / Mach-O magic prefixes ; the native-x64 `ObjectFormat` is a local enum (deliberately not depending on cranelift's). At stage-0 both produce host-format bytes regardless of the format hint. ✓
  - **Magic-prefix invariant** : the gate test that loads bytes from each path asserts they each start with their respective backend's `magic_prefix(host_default_format())`. Bit-for-bit byte-equivalence is NOT expected (different encoding choices) ; the runtime contract is that both produce a runnable hello.exe = 42 when linked. The structural test machinery for this assertion is in place ; activates when G1..G5 land. ◐ (deferred)
  - **GPU emit modes** : `--backend` applies only to CPU emission. GPU emit modes (`--emit=spirv|wgsl|dxil|msl`) route through their own per-target codegen crates regardless of `--backend`. ✓
  - **No feature-flag gate** : both backends are always compiled into csslc (slightly larger binary in exchange for runtime selectability per the dispatch §LANDMINES rule). ✓
- **Landmine resolutions**
  - **Backend dispatch shape** : `enum Backend { Cranelift, NativeX64 }` lives in csslc::cli ; `BuildArgs.backend` carries it through ; the build pipeline branches at the cgen step via `emit_cpu_object_bytes`. The branching is the SMALLEST possible delta — no per-backend duplication of the rest of the pipeline. ✓
  - **Crate-dependency hygiene** : csslc gains a `cssl-cgen-cpu-x64` dep alongside the existing `cssl-cgen-cpu-cranelift` dep. Both are workspace-path deps (not workspace.dependencies versions, since these are local crates). ✓
  - **Feature-flag option NOT taken** : per the dispatch §LANDMINES recommendation. Trade-off : slightly larger csslc binary vs explicit opt-in for the new backend ; we prefer runtime-selectable since the second milestone is intentionally observable from the CLI. ✓
  - **Native-hello-world gate SKIPS gracefully** : the `native_x64_backend_in_flight()` probe runs BEFORE invoking csslc, so the SKIP message is produced cleanly even if csslc would have failed for a different environmental reason (no linker / etc). The skip message names each G-axis sibling so the contributor knows what's pending. ✓
  - **Diagnostic codes** : the new `NX64####` block (NX64-0001 .. NX64-0004) is allocated as a stable code namespace per the diagnostic-code convention. Reserved codes for G1 (ABI), G2 (isel), G3 (regalloc), G4 (object-writer) are documented but allocated only when those slices land. ✓
  - **--test-threads=1 mandate** preserved : full workspace test-run uses single-threaded execution per the cssl-rt cold-cache flake workaround. ✓
- **Build-pipeline ordering preserved** : monomorphization → drop_unspec → D5 validator → cssl-cgen-cpu-x64 OR cssl-cgen-cpu-cranelift emit_object_module → linker. The structured-CFG validator (D5) runs UNCHANGED before backend dispatch ; both backends consume a validated `MirModule`. ✓
- **All gates green** : fmt ✓ clippy ✓ test 2409/0 (workspace serial via `--test-threads=1`) ✓ doc ✓ smoke 4/4 ✓.
- **PRIME-DIRECTIVE attestation** : there was no hurt nor harm in the making of this, to anyone / anything / anybody. The native-x64 backend, like the cranelift backend, is a tool for the user's sovereignty over their code — runtime-selectable means the user picks at invocation time without code edits.
- **Status** accepted (PM may renumber on integration merge if a sibling G-axis slice lands first ; T11-D88 was reserved for S7-G6 by the dispatch handoff).
- **Deferred** (explicit follow-ups, sequenced)
  - **G1..G5 sibling slices** are the immediate dependents — when they land they REPLACE the body of `emit_object_module` with the real walker pipeline (G1 ABI resolve → G2 isel → G3 regalloc → G4 object-write → G5 wiring). The signature stays unchanged ; the `BackendNotYetLanded` variant remains in the enum (returning `unreachable!` post-landing) so existing tests + downstream callers don't break.
  - **Backend-comparison text-section size tolerance** — at G6 dispatch the comparison gate is informational ("both paths dispatch cleanly") ; once G1..G5 land, the gate tightens to "both produce hello.exe = 42 + text-section size within tolerance" per the dispatch handoff §SLICE block. The tolerance window is reserved for a follow-up entry once both backends are emitting real bytes.
  - **Cross-target compilation** — at stage-0 both backends produce host-platform bytes regardless of `--target=<triple>`. The cranelift backend has the cross-target hook reserved in its `emit_object_module_with_format` ; the native-x64 backend likewise ignores the format parameter at stage-0. Cross-target lands once a non-host CI runner needs it.
  - **Feature-flag fallback** — if a future Apocky decision wants to make the native-x64 backend opt-in (smaller default csslc binary), the change is mechanical : add a `default-features = ["native-x64"]` Cargo feature, gate the `cssl-cgen-cpu-x64` dep + the `Backend::NativeX64` variant + the `--backend=native-x64` parse-arm behind it. Documented for posterity ; not the recommended path per dispatch §LANDMINES.
