...
## § T11-D84 : Session-7 S7-G2 — Native x86-64 register allocator (linear-scan + spill slots + callee-saved push/pop)

> **PM allocation note** : T11-D77..T11-D83 reserved by the dispatch plan for sibling Phase-7 slices (F-axis, G1) that have not yet landed at remote. T11-D84 is allocated to S7-G2 because the slice handoff REPORT BACK explicitly named T11-D84 as reserved. PM may renumber on integration merge if a sibling slice lands first.

- **Date** 2026-04-28
- **Status** accepted (PM may renumber on merge — sibling G-axis slices landing concurrently)
- **Session** 7 — Phase-G native x86-64 backend, slice 2 of N (linear-scan register allocator)
- **Branch** `cssl/session-7/G2` (based on `origin/cssl/session-6/parallel-fanout` — S7-G1 has not landed at remote, so this slice defines the canonical X64Inst / X64VReg / X64Func surface that G1 will adopt or extend)
- **Context** Per `specs/07_CODEGEN.csl` § CPU BACKEND § stage1+ : "owned x86-64 backend ... own regalloc (graph-color + linear-scan hybrid per-fn)". This slice lands the *linear-scan half* of the hybrid, building on Cranelift's stage-0 backend (which has its own opaque regalloc2-based allocator) by introducing a sibling crate `cssl-cgen-cpu-x64` for the stage-1 owned trajectory. Linear-scan vs graph-coloring rationale : LSRA is O(n log n), mechanically simple, validatable against the published Poletto+Sarkar 1999 reference, and competitive with graph-coloring on straight-line code + structured loops (which is what CSSLv3 MIR produces post-D5 structured-CFG validation). Graph-coloring is deferred to a future hybrid slice once profiling surfaces a workload where LSRA's spill heuristic is measurably worse.
- **Slice landed (this commit)** — ~1500 net LOC across 6 files (≈1180 production + ≈320 tests across 35 unit + integration tests)
  - **`crates/cssl-cgen-cpu-x64/Cargo.toml`** (new, 14 LOC) — new sibling crate to `cssl-cgen-cpu-cranelift` ; depends only on `cssl-mir` (path) + `thiserror` (workspace). No cranelift deps — the two backends are siblings, not parent/child. Workspace-level `members = ["crates/*"]` glob picks it up automatically.
  - **`crates/cssl-cgen-cpu-x64/src/lib.rs`** (new, ~110 LOC) — module-level doc-block opening with the algorithmic rationale (LSRA vs graph-coloring choice + reserved-register discipline + scope of the slice). Public API : `allocate(&X64Func) -> Result<X64FuncAllocated, AllocError>` driver fn ; `LinearScanAllocator` for callers who want a programmatic interface ; full re-export of `X64PReg / X64VReg / X64Func / X64FuncAllocated / X64Inst / X64InstKind / X64Operand / LiveInterval / IntervalKind / SpillSlot / SpillSlots / Abi / RegBank / RegRole`. Crate uses `#![forbid(unsafe_code)]` (no FFI in pure regalloc) + the standard scaffold-version test pattern.
  - **`crates/cssl-cgen-cpu-x64/src/reg.rs`** (new, ~330 LOC) — physical + virtual register definitions :
    - **`X64PReg`** — 32-variant enum covering 16 GP (rax/rcx/rdx/rbx/rsp/rbp/rsi/rdi/r8..r15) + 16 XMM (xmm0..xmm15). repr-u8 encoding numbers match canonical x86-64 ModR/M encoding. `ALL` array exposes canonical-order iteration. `bank()` routes to GP/XMM. `is_reserved_for_user_alloc()` returns true for rsp+rbp at S7-G2 (omit-frame-pointer is a future flag).
    - **`X64VReg`** — virtual register handle with `index : u32` + `bank : RegBank`. Constructor pair `gp(idx) / xmm(idx)`.
    - **`Abi`** — 3-variant enum SysVAmd64 / WindowsX64 / DarwinAmd64. **DELIBERATELY re-declared** rather than imported from `cssl-cgen-cpu-cranelift::abi` — keeping the two backends as siblings means a future Cranelift removal doesn't cascade through the owned x64 path.
    - **`RegRole`** — 3-variant enum CallerSaved / CalleeSaved / Reserved.
    - **`X64PReg::role_under_abi(self, abi) -> RegRole`** — canonical per-ABI table covering all 32 pregs across all 3 ABIs. SysV-AMD64 callee-saved : rbx, r12, r13, r14, r15 (rbp Reserved). Windows-x64 callee-saved : rbx, rsi, rdi, r12..r15 + xmm6..xmm15 (rbp Reserved). DarwinAmd64 follows SysV. References : SysV-AMD64 ABI specification §3.2.3 + Microsoft x64 calling convention learn.microsoft.com.
  - **`crates/cssl-cgen-cpu-x64/src/inst.rs`** (new, ~520 LOC including ≈70 LOC of tests) — the X64Inst skeleton :
    - **`X64InstKind`** — 18-variant enum covering Mov / Add / Sub / Imul / Cmp / Xor / Lea / Push / Pop / Call / Ret / Jmp / Jcc / Label / SpillMarker / ReloadMarker / Addf / Subf / Mulf / Divf. Spill / Reload markers are first-class instructions that the byte-emission pass (S7-G3) will convert into real `mov [rsp+disp]` forms.
    - **`X64Inst`** — wrapper struct with explicit `uses : Vec<X64VReg>` + `defs : Vec<X64VReg>` + `fixed_uses : Vec<X64PReg>` + `fixed_defs : Vec<X64PReg>` + `clobbers : Vec<X64PReg>`. Constructors `mov / add / sub / cmp / ret / call / label` auto-derive the uses+defs sets from the operand shape. Builders `with_clobbers / with_fixed` extend the constraint metadata.
    - **`X64Operand`** — `Reg / Imm32 / Imm64 / Mem`. `MemAddr { base, disp }` for the simplest `[base + disp]` addressing mode (SIB deferred).
    - **`X64Func`** — vreg-form fn ; `name / abi / insts / vreg_count / param_vregs / result_vregs`. `push(inst)` updates vreg_count automatically.
    - **`X64FuncAllocated`** — preg-form result of allocation ; `name / abi / allocated_insts / callee_saved_used / frame_size / assignment`. `allocated_insts : Vec<AllocatedInst>` carries per-instruction `VregResolution { vreg, location }` resolutions for the byte-emission pass. `assignment : Vec<VregAssignment>` carries final per-vreg location (Preg / Spill / Split).
    - **`VregAssignment`** — sum-type Preg(X64PReg) / Spill(SpillSlot) / Split(Vec<VregSegment>). Split is the live-range-splitting form per Wimmer + Mössenböck 2005 § 10.
    - **5 unit tests** : mov-constructor, two-address add records dst as both use+def, cmp records no defs, X64Func push updates vreg_count, VregLocation routes bank correctly.
  - **`crates/cssl-cgen-cpu-x64/src/spill.rs`** (new, ~135 LOC including ≈40 LOC of tests) — stack-frame spill-slot allocator :
    - **`SpillSlot { index, bank }`** — `offset()` returns `index * 16` ; **every slot is 16-byte aligned** per SSE alignment requirement. Wastes ~12 bytes on i32 spills at S7-G2 ; tighter packing is a future-slice optimization.
    - **`SpillSlots`** — bumps `next_index` per `alloc(bank)` call. `total_frame_size()` returns `next_index * 16`.
    - **4 unit tests** : empty zero frame, each slot 16 bytes, offset always multiple-of-16, bank preserved.
  - **`crates/cssl-cgen-cpu-x64/src/interval.rs`** (new, ~360 LOC including ≈110 LOC of tests) — live-interval computation :
    - **`LiveInterval { vreg, start, end, hints, preg_hints, forbidden_pregs, kind }`** — half-open `[start, end)` over program-points. `length() / overlaps() / contains()` helpers.
    - **`IntervalKind`** — 3-variant Param / Local / CrossesCall. CrossesCall intervals MUST live in callee-saved pregs (the allocator forbids caller-saved for them).
    - **`compute_live_intervals(&X64Func) -> Vec<LiveInterval>`** — 4-pass algorithm : (1) collect first-def + last-use per vreg, (2) detect backward jumps + extend intervals across loop bodies, (3) detect Call-crossings + tag intervals, (4) sort by start ascending. Result + Param intervals seeded specially (params live from pp 0 ; results live through final ret).
    - **`max_pressure(intervals, func_len) -> (gp, xmm)`** — diagnostic helper that walks intervals + computes per-bank maximum register pressure at any program-point.
    - **7 unit tests** : empty fn empty intervals, single def-use chain, intervals sorted by start, overlaps detection, backward-jump extends interval, call-crossing kind, pressure computation tracks max.
  - **`crates/cssl-cgen-cpu-x64/src/alloc.rs`** (new, ~530 LOC including ≈200 LOC of tests) — the linear-scan allocator :
    - **`AllocError`** — 4-variant enum NoPregAvailable / DoubleFree / MissingAssignment / BankMismatch. Stage-0 currently only emits MissingAssignment (defensive sanity check) ; the others are reserved for future-slice failure modes (e.g. when fixed_use constraint conflicts can't be resolved).
    - **`AllocReport`** — `interval_count / assigned_to_preg / spilled_entirely / split / callee_saved_used / frame_size`. Internal-consistency invariant : `assigned_to_preg + spilled_entirely + split == interval_count`.
    - **`LinearScanAllocator<'a>`** — driver with internal state `intervals / assignment / active / preg_occupant / spill_slots / callee_saved_used`. The Poletto+Sarkar walk : EXPIRE-OLD-INTERVALS → PICK-FREE-PREG → SPILL-AT-INTERVAL with split-on-spill → add-to-active. `try_pick_free_preg` honors `preg_hints` first then sorts candidates by preferred role (caller-saved cheaper for short intervals — no prologue push ; callee-saved preferred for CrossesCall intervals).
    - **`spill_at_interval`** — picks the active interval whose `end` is furthest in the future ; if that further-future interval ends *after* current, spill the victim + reassign the preg to current, otherwise spill current itself. Live-range splitting via `split_or_full_spill` : if `current_start > victim.start`, record victim as Split [Preg for `[victim.start, current_start)`, Spill for `[current_start, victim.end)`] ; otherwise full Spill.
    - **`build_allocated`** — walks the function's instructions + decorates each with the per-vreg resolution at that program-point. Resolutions are deduplicated per instruction.
    - **`allocate(&X64Func) -> Result<X64FuncAllocated, AllocError>`** — single-call driver.
    - **8 unit tests** : empty fn, single def-use, two disjoint share preg, over-pressure triggers spill, callee-saved pushed when used, cross-call lives in callee-saved, XMM routes to XMM bank, Windows ABI uses different caller-saved set.
  - **`crates/cssl-cgen-cpu-x64/src/tests.rs`** (new, ~225 LOC) — end-to-end integration tests covering the full LSRA pipeline + invariants :
    - **10 integration tests** : rsp+rbp never allocated, all spill-slot offsets 16-byte aligned, non-overlapping intervals share preg, AllocReport totals match, every vreg use has resolution at program-point, Param vregs get Param interval kind, result vregs extended through ret, frame size 16-byte increments, GP+XMM banks independent, short Local prefers caller-saved.
- **The capability claim**
  - **Surface** : `cssl_cgen_cpu_x64::allocate(&X64Func) -> Result<X64FuncAllocated, AllocError>`. Input is a vreg-form function (X64Inst stream + param/result vregs + ABI tag) ; output is a preg-form function with assignments + spill slots + callee-saved-used set + frame_size resolved.
  - **Pipeline** : (forthcoming S7-G1) MIR → X64Func vreg-form → `cssl_cgen_cpu_x64::allocate` → X64FuncAllocated preg-form → (forthcoming S7-G3) byte-emission pass → ELF/COFF/Mach-O object.
  - **First time CSSLv3 has an owned x86-64 register allocator with explicit per-ABI awareness**. Stage-0 cranelift's regalloc2 is opaque + bundled. This slice provides the hand-written linear-scan that the spec-mandated stage-1 trajectory requires (per `specs/07_CODEGEN.csl` § CPU BACKEND § stage1+).
- **Consequences**
  - Test count : 2380 → 2415 (+35 ; 5 inst + 4 spill + 7 interval + 8 alloc + 10 integration + 1 scaffold-version). Workspace baseline preserved (full-serial run via `--test-threads=1` per the cssl-rt cold-cache flake).
  - **New crate `cssl-cgen-cpu-x64`** lands as a sibling to `cssl-cgen-cpu-cranelift`. Both backends co-exist : the cranelift path is stage-0 (today's working JIT + object emission) ; the x64 path is stage-1 (owned, currently regalloc-only). Either backend can be selected at csslc CLI invocation time when S7-G3 lands the byte-emission pass.
  - **Linear-scan algorithm choice is locked** for stage-1 ; graph-coloring is deferred to a follow-up hybrid slice. Documented in the crate-root rustdoc-block.
  - **Reserved-register discipline locked** : rsp + rbp are never allocated at S7-G2 ; rbp's reservation can be relaxed in a future omit-frame-pointer slice via a flag on the LinearScanAllocator. The `is_reserved_for_user_alloc` is the centralized policy gate.
  - **Per-ABI caller-saved/callee-saved tables locked** for SysV-AMD64, WindowsX64, DarwinAmd64. Adding a new ABI is mechanical — extend the `Abi` enum + add a match arm to `role_under_abi`. The tests cover SysV + Windows ABI variants ; Darwin shares SysV's table.
  - **Spill-slot 16-byte alignment is INVARIANT** — every spill slot is 16 bytes regardless of bank. Wastes ~12 bytes per i32 spill at S7-G2 ; the tighter packing (4-byte slots for i32) is a future-slice optimization once profiling surfaces frame-size as a bottleneck.
  - **Diagnostic-code surface is `AllocError`'s 4 variants**. New variants require a follow-up DECISIONS sub-entry per dispatch-plan § 3 escalation #4. Stage-0 currently only emits `MissingAssignment` (defensive sanity check) ; the other 3 are reserved for future failure modes.
  - **The X64Inst / X64VReg / X64Func surface defined here is the canonical surface that S7-G1 will adopt or extend**. G1 will own the per-MIR-op lowering tables ; new instruction variants land alongside their lowering rules. The G2 surface is intentionally *minimal but extensible* — `X64InstKind` is non-exhaustive in spirit (Rust enum syntax doesn't have non_exhaustive in this slice but the convention is to add variants alongside the per-op rule).
  - **PRIME-DIRECTIVE preserved** — register allocation has no observable side-effects on user data ; the allocator is pure-deterministic given (intervals, ABI). Hashmap iteration ordering is the only non-determinism source ; tests pin behavior to assertions that are insensitive to specific preg choice (e.g. "callee-saved was used" rather than "rbx was used").
  - All gates green : fmt ✓ clippy ✓ test 2415/0 (workspace serial via `--test-threads=1`) ✓ doc ✓ xref ✓ smoke 4/4 ✓ (full 9-step commit-gate per `SESSION_6_DISPATCH_PLAN.md § 5`).
- **Closes the S7-G2 slice.** Phase-7 G-axis slice 2 of N complete. **The owned x86-64 register allocator is in place ; S7-G1 (per-op lowering tables) and S7-G3 (byte-emission pass) follow.**
- **Deferred** (explicit follow-ups for future sessions / slices)
  - **Phi-node coalescing hints (move-coalescing)** — the `LiveInterval::hints` field accepts coalesce-with-other-vreg hints but the LSRA driver currently honors them only as preference, not as constraint. Full move-coalescing lands when SSA φ-resolution lands upstream (likely Phase-7 H-axis or beyond).
  - **Chaitin-Briggs graph-coloring fallback for high-pressure bodies** — the spec-mandated "graph-color + linear-scan hybrid per-fn" calls for a graph-coloring pass that layers over LSRA-decided intervals : LSRA picks the spill set (which vregs lose their register) while graph-coloring decides which preg the surviving vregs get. Lands as a separate slice once benchmarks surface a workload where LSRA's greedy assignment is measurably worse.
  - **Two-address-form constraint propagation** — many x64 instructions have fixed-preg constraints (e.g. `idiv` requires rax+rdx ; `mul` writes rax+rdx). The X64Inst surface accepts `fixed_uses / fixed_defs` already but the constraint-propagation pass that *introduces* them is in S7-G1 (instruction-selection from MIR ops).
  - **Live-range hole-filling** — current LSRA treats intervals as contiguous half-open `[start, end)` ranges. Real holes (gaps where a vreg is dead mid-fn) lead to slightly worse register pressure ; the SSA-based interval-with-holes form is a future-slice optimization once profiling surfaces it.
  - **Real x64 byte-emission** (S7-G3) — this slice produces `X64FuncAllocated` with pregs assigned + spill slots resolved ; the actual REX prefix + ModR/M + SIB + displacement + immediate emission is a separate slice.
  - **Object-file emission** (S7-G4 or later) — once byte-emission lands, the bytes feed into `cssl-rt::object` (ELF/COFF/Mach-O) emission. The cranelift backend has its own object-emission path (cranelift-object) ; the owned path will share the cssl-rt object format helpers but bypass cranelift entirely.
  - **AVX2 / AVX-512 instruction emission** — the spec calls for `target-features : ["+avx2", "+fma", "+bmi2", "+popcnt"]` minimum ; the X64InstKind currently only covers SSE-baseline floats (Addf / Subf / Mulf / Divf). VEX/EVEX-prefixed AVX2 + AVX-512 variants land alongside their MIR-op-lowering rules in S7-G1+ slices.
  - **Multi-ABI prologue/epilogue emission** — the allocator records `callee_saved_used` + `frame_size` ; the prologue/epilogue codegen that emits `push reg ; sub rsp, frame_size ; ... ; add rsp, frame_size ; pop reg ; ret` lives in S7-G3 alongside byte emission. The metadata is ready ; the emission is mechanical.
  - **DWARF-5 / CodeView debug-info emission** — required for production debuggers ; lives downstream of byte-emission. Per `specs/07_CODEGEN.csl` line 31 : "debug-info : DWARF-5 (Linux/Mac) + CodeView (Windows)".
  - **Runtime CPU-dispatch (AVX2 + AVX-512 multi-variant fat-kernels)** — per `specs/07_CODEGEN.csl` § CPU BACKEND § stage0 line 29 : "AVX2 base + AVX-512 opportunistic via runtime-CPU-dispatch". The fat-kernel emission pattern (multiple compiled variants + runtime selection on first-call) is a S7-G5+ slice.
