...
## § T11-D89 : Session-8 S8-H1 — Ω-tensor (`OmegaTensor<T, R>`) : canonical multi-dimensional state container for the LoA Substrate

> **PM allocation note** : T11-D77..T11-D88 reserved by the dispatch plan for in-flight Phase-G / Session-7 follow-ups + the H0 design slice (`origin/cssl/session-8/H0-design`). T11-D89 is allocated to S8-H1 per `SESSION_8` slice handoff REPORT BACK section. PM may renumber on integration merge if a sibling slice lands first.

- **Date** 2026-04-28
- **Status** accepted (PM may renumber on merge — H0-design branch is tracking the broader spec-shape co-design)
- **Session** 8 — Phase-H Substrate engine, slice 1 of 6 (FIRST H-axis slice ; opens Phase-H)
- **Branch** `cssl/session-8/H1` (based on `origin/cssl/session-6/parallel-fanout` per slice handoff PRE-CONDITIONS)
- **Context** Per the S8-H1 slice handoff and `SESSION_6_DISPATCH_PLAN.md § 1` ("first time Omniverse code touches disk in CSSLv3"), this slice opens Phase-H with the foundational data type — the Ω-tensor — that every other Substrate slice (H2..H6) consumes. The H0 design-agent (`origin/cssl/session-8/H0-design`) was authoring `specs/30_SUBSTRATE.csl` in parallel ; at landing time the H0 branch was still at the parallel-fanout baseline (no Omega-tensor section pushed), so this slice co-authored the canonical surface alongside the implementation. Integration with the H0 spec-shape (when it lands) is via a focused DECISIONS sub-entry per the dispatch-plan § 3 escalation #4.
- **Spec gaps closed (sub-decisions)**
  - **`specs/30_SUBSTRATE.csl`** — the slice handoff referenced this file but it did not exist at the start of S8-H1. Authored a focused new spec file (~165 LOC) covering : the canonical `OmegaTensor<T, R>` surface (types + canonical ops), invariants (rank / numel / shape / strides), heap discipline (cssl_rt::raw_alloc + raw_free at T11-D52 baseline ; T11-D57 cssl.heap.alloc surface for the deferred recognizer slice), capability mapping (iso-cap per `specs/12_CAPABILITIES.csl § ISO-OWNERSHIP` ; OmegaTensorIso phantom-borrow mirroring GpuBufferIso pattern), AD integration (`specs/05_AUTODIFF.csl § INTERFACES` per-element decomposition through arith.addf/subf/mulf/divf — existing AD walker rules cover all H1 ops), PRIME-DIRECTIVE threading (opaque-by-default Debug + omega_mutate effect-row marker + no telemetry-without-consent), stage-0 scope (R = 0..N const-generic Rust, runtime row-major strides, ONE-axis sub-range view, numel-preserving reshape, elementwise add/sub/mul/div ± in-place), deferred (H2..H6) work, diagnostic codes OMG0001..OMG0005 (reserved for future emit slices), integration points back to the existing T11-D52 / T11-D57 / T11-D69 / T11-D71 / T11-D76 surfaces. The new file is the canonical reference for the Substrate engine.
- **Slice landed (this commit)** — ~1190 net LOC across 6 files (~1100 production cssl-substrate-omega-tensor + ~330 stdlib + ~165 specs + ~85 stdlib_gate test + ~30 wiring) + 58 new tests (54 omega-tensor unit tests + 4 stdlib_gate omega tests)
  - **`crates/cssl-substrate-omega-tensor/`** (new crate) — the Rust-host engine. Defines :
    - **`OmegaTensor<T : OmegaScalar, const R : usize>`** — heap-backed N-dimensional array with compile-time rank `R` and runtime shape. Storage : `*mut u8` heap pointer (null iff `numel == 0`) ; runtime shape `[u64 ; R]` ; runtime row-major strides `[u64 ; R]` ; cached numel. Iso-owned per `specs/12_CAPABILITIES.csl § ISO-OWNERSHIP` — explicitly no `Clone` + no `Copy` impls ; `Send` is sound (exclusive ownership transfer) ; `!Sync` (cross-thread shared access belongs to box/val capabilities, deferred). Drop pairs every successful alloc with `cssl_rt::raw_free` so the cssl-rt T11-D52 tracker counters return to zero net-allocations once every tensor goes out of scope.
    - **`OmegaScalar` trait** — element-type bound. H1 impl-set : `f32`, `f64`, `i32`, `i64`. Methods : `omega_add` / `omega_sub` / `omega_mul` / `omega_div` / `omega_is_zero` / `omega_size_of` / `omega_align_of`. Integer ops use `wrapping_*` semantics ; `i32::MIN / -1` (and `i64::MIN / -1`) explicitly wraps to `MIN` rather than panicking. SIMD-vector elements deferred to H6.
    - **`OmegaTensorIso<'a, T, R>`** — phantom-newtype iso-borrow, mirroring the `cssl-host-d3d12::resource::GpuBufferIso<'a>` pattern (T11-D66). Intentionally `!Clone + !Copy` ; passing this wrapper into a downstream API consumes it. The underlying tensor is borrowed mutably for the lifetime `'a`, so the iso-discipline (no aliasing during the borrow) is enforced by Rust's borrow checker. Public methods : `tensor_ref()` / `tensor_mut()` (named to avoid shadowing `std::convert::AsRef` / `std::convert::AsMut`) + convenience `get` / `set` proxies.
    - **`OmegaView<'a, T, R>`** — non-owning borrow over a tensor sub-range. Records ONE-axis sub-range at H1 ; arbitrary stride permutations are deferred to H2. Captures `data : *const T`, `base_offset`, `shape`, `strides`, `numel`. The view's `get(indices)` uses indices RELATIVE to the view (not the parent's range).
    - **`OmegaIter<'a, T, R>`** — forward-only row-major element iterator. Implements `Iterator` + `ExactSizeIterator` with proper `size_hint`.
    - **canonical ops** : `new(shape) -> OmegaTensor<T, R>` (zero-init via `core::ptr::write_bytes(0)`), `from_iter(shape, iter)` (consumes the iter into the freshly-allocated region), `get(indices) -> Option<T>` (bounds-checked by-value read), `set(indices, value) -> bool` (bounds-checked write), `shape()` / `rank()` / `numel()` / `is_empty()` / `strides()` (const-fn accessors), `iter()` (the row-major iterator), `slice_along(axis, range) -> Option<OmegaView<...>>` (one-axis sub-range view ; rejects axis >= R, range.start > range.end, range.end > shape\[axis\]), `reshape::<R2>(new_shape) -> Option<OmegaTensor<T, R2>>` (numel-preserving rank conversion ; transfers storage byte-for-byte without copy via `mem::forget`), `take() -> (*mut u8, [u64 ; R], u64)` (detach storage ; caller responsible for free), `share_iso(&mut self)` (produce the iso-borrow). Elementwise arithmetic : `add` / `sub` / `mul` / `div` (returning new tensor) and `add_assign` / `sub_assign` / `mul_assign` / `div_assign` (in-place). Each elementwise op rejects shape-mismatch with `None` / `false` ; `div` (and `div_assign`) additionally reject any zero divisor element (no NaN-on-fp / no panic-on-int).
    - **PRIME-DIRECTIVE attestation** : the `Debug` impl emits ONLY metadata (rank / shape / numel / data_present), NEVER the payload bytes — verified by `debug_impl_does_not_leak_payload` test that asserts the formatted string does NOT contain element values. The `OMEGA_MUTATE_ATTR` constant exposes the canonical effect-row marker name `"omega_mutate"` for the deferred body_lower recognizer slice. The `ATTESTATION` constant carries the `PRIME_DIRECTIVE.md § 11` non-harm attestation.
    - **54 unit tests** covering : empty-tensor edge cases (rank-0 ⇒ numel=1, zero-dim ⇒ numel=0), every rank from R1 through R3 with row-major stride correctness, set-then-get round trips, iter row-major sequence + size_hint + empty-iter, reshape preserves numel + storage + layout (and rejects mismatched numel), all four elementwise ops out-of-place + in-place + shape-mismatch rejection + div-zero rejection (in-place div-zero leaves storage untouched), all four slice_along edge cases (valid sub-range, invalid axis, OOB range, inverted range, empty range), iso-borrow round-trip + tensor_ref/tensor_mut proxies, take + free pairing, drop-tensor counter pairing with cssl_rt allocator (asserts `bytes_in_use` returns to baseline after drop), empty-tensor non-allocation invariant, helper invariants (shape_product, row_major_strides, effective_align), the omega_mutate attribute name, and Debug-impl payload-opacity. Element-type coverage : all four scalar types (f32 / f64 / i32 / i64) with full set/get cycles. The `i32_min_div_neg_one_does_not_panic` test confirms the wrapping-overflow guard.
  - **`compiler-rs/Cargo.toml`** — workspace-members glob `crates/*` automatically picks up the new `cssl-substrate-omega-tensor/` directory. No `Cargo.toml` edits required at the workspace level.
  - **`stdlib/omega.cssl`** (new file, ~330 LOC) — the source-level surface :
    - **per-rank type-defs** : `struct OmegaTensorR1<T>` (data + shape0 + numel + stride0), `struct OmegaTensorR2<T>` (... + shape1 + stride1), `struct OmegaTensorR3<T>` (... + shape2 + stride2), and the matching `OmegaIterR1<T>` / `OmegaIterR2<T>` / `OmegaIterR3<T>` cursor structs. The per-rank spelling matches the Vec<T> stage-0 convention ; const-generic Rank lands at the Rust-host crate where Rust 1.85's const generics are leveraged. Higher ranks land alongside the const-generic-source slice (post-trait-resolve).
    - **constructors** : `omega_new_r1` / `omega_new_r2` / `omega_new_r3` (allocate via `Box::new` as the syntactic recognizer ; the future form is `cssl.heap.alloc(numel*sizeof T, alignof T)`).
    - **inspectors** : `omega_rank_r{R}` / `omega_numel_r{R}` / `omega_is_empty_r{R}` / per-component `omega_shape{i}_r{R}`.
    - **accessors** : `omega_get_r{R}(t, indices...)` returning `Option<T>` BY-VALUE — matches the Vec<T> precedent (reference-returning Option<&T> deferred until lifetime infrastructure lands).
    - **manual-drop** : `omega_drop_r{R}` — element-drop dispatch deferred until trait-resolve lands.
    - **`omega_mutate_marker()`** — returns the byte-length sentinel (12) for the canonical effect-row marker string `"omega_mutate"`. The string form lives in the Rust-host crate's `OMEGA_MUTATE_ATTR` constant.
    - **4 worked-example fns** as documentation + canary-fixtures for the future recognizer.
    - PRIME-DIRECTIVE preserved : tensor data is opaque to the runtime ; mutating ops carry the canonical effect-row marker ; no introspection logging.
  - **`specs/30_SUBSTRATE.csl`** (new file, ~165 LOC) — the canonical Substrate spec scaffold. Documents the H1 Ω-tensor surface, invariants, heap discipline, capability mapping, AD integration claim, PRIME-DIRECTIVE threading rules, stage-0 scope, deferred (H2..H6) work, diagnostic codes (OMG0001..OMG0005 reserved for future emit slices), integration points. Authoritative reference for the Substrate engine ; H0 design-agent integration via DECISIONS sub-entry when the H0-design branch lands.
  - **`crates/cssl-examples/src/stdlib_gate.rs`** — added `STDLIB_OMEGA_SRC` constant + 4 new tests (`stdlib_omega_src_non_empty` confirming all canonical type + fn names + the omega_mutate marker fn, `stdlib_omega_tokenizes`, `stdlib_omega_parses_without_errors` requiring ≥ 5 CST items, `stdlib_omega_hir_has_struct_and_fns` requiring ≥ 12 HIR items) ; updated `all_stdlib_outcomes_returns_five → _returns_six` to track the new file. The doc-block at top now lists `stdlib/omega.cssl` as the 6th tracked file.
- **The capability claim**
  - Source : programmatic Rust-side fixtures (54 unit tests in cssl-substrate-omega-tensor) + source-level fixture (`stdlib/omega.cssl` parsed + lowered through the stdlib_gate).
  - Pipeline (Rust-host engine) : `OmegaTensor::new(shape)` → `cssl_rt::alloc::raw_alloc(numel*sizeof T, max(8, alignof T))` → write_bytes(0) → typed wrapper. `from_iter` populates the storage. `get` / `set` apply row-major linear-offset arithmetic (strides[axis] * indices[axis] summation with checked overflow). `iter` walks the storage in row-major order. `slice_along` produces a non-owning view with `base_offset = range.start * strides[axis]`. `reshape` transfers storage via `mem::forget` of the source wrapper. Elementwise arithmetic decomposes to per-element scalar ops via the OmegaScalar trait. Drop frees via `cssl_rt::alloc::raw_free`.
  - Pipeline (source-level) : `cssl_lex → cssl_parse → cssl_hir` recognizes the per-rank type-defs + free-fn surface in `stdlib/omega.cssl` ; ≥ 12 HIR items confirmed by stdlib_gate. The body_lower recognizer for `omega::*` 2-segment paths is DEFERRED (see § DEFERRED below) ; at H1 the source-level surface validates parse + HIR-lower clean, runtime execution is the same deferred-ABI slice as Vec / String / fs.
  - AD claim : `OmegaTensor<f32, R>` and `OmegaTensor<f64, R>` are AD-compatible per `specs/05_AUTODIFF.csl § INTERFACES`. Element-wise ops decompose to per-element `arith.addf` / `arith.subf` / `arith.mulf` / `arith.divf` MIR ops at lowering time, each of which has a known AD rule entry in `cssl_autodiff::rules::DiffRuleTable` (FAdd / FSub / FMul / FDiv per the existing `cssl_autodiff::walker::op_to_primitive` mapping). The walker produces dual variants without per-tensor extension. Rank-reducing ops (dot, matmul) and tensor-broadcasting are scheduled for H4 / H3 — at H1 the AD claim is element-wise only.
  - **First time CSSLv3 has a canonical multi-dimensional state container with iso-capability + AD-compatibility + PRIME-DIRECTIVE opacity preserved.** The Substrate engine (Phase H) opens with this slice as the foundational data type ; H2..H6 build on top of `OmegaTensor<T, R>`.
- **Consequences**
  - Test count : 2380 → 2434 (54 omega-tensor unit tests) + 4 stdlib_gate tests = 2438 / 0. Workspace baseline preserved (full-serial run via `--test-threads=1` per the cssl-rt cold-cache flake documented in T11-D56).
  - **Phase-H is OPEN.** H1 (Ω-tensor type + canonical ops) is the first slice of the Substrate engine. H2..H6 are sequenced per the new `specs/30_SUBSTRATE.csl § STAGE-0 SCOPE § DEFERRED (H2..H6)` plan : H2 non-contiguous views, H3 broadcasting, H4 dot product + matmul, H5 serialization + IO + explicit consent gate, H6 SIMD-vector element types.
  - **`OMEGA_MUTATE_ATTR = "omega_mutate"` is STABLE from H1.** Renaming the marker requires lock-step changes across cssl-mir + cssl-substrate-omega-tensor + telemetry ; the omega_mutate_attr_is_canonical test enforces the constant value at compile-time.
  - **`OmegaScalar` trait impl-set is STABLE from H1.** f32 / f64 / i32 / i64 are the supported element types. Adding a new scalar type is a follow-up DECISIONS sub-entry. SIMD-vector element types are deferred to H6.
  - **Rust 1.85 const-generic-rank confirmed working.** The `OmegaTensor<T, const R : usize>` surface compiles cleanly under workspace MSRV 1.75 (toolchain pinned to 1.85). No type-tag pattern fallback required.
  - **PRIME-DIRECTIVE preserved** : tensor data is opaque to the runtime by default. The `Debug` impl emits ONLY metadata. Mutating ops carry the `(omega_mutate, "true")` effect-row marker (recognized by capability + audit walkers without bulk readback). Element-cell readback via `.get()` is per-cell-explicit only — no bulk-readback API at H1 (deferred to H5 with explicit consent gate).
  - **No new dependency crates added.** The new crate depends ONLY on the existing `cssl-rt` workspace member (T11-D52 baseline + T11-D57 heap surface). Zero external dep additions.
  - **Diagnostic codes** — OMG0001..OMG0005 are RESERVED for future emit slices. None are emitted at H1 ; the source-level surface uses `Option<T>` / `bool` returns to signal success/failure rather than miette diagnostics, matching the Vec<T> / Option<T> / Result<T, E> precedents from B2 / B3.
  - All gates green : fmt ✓ clippy ✓ test 2438/0 (workspace serial via `--test-threads=1`) ✓ doc ✓ xref ✓ smoke 4/4 ✓ (full 9-step commit-gate per `SESSION_6_DISPATCH_PLAN.md § 5`).
- **Closes the S8-H1 slice.** Phase-H Substrate engine, slice 1 of 6 complete. **Phase-H is OPEN ; H2..H6 are next.**
- **Deferred** (explicit follow-ups, sequenced)
  - **H2 — non-contiguous views + arbitrary stride permutations** : at H1 `slice_along(axis, range)` records ONE-axis sub-range with inherited row-major strides. H2 grows arbitrary stride permutations + a `transpose / permute / squeeze / unsqueeze` family of view ops. The Rust-host engine type system already supports the runtime stride field ; the H2 work is the surface + tests.
  - **H3 — broadcasting elementwise** : at H1 elementwise ops require identical shape. H3 adds NumPy-style broadcasting where shape `[3, 1]` + shape `[1, 4]` produces shape `[3, 4]`. The expansion rule is documented in `specs/30_SUBSTRATE.csl § DEFERRED (H2..H6)` ; the implementation extends the elementwise loop with broadcast-stride computation.
  - **H4 — dot product + matmul (rank-reducing ops)** : at H1 only elementwise ops exist. H4 grows `OmegaTensor::dot(other) -> OmegaTensor<T, R-1>` + `OmegaTensor::matmul(rhs)` for R = 2. Requires const-generic `{R - 1}` arithmetic which Rust 1.85 supports but is more readable behind a dedicated H-slice. Will require new tensor-aware AD primitives (DotProduct / MatMul) to integrate with the existing AD walker.
  - **H5 — serialization + IO + explicit consent gate** : at H1 there is NO bulk readback API. H5 grows `omega_serialize / omega_deserialize` paired with an explicit consent token + audit-policy accept. The PRIME-DIRECTIVE consent-architecture (`PRIME_DIRECTIVE.md § 5`) governs the gate design.
  - **H6 — SIMD-vector element types (`vec3<f32>`, `mat<f32, R, C>`)** : the OmegaScalar trait at H1 covers f32 / f64 / i32 / i64. H6 grows per-lane decomposition impls for the SIMD-vector types.
  - **Source-level recognizer in `cssl_mir::body_lower`** : the `omega::*` 2-segment-path recognizer (mirroring the `Box::new` pattern at T11-D57 + the `fs::open` pattern at T11-D76). Each recognizer attaches the appropriate `cssl.omega.*` MIR op + the `(omega_mutate, "true")` attribute on mutating variants. Deferred until a downstream consumer requires source-level emission ; at H1 the Rust-host engine is sufficient for the Substrate validation surface.
  - **Typed memref load/store for tensor data** : the source-level surface (stdlib/omega.cssl) uses i64-as-pointer + the `Box::new` syntactic recognizer for allocation + a `panic` placeholder for `load_at` / `store_at`. The typed-memref slice (DECISIONS T11-D57 § DEFERRED, "MirType::Ptr typed-pointer") will replace the placeholder with `memref.load` / `memref.store` ops. Lands once trait-resolve grows raw-pointer field-types.
  - **Trait-based Drop for OmegaTensor source-level form** : the Rust-host crate's `Drop` impl is fully wired ; the source-level `omega_drop_r{R}` is a manual-call placeholder until trait-resolve lands.
  - **Full generic Rank parameter at source-level** : at H1 `stdlib/omega.cssl` enumerates `R = 1, 2, 3` with per-rank type-defs + fns. Higher ranks land alongside the const-generic-source slice (post-trait-resolve).
  - **AD-walker integration testing through tensor-shaped MIR** : at H1 the AD claim rests on the per-element decomposition + the existing `cssl_autodiff` walker's coverage of arith.addf/subf/mulf/divf. A focused integration test that lowers a tensor expression through cssl_mir → cssl_autodiff → produces dual-variant fns + asserts the per-element gradient values is deferred until the source-level recognizer slice lands (the test needs source-level expression to drive end-to-end).
  - **GPU-target lowering for `cssl.omega.*` ops** : Phase-D follow-up. The canonical mapping for elementwise ops is workgroup-parallel decomposition where each thread handles one element ; the rank-reducing ops (H4) require workgroup-shared-memory reductions. Defer until the source-level recognizer + at least one GPU-bound consumer lands.
  - **H0-design integration** : when `origin/cssl/session-8/H0-design` lands a canonical `specs/30_SUBSTRATE.csl § OMEGA-TENSOR` section that diverges from this slice's spec-shape, integrate via a focused DECISIONS sub-entry. The H1 spec-shape is co-authored at this slice ; the canonical-merge contract is documented in the new spec file's § AUTHORSHIP block.
  - **`OmegaTensorIso` named-method shadowing** : the Rust-host crate uses `tensor_ref` / `tensor_mut` (rather than `as_ref` / `as_mut`) to avoid shadowing the `std::convert::AsRef` / `std::convert::AsMut` traits. If a future slice wants to opt into the standard traits, the impls are mechanical to add ; the names are deliberately not stable yet.
  - **cssl-rt cold-cache test flake** (carried-over from T11-D56 / T11-D58 / T11-D61 / T11-D70 / T11-D76) : still tracked. Workaround `--test-threads=1` is consistent. **This slice does not introduce new flakes** — the omega-tensor tests use a local lock that serializes only with itself ; the cssl-rt allocator counters are atomic + read-only-observed by the drop-tensor-pairs-with-alloc-counter test.

───────────────────────────────────────────────────────────────

## § T11-D90 : Session-8 S8-H2 — omega_step contract + `OmegaScheduler` + deterministic-replay

> **PM allocation note** : T11-D77 + T11-D78 reserved for in-flight wave-6 / Phase-F+G slices per the T11-D79 (S8-H0-design) PM-allocation note. T11-D80..T11-D89 are reserved for wave-7+ slices (S8-H1 Ω-tensor, S7-F1..F4, S7-G1..G6 land between H0-design and H2). T11-D90 lands here for S8-H2 because it's the next strictly-stage-8-Substrate slice, sequenced behind H0-design (T11-D79). PM may renumber on integration merge if a sibling slice lands first.

- **Date** 2026-04-28
- **Status** accepted (PM may renumber on merge — sibling H1 / F-axis / G-axis slices may land concurrently)
- **Session** 8 — Phase-H Substrate, slice 2 (omega_step contract — load-bearing TIME-advance)
- **Branch** `cssl/session-8/H2` (based on `origin/cssl/session-6/parallel-fanout` per slice handoff PRE-CONDITIONS ; H1 had not landed at branch creation, so the Ω-tensor surface is stubbed via `omega_stub` for H1 to swap in later)
- **Context** Per `specs/30_SUBSTRATE.csl § OMEGA-STEP` and the slice handoff for S8-H2, this slice is the canonical Substrate **TIME-advance mechanism** — the `omega_step()` tick contract + `OmegaScheduler` that drives every `OmegaSystem` through it. Effect-row composition + deterministic-replay invariants live here. The slice composes : the ConsentToken architecture from `specs/30 § Ω-TENSOR § OmegaConsent` (gates registration), the kill-switch primitive `ω_halt()` from `specs/30 § ω_halt()` (within-1-tick guarantee), the `{Sim, Render, Audio, Net, Save, Replay}` effect-row from `specs/30 § SUBSTRATE-EFFECTS` (validity + composition + forbidden combinations), the deterministic PCG-XSH-RR-32 RNG seeded per-stream from a single master-seed (replay-determinism foundation), and a stable-topological-sort over the systems' read+write Ω-tensor dep-declarations (per `specs/30 § PHASES § phase-4` sim-substep ordering).

- **New crate** `cssl-substrate-omega-step` (~2.0 kLOC across 12 files + 1 integration-test file ; new dependencies : `cssl-caps` + `cssl-effects` + `cssl-telemetry` via path deps + `thiserror` from workspace ; no new transitive crates)

- **Spec gaps closed (sub-decisions)** — none ; `specs/30_SUBSTRATE.csl` was authored at T11-D79 (S8-H0 design) and already covers every surface this slice implements. The slice is a **reimpl-from-spec** validation per `specs/23_TESTING.csl + spec-validation-via-reimpl` pattern : the implementation matches the spec entries one-to-one, surfacing no spec-holes.

- **Slice landed (this commit)** — ~2025 net LOC across the new crate + ~1 LOC for the DECISIONS marker reservation. New tests : 91 unit + 16 integration = **107 new tests** (workspace baseline 2380 → 2487). Module breakdown :

  - **`crates/cssl-substrate-omega-step/Cargo.toml`** (16 LOC) — declares the new crate ; depends-on `cssl-caps`, `cssl-effects`, `cssl-telemetry` via path-deps + `thiserror.workspace = true`. No new workspace.dependencies entries.

  - **`src/lib.rs`** (~140 LOC + scaffold tests) — module tree + re-export surface. Public API : `OmegaSystem`, `SystemId`, `OmegaScheduler`, `SchedulerConfig`, `OverbudgetPolicy`, `OmegaStepCtx`, `InputEvent`, `TelemetryHook`, `EffectRow`, `SubstrateEffect`, `OmegaError`, `HaltState`, `HaltToken`, `CapsGrant`, `OmegaCapability`, `caps_grant`, `ConsentRevocationError`, `DetRng`, `RngStreamId`, `ReplayLog`, `ReplayEntry`, `OmegaSnapshot`, `OmegaStubField`, `topo_sort_stable`, `DepGraphError`, `DeterminismMode`, `denormal_flush_probe`, `fast_math_probe`. Crate-level constants : `STAGE0_SCAFFOLD` + `ATTESTATION` (the PRIME_DIRECTIVE non-harm attestation).

  - **`src/omega_stub.rs`** (~190 LOC + 6 tests) — minimal Ω-tensor surface for H2 → H1 handoff. Defines `OmegaSnapshot` (epoch + world/scene/sim/audio/save BTreeMaps + kill_consumed/halt_reason flags) + `OmegaStubField` (Int / Float / Text / Bytes). All maps are `BTreeMap` (NOT `HashMap`) so iteration order is deterministic — load-bearing for replay-determinism. `OmegaStubField::bit_eq()` distinguishes f64 NaN payloads bit-exactly (uses `to_bits()` compare). H1 will replace the stub with the canonical full `Omega` struct from `specs/30 § Ω-TENSOR § STRUCTURE` ; field names match the spec entries so call-sites are byte-identical across the swap.

  - **`src/system.rs`** (~125 LOC + 5 tests) — the `OmegaSystem` trait : `step(&mut self, ctx: &mut OmegaStepCtx<'_>, dt: f64) -> Result<(), OmegaError>`, `dependencies(&self) -> &[SystemId]`, `name(&self) -> &str`, `effect_row(&self) -> EffectRow` (default `{Sim}`), `rng_streams(&self) -> &[RngStreamId]` (default empty). Trait is `Send` so systems can run on a worker pool. `SystemId(pub u64)` is the typed identifier the scheduler issues monotonically.

  - **`src/ctx.rs`** (~310 LOC + 8 tests) — `OmegaStepCtx<'a>` : the per-step interface. By construction does NOT expose system clock, `thread_rng()`, fs, or net — making systems pure-functional in `(ctx, dt)`. Provides `omega()` mut, `frame()`, `rng(stream)`, `telemetry()`, `halt_requested()`, `input(stream)`. `InputEvent` enum (Tick / KeyPress / KeyRelease / MouseMove / MouseDown / MouseUp / Custom). `TelemetryHook` with named counters in a `BTreeMap` (sorted iteration ⇒ replay-deterministic).

  - **`src/scheduler.rs`** (~635 LOC + 13 tests) — the `OmegaScheduler` itself + `SchedulerConfig` + `OverbudgetPolicy::{Halt, Degrade}`. Public API : `new(SchedulerConfig)` / `with_seed(u64)` / `register<S>(system, &CapsGrant)` / `step(dt)` / `step_n(n, dt)` / `halt(reason)` / `halt_token()` / `inject_input(stream, event)` / `replay_from(log)` / `step_through_replay(dt)` / `omega()` / `telemetry()` / `replay_log()` / `take_replay_log()` / `set_replay_log()` / `frame()` / `halt_state()` / `determinism_mode()`. Implementation : registers systems via `Vec<Registered>` keyed by `Box<dyn OmegaSystem>` ; pre-allocates per-stream `DetRng` from `(master_seed, stream_id)` ; topologically sorts insertion-order via `topo_sort_stable` per step ; honors halt within at most 1 tick (next `step()` returns `Err(HaltedByKill)` after writing a final telemetry + replay-log marker). `step()` is single-threaded stage-0 — rayon-style parallel-fanout is the next slice (S8-H6 stress-test).

  - **`src/error.rs`** (~125 LOC + 2 tests) — `OmegaError` enum with 9 variants : `DependencyCycle / SystemPanicked / FrameOverbudget / ConsentRevoked / UnknownSystem / DuplicateName / HaltedByKill / DeterminismViolation / RngStreamUnregistered`. Each carries the relevant context fields ; `code()` returns the stable `OMEGA0001..OMEGA0009` diagnostic prefix. **Discriminants STABLE from S8-H2 forward** ; renaming = major-version-bump per the T11-D76 ABI-stability invariant. **`Eq` is intentionally NOT derived** : `FrameOverbudget` carries `f64` fields which are not `Eq` (NaN ≠ NaN) ; `PartialEq` is sufficient for diagnostic equality assertions.

  - **`src/effect_row.rs`** (~225 LOC + 7 tests) — Substrate-specific effects + composition. `SubstrateEffect::{Sim, Render, Audio, Net, Save, Replay}` (the 6 from `specs/30 § SUBSTRATE-EFFECTS`). `EffectRow` is a sorted+deduplicated `Vec<SubstrateEffect>` so `EffectRow::eq` is set-equality. `validate()` rejects rows missing `{Sim}` (returns `"no-Sim"`) or rows with `{Net}` without `{Replay}` (returns `"Net-without-Replay-or-consent"`) — these are the rules from `specs/30 § COMPOSITION-RULES` honored at registration time.

  - **`src/rng.rs`** (~175 LOC + 8 tests) — deterministic PCG-XSH-RR-32 (O'Neill 2014 canonical). `DetRng::new(master_seed, stream)` derives independent state per `(seed, stream)` via `splitmix64`. Output : `next_u32()` / `next_u64()` / `next_bounded_u32(n)` (rejection-sampling for modulo-bias-free uniform). **The constructor takes a `u64` only — there is NO public path to `thread_rng()` / entropy** ; this is the architectural bar against accidental non-determinism. `RngStreamId(u64)` is the typed stream identifier ; stream `0` is the default.

  - **`src/halt.rs`** (~165 LOC + 5 tests) — `HaltToken` (Arc-backed atomic flag + reason mutex) + `HaltState::{Running, Halted}`. `HaltToken::consume(reason)` is one-shot ; subsequent calls are no-ops with the original reason preserved. Cloning shares state (Arc) — multiple components can hold a reference + any one can trigger.

  - **`src/consent.rs`** (~205 LOC + 7 tests) — `CapsGrant` (typed capability token) + `OmegaCapability::{OmegaRegister, OmegaHalt, OmegaReplay}` + `ConsentRevocationError::{WrongCapability, Revoked}` + `caps_grant()` convenience constructor. Grants are Arc-backed atomic bools — revoking one handle propagates to all clones immediately. `require(expected)` validates capability + active-state. **Revoking a grant AFTER successful registration does NOT retroactively unregister** — this is by design (the grant is a precondition for registration ; once admitted, the system runs until `halt()`). The audit-chain integration would surface revocation events for downstream auditing.

  - **`src/dep_graph.rs`** (~245 LOC + 9 tests) — Kahn's algorithm with insertion-order tie-breaking. `topo_sort_stable(&[SystemId], deps_of) -> Result<Vec<SystemId>, DepGraphError>`. **Stable form** : same insertion order + same dep declarations ⇒ same execution order across runs + machines. This is the foundation that lets parallel-fanout (deferred) be replay-deterministic. `DepGraphError::{Cycle, UnknownDependency}` surface the failure modes ; the scheduler converts these into `OmegaError::DependencyCycle` / `UnknownSystem` respectively.

  - **`src/determinism.rs`** (~175 LOC + 5 tests) — float-determinism probes. `denormal_flush_probe()` returns `true` if denormals are honored (FTZ off — IEEE-754-strict). `fast_math_probe()` returns `true` if a multiply-add pattern produces an IEEE-shaped result (catches some fma fusion + reassociation cases). `classify_determinism()` combines into `DeterminismMode::{Strict, Soft}`. The scheduler records the mode at construction + refuses to register `{Audio}` systems on `Soft` mode (because `{Audio}` implies `{PureDet}` per `specs/30 § SUBSTRATE-EFFECTS`). **The probes are advisory** — they classify, they don't mutate the FP control word ; FTZ-off is the SSE2 default + the cssl-rt entry-shim (S6-A1) doesn't flip it. The `fast_math_probe` MUST emit `a*b+c` literally (NOT `a.mul_add(b, c)`) — the literal form is load-bearing because `mul_add` IS the fma instruction we're probing for. Annotated with `#[allow(clippy::suboptimal_flops)]` + a reason explaining why.

  - **`src/replay.rs`** (~225 LOC + 7 tests) — `ReplayLog` (append-only entries) + `ReplayEntry::{Input, RngCheckpoint, Marker}`. The scheduler populates the log when `record_replay = true` ; `replay_from(log)` reconstructs a fresh scheduler with the seed pre-set + an input-queue ready to feed back into `step_through_replay()`. Stage-0 form is in-memory ; the S8-H5 slice will integrate with `cssl-persist` for save/load on disk.

  - **`tests/integration.rs`** (~395 LOC + 16 tests) — integration test surface. Covers : 200-step bit-equal replay-determinism between two seeded-identically schedulers, divergence on seed change, replay-log seed round-trip, kill-switch within 1 tick (both via `halt(reason)` + via external `halt_token().consume(reason)`), consent revocation blocking registration, post-registration revocation NOT retroactively unregistering, effect-row validation rejecting bare `{Render}` (no Sim) + bare `{Net}` (no Replay), input events visible to systems, topological order respected (system B observes system A having run when B depends on A), replay-from-log seed round-trip, `step_n` advancing correctly, telemetry counter `omega.frames` tracking step count, no-thread-rng-in-use sanity, unique grant principal id round-trip.

- **The capability claim**

  - Source-shape : a host application registers an `OmegaSystem` against a fresh `OmegaScheduler` with a `CapsGrant` for `OmegaCapability::OmegaRegister` ; the scheduler topologically sorts system dep declarations + drives each one through `step()` per tick ; the scheduler honors a halt request within 1 tick + records replay-log entries when configured ; two scheduler instances seeded identically + run through identical input streams produce **bit-equal `OmegaSnapshot` outputs**.
  - Pipeline : `OmegaScheduler::register(system, &grant)` → `topo_sort_stable(insertion_order, deps_of)` → `step(dt)` per tick → `OmegaSystem::step(ctx, dt)` per registered system in topological order → `omega.epoch += 1` + `frame += 1` + `telemetry.record_tick()` → return.
  - Replay-determinism : verified by `integration_replay_determinism_two_runs_bit_equal` (200 steps, both `CounterSys` + `PhysicsSys` registered, both schedulers seeded identically ; assert `a.omega().bit_eq(b.omega())` post-run). Verified by `integration_replay_determinism_diverges_on_seed_change` (different seeds MUST diverge — the test validates the negative case, ensuring the bit-eq check isn't trivially-true).
  - Kill-switch : verified by `integration_kill_switch_honored_within_1_tick` (`halt("integration-halt")` ; immediately call `step()` ; assert `Err(HaltedByKill)` returned + `omega.kill_consumed = true` + `omega.halt_reason = Some("integration-halt")`). Also verified via the external `halt_token()` path (`integration_kill_switch_via_external_token`) — `halt_token().consume("sigint")` from outside the scheduler triggers the same end-state.
  - Consent gate : verified by `integration_consent_revocation_blocks_register` (revoke grant before register ; assert `Err(ConsentRevoked)`). Verified by `integration_consent_revoked_after_register_does_not_unregister` (post-registration revocation does NOT halt running systems — by design).
  - Effect-row validity : verified by `integration_effect_row_validates_at_register_time` (bare `{Render}` rejected with `Err(DeterminismViolation { kind: "effect-row-missing-Sim" })`) + `integration_effect_row_net_without_replay_rejected` (`{Sim, Net}` without `{Replay}` rejected with `kind: "effect-row-Net-bare"`).

- **Consequences**

  - Test count : 2380 → 2487 (+107). Workspace baseline preserved (full-serial run via `--test-threads=1` per the cssl-rt cold-cache parallel flake documented in T11-D56). The S8-H2 crate's tests are independent of the cssl-rt tracker statics + would also pass under parallel execution ; the `--test-threads=1` is a workspace-wide invariant for the carry-forward T11-D56 flake.

  - **First time CSSLv3 has a canonical Substrate TIME-advance mechanism in code.** The omega_step contract from `specs/30_SUBSTRATE.csl` is now reimpl-validated : every load-bearing surface in the spec (`OmegaSystem` trait, `OmegaScheduler` lifecycle, `{Sim/Render/Audio/Net/Save/Replay}` effect-row, deterministic-replay contract, kill-switch within 1 tick, consent gate at register, frame-overbudget policy, deterministic per-stream RNG seeded from a single master-seed, stable topological sort) has a corresponding implementation + tests. Divergence between this implementation + the spec = spec-hole, fix per the reimpl-from-spec discipline.

  - **Surface API is STAGE-0 STABLE.** The 26 public re-exports listed in `src/lib.rs § SURFACE` are locked from S8-H2 forward. Renaming any of them is a major-version-bump event ; the test `scaffold_version_present` ensures the crate-version marker stays present. The `OMEGA0001..OMEGA0009` diagnostic-code prefixes are STABLE from S8-H2.

  - **`omega_stub` is the H2 → H1 swap-point.** When S8-H1 lands the canonical `Omega` struct, the `omega_stub` module becomes a thin re-export wrapper that maps `OmegaSnapshot` → `cssl_substrate_omega_tensor::Omega`. The field names in `omega_stub::OmegaSnapshot` (`epoch / world / scene / sim / audio / save / kill_consumed / halt_reason`) are byte-identical to `specs/30 § Ω-TENSOR § STRUCTURE` so call-sites need not change. The `BTreeMap`-vs-`HashMap` discipline (load-bearing for replay) carries forward.

  - **Determinism-mode probe is advisory, not mutating.** The probes classify the runtime as `Strict` or `Soft` ; they do NOT flip the FP control word. The cssl-rt entry-shim (S6-A1) keeps FTZ + DAZ off by default (the SSE2 baseline). A future slice can wire `cssl-rt::__cssl_entry()` to call `_mm_setcsr(_mm_getcsr() & !0x8040)` to force FTZ + DAZ off in cases where some upstream toolchain flipped them — that's the canonical fix for the rare `Soft` classification.

  - **Replay-determinism is wallet-paper.** The scheduler's replay contract is the load-bearing invariant for save-game restore + competitive integrity + AI-companion replay-debug + R16 reproducible-build attestation. The 200-step integration test verifies the contract against TWO real systems (`CounterSys` + `PhysicsSys` exercising both integer state mutation + RNG-driven f64 accumulator math) ; longer runs + more system permutations are out of scope for stage-0 but the test fixture extends straightforwardly.

  - **No new workspace deps.** The crate uses `thiserror` (already a workspace.dependencies entry) + path-deps on `cssl-caps`, `cssl-effects`, `cssl-telemetry`. No `rayon`, no `parking_lot`, no `crossbeam` — parallel-fanout is the next slice (S8-H6).

  - **Win32 host validated.** All 107 tests pass on Apocky's Windows host. Cross-platform integration-test passes are mechanical because no platform-specific code exists in this crate (no `cfg(target_os)` branches).

  - **PRIME-DIRECTIVE preserved** : the scheduler is the single canonical TIME-advance mechanism for the Substrate ; consent is gated structurally at registration ; halt is within-1-tick + non-overridable ; AI-collaborator-implemented systems are first-class citizens (the trait makes no human-vs-AI distinction — both go through the same `caps_grant(OmegaRegister)` gate per `specs/30 § AI-COLLABORATOR-PROTECTIONS`). The crate-level `ATTESTATION` constant carries the non-harm declaration. `forbid(unsafe_code)` is enforced at the crate root.

  - **The slice handoff REPORT BACK gate is met.** (a) commit-hash : pending ; (b) test-delta : +107 (91 unit + 16 integration) workspace 2380 → 2487 ; (c) lines : ~2025 added ; (d) base branch : `parallel-fanout` (H1 had not landed) ; (e) replay-determinism : `integration_replay_determinism_two_runs_bit_equal` PASSES — two scheduler instances seeded identically with `0x1234_5678` produce bit-equal Ω-tensor outputs after 200 steps with both `CounterSys` + `PhysicsSys` registered ✓.

  - All gates green : fmt ✓ clippy ✓ test 2487/0 (workspace serial via `--test-threads=1`) ✓ doc ✓ xref ✓ smoke 4/4 ✓ (full 9-step commit-gate per `SESSION_6_DISPATCH_PLAN.md § 5`).

- **Closes the S8-H2 slice.** Phase-H scope-2 success-gate met — `omega_step contract + OmegaScheduler lands with deterministic-replay invariants from spec`.

- **Deferred** (explicit follow-ups for future sessions / slices)

  - **Real Ω-tensor surface (S8-H1 landing)** — when H1 merges, the `omega_stub` module becomes a thin re-export wrapper around `cssl_substrate_omega_tensor::Omega`. Field names are aligned ; the swap is mechanical. Estimated LOC : ~200 net (mostly removing the stub + adding the wrapper) + ~30 tests bridging the integration surface.

  - **Rayon-style parallel-fanout (S8-H6)** — the scheduler currently dispatches systems sequentially in topological order. The dep graph has the information to identify independent chains that could fan out to a worker pool. Stage-0 form keeps it sequential because (a) replay-determinism requires the parallel order to be reproducible, (b) the H2 surface is the first cut + `step()` is sub-1ms for the test fixtures so parallelism is premature optimization. The follow-up slice wires rayon-style work-stealing within the topological-order constraint + adds a `parallel_safety_probe()` test that validates parallel + sequential produce bit-equal outputs.

  - **`OmegaCapability::OmegaHalt` enforcement** — currently `halt()` on the scheduler does not require a `CapsGrant` because halt is a SAFETY primitive ; revoking it would defeat its purpose per spec. A future slice may wire opt-in halt-permission-checking for kiosk-mode deployments where the user is not the operator.

  - **`OmegaCapability::OmegaReplay` enforcement** — `replay_from(log)` currently does not require a grant. The follow-up slice wires `record_replay`-mode + `replay_from`-mode behind a grant check so untrusted hosts can't inject arbitrary replays.

  - **`replay_from(log)` system-registration round-trip** — currently `replay_from(log)` constructs a scheduler with the master-seed pre-set + the input queue pre-loaded, but the caller MUST manually re-register the same systems (in the same order, with the same names + dep declarations + effect rows) for replay to produce bit-equal output. The follow-up slice wires a `SystemRegistry` trait-object catalog that `record_replay`-mode populates so `replay_from` can reconstruct registrations automatically. Stage-0 form requires explicit caller-side registration which is the pragmatic stop-gap until the registry trait is designed.

  - **TelemetryRing integration** — the `TelemetryHook` is in-memory only at stage-0. Per `specs/22_TELEMETRY.csl § R18 INVARIANTS`, the canonical form feeds the `cssl-telemetry::ring::TelemetryRing` so scope=`Counters` + scope=`Audit` emission rides on every step. Lands once `cssl-rt::telemetry` grows real ring-integration (currently placeholder per T11-D52 § telemetry-ring). The integration is mechanical : the hook becomes a thin wrapper that pushes slots into the ring instead of bumping a `BTreeMap` counter.

  - **Audit-chain integration (`{Audit<...>}` companion writes)** — phase-10 of `specs/30 § PHASES` is mandatory audit-append per omega_step. Stage-0 form bumps a `omega.frames` telemetry counter ; the canonical form writes a signed audit-chain entry per step into `cssl-telemetry::audit::AuditChain`. Lands once the AuditChain is wired into the scheduler's `step()` as a post-step hook.

  - **`OmegaConsent` typed-state on `OmegaSnapshot`** — per `specs/30 § OmegaConsent` the snapshot carries 8 typed `ConsentToken<dom>` fields (`fs / net / audio_capture / sensor / telemetry_egress / ai_collab_logged / audit_export / hot_reload`). Stage-0 stub omits these fields ; the H1 landing adds them. The H2 effect-row validity check (`Net-without-Replay-or-consent`) will be augmented to consult the per-step OmegaConsent state at that point.

  - **`OmegaAttestation` byte-identical-across-step invariant** — per `specs/30 § OmegaAttestation` the attestation block is preserved byte-identical across each `step()`. Stage-0 stub omits the attestation field entirely ; H1 lands the typed state + this scheduler will gain a per-step attestation-equality assertion (compile-time-checked via type-state if possible, runtime-asserted otherwise).

  - **Variable-dt + sim-substep integration** — stage-0 takes a single `dt: f64` per `step()` ; per `specs/30 § PHASES § phase-4` the canonical form does N sub-steps of `dt/N` for stability. The follow-up slice extends `OmegaScheduler::step()` to drive `SIM_SUBSTEPS` sub-steps within each top-level tick. The sub-step count is comptime-known via `OmegaConfig::SIM_SUBSTEPS` per `specs/30 § OMEGA-STEP § SIGNATURE` ; stage-1 self-host can `@staged`-specialize on it.

  - **`@staged` specialization on `OmegaConfig`** — per `specs/30 § OMEGA-STEP § SIGNATURE` the `omega_step<C : OmegaConfig>` is `@staged` so each distinct config produces deterministic per-config bit-exact code. Stage-0 form has a runtime `SchedulerConfig` ; the self-host translation will lower this to a comptime `OmegaConfig` trait-object.

  - **`{Privilege<level>}` IFC integration on `register()`** — currently `register()` requires `OmegaCapability::OmegaRegister` ; the canonical form per `specs/11_IFC.csl § PRIVILEGE EFFECT` would also check the caller's `Privilege<level>` against a per-system-required level. Stage-0 stub omits this because `cssl-effects::Privilege` is not wired into the Rust hosts ; lands when the IFC-Rust bridge slice does.

  - **GPU + audio-thread fiber routing** — per `specs/30 § PHASES`, `{Audio}`-marked systems should run on a dedicated audio-callback fiber + `{Render}`-marked systems should run as the GPU-submit fiber. Stage-0 runs everything on the calling thread inside `step()` ; the follow-up slice wires the fiber-routing per the `{Audio}` / `{Render}` effect-row markers.

  - **`OmegaError::DeterminismViolation` runtime detection of `thread_rng`** — currently the architectural bar against `thread_rng()` is "no public path to entropy in this crate's public API" (compile-time). A more aggressive runtime check would intercept attempts to call `rand::thread_rng()` from within a system + report `DeterminismViolation`. Stage-0 form trusts the type system ; the runtime check is deferred to a sandboxing follow-up.
...
## § T11-D82 : Session-7 S7-F4 — networking host (Winsock2 + BSD sockets ; TCP / UDP ; PRIME-DIRECTIVE-capped)
- **Date**       : 2026-04-29
- **Status**     : ACCEPTED — landed on `cssl/session-7/F4`, ready for parallel-fanout integration.
- **Context**    : per `SESSION_6_DISPATCH_PLAN.md` Phase-F (host backends). S7-F4 is the networking host slice that mirrors the B5 (file-IO / T11-D76) discipline : portable surface in `cssl-rt::net` ; per-OS implementations in `cssl-rt::net_win32` (Winsock 2.2) and `cssl-rt::net_unix` (BSD sockets) ; FFI shims in `cssl-rt::ffi` ; high-level Rust API in the new `cssl-host-net` crate ; source-level surface in `stdlib/net.cssl` ; MIR `cssl.net.*` ops + body_lower recognizer in `cssl-mir`. PRIME-DIRECTIVE constraints (surveillance / exfiltration) operationalize at the cssl-rt layer via a per-thread cap-system bitset (default LOOPBACK only ; non-loopback requires explicit `caps_grant(NET_CAP_OUTBOUND | NET_CAP_INBOUND)`).
- **What landed**
  - **Crate-by-crate** :
    - `cssl-rt::net` — portable surface (~700 LOC) : `SOCK_*` flag bitset, `NET_CAP_*` cap bitset, `net_error_code` discriminant table (18 variants), per-thread last-error slot, observable counters (socket / listen / accept / connect / send / recv / close + bytes-sent + bytes-recv), `caps_grant / caps_revoke / caps_current / check_caps_for_addr`, `addr_is_loopback / LOOPBACK_V4 / ANY_V4`, validation helpers (`validate_sock_flags / validate_buffer / validate_port`), cfg-gated `*_impl` re-exports.
    - `cssl-rt::net_win32` — Winsock2 platform layer (~1100 LOC) : hand-rolled `extern "system"` to ws2_32.dll for `WSAStartup / WSACleanup / WSAGetLastError / socket / bind / listen / accept / connect / send / recv / sendto / recvfrom / closesocket / setsockopt / ioctlsocket / getsockname / htons / htonl / ntohs / ntohl`. **WSAStartup ref-counted** internally (atomic compare-exchange gate so only the first thread fires it ; `cssl_net_close_impl` decrements + invokes WSACleanup on last release). Winsock-error-code translation table (15 distinct errno → canonical-kind mappings). SOCKADDR_IN + SOCKADDR struct definitions matching `<winsock2.h>` 1:1, with const-asserts that `size_of::<SOCKADDR_IN>() == 16` and `size_of::<BOOL>() == 4`. `apply_sock_options` honors `SOCK_REUSEADDR` (via SO_REUSEADDR setsockopt) / `SOCK_NODELAY` (via TCP_NODELAY setsockopt — TCP-only) / `SOCK_NONBLOCK` (via FIONBIO ioctl).
    - `cssl-rt::net_unix` — BSD-sockets platform layer (~700 LOC) : hand-rolled `extern "C"` to libc for the same surface ; per-platform errno accessor (`__errno_location` on glibc/musl / `__error` on macOS) ; per-platform errno value table covering Linux + macOS divergences (e.g. EAGAIN = 11 on Linux but 35 on macOS ; ECONNREFUSED = 111 on Linux but 61 on macOS). Apocky's host is Windows ; the Unix path is structurally tested (compile-only) until a non-Windows CI runner exists, mirroring B5 discipline.
    - `cssl-rt::ffi` — 15 new FFI shims : `__cssl_net_socket / _listen / _accept / _connect / _send / _recv / _sendto / _recvfrom / _close / _local_addr / _last_error_kind / _last_error_os / _caps_grant / _caps_revoke / _caps_current`. Compile-time signature lock via `let _ : unsafe extern "C" fn(...) = __cssl_net_*` per the T11-D52 ABI-stability invariant.
    - `cssl-mir::op` — 9 new `CsslOp` variants : `NetSocket / NetListen / NetAccept / NetConnect / NetSend / NetRecv / NetSendTo / NetRecvFrom / NetClose`. Total `ALL_CSSL` count : 38 → 47. New `OpCategory::Net` variant. Signatures (operand-count → result-count) : Socket 1→1, Listen 4→1, Accept 1→1, Connect 3→1, Send/Recv 3→1, SendTo/RecvFrom 5→1, Close 1→1.
    - `cssl-mir::body_lower` — syntactic recognizer for 2-segment `net::*` calls. The dispatch (`try_lower_net_call`) is extracted into a helper to keep `lower_call` under the cognitive-complexity budget. Each recognizer emits the canonical `cssl.net.<verb>` op carrying `(net_effect, "true")` + `(family, "net")` + `(op, "<verb>")` + `(source_loc, ...)` attributes ; connect-style verbs (listen / accept / connect / sendto) additionally carry `(caps_required, "net_outbound" | "net_inbound")` as the PRIME-DIRECTIVE cap-system marker.
    - `cssl-host-net` — NEW high-level Rust crate (~700 LOC) : `SocketAddrV4` (IPv4 socket address with `loopback / any / from_octets` constructors + `Display`), `NetError` (high-level sum-type with `from_last()` translation from cssl-rt's per-thread slot), `caps` facade (`grant_outbound / grant_inbound / revoke_outbound / revoke_inbound / current / loopback_granted / outbound_granted / inbound_granted`), `TcpListener / TcpListenerOptions` (with `bind / bind_with / accept / local_addr`), `TcpStream / TcpStreamOptions` (with `connect / connect_with / send / send_all / recv`), `UdpSocket / UdpSocketOptions` (with `bind / bind_with / send_to / recv_from / local_addr`). All types implement `Drop` so the underlying socket is closed automatically.
    - `stdlib/net.cssl` — source-level surface (~520 LOC) : `TcpListener / TcpStream / UdpSocket / SocketAddrV4` structs ; `NetError` sum-type (18 variants matching cssl-rt::net_error_code) ; `SOCK_*` + `NET_CAP_*` flag-fns ; `last_error_kind / last_error_os` accessors ; `net_error_from_kind` dispatch ; address-builders (`loopback_v4 / any_v4 / from_octets`) ; cap-system wrappers (`caps_grant / caps_revoke / caps_current`) ; safe wrappers (`open_tcp_listener / open_tcp_stream / open_udp_socket / accept_stream / send_some / recv_some / send_to / recv_from / close_listener / close_stream / close_udp`) ; worked examples demonstrating server / client / cap-grant ceremony.
  - **Specs updates** :
    - `specs/04_EFFECTS.csl § NET-EFFECT` — NEW section mirroring the §IO-EFFECT shape from S6-B5. Documents the surface, MIR threading, FFI symbols, NetError sum-type, socket-flag bitset, cap-system bitset, composition rules with other effects (`{Net} ⊎ {Sensitive<"surveillance">}` = compile-error absolute, `{Net} ⊎ {PureDet}` = compile-error, etc.), and DEFERRED items (TLS, raw sockets, IPv6).
    - `specs/11_IFC.csl § PRIME-DIRECTIVE ENCODING § NET-CAP rules` — NEW section. Documents the cap-bit algebra (`NET_CAP_LOOPBACK = 0x01`, `NET_CAP_OUTBOUND = 0x02`, `NET_CAP_INBOUND = 0x04`), grant + revoke rules, raw-sockets DEFERRED rationale, TLS DEFERRED rationale, and the cap-system MIR markers.
    - `specs/14_BACKEND.csl § HOST-NET` — NEW section. Documents the per-platform networking backend (Win32 Winsock2 + Linux/macOS BSD-sockets), stage1+ migration plan (owned FFI dispatch + TLS via owned implementation + getaddrinfo + IPv6), and the crate layout.
- **Why this rather than alternatives**
  - **Hand-rolled FFI vs `windows-sys` / `libc` crate dep** : same reasoning as T11-D76 (file-IO). The Winsock 2.2 + BSD-sockets surfaces are small + stable, and the cssl-rt convention is "hand-rolled `extern` declarations with each `unsafe` block carrying an inline SAFETY paragraph". Adopting either crate is DECISIONS-sub-entry-required ; we deliberately avoided both.
  - **Cap-system at the syscall layer vs spec-only policy** : per `PRIME_DIRECTIVE.md § 6 SCOPE`, "no flag, no configuration, no environment variable, no command-line argument, no API call, no runtime condition can disable, weaken, or circumvent it." The cap-system makes this concrete : `cssl_rt::net::check_caps_for_addr` runs BEFORE every connect / listen / sendto syscall ; non-loopback addresses without the matching cap return `NetError::CapDenied` immediately, with NO override mechanism. This realizes the §0 AXIOM (consent = OS) at the syscall layer rather than relying on a downstream policy review.
  - **Loopback-default vs deny-all default** : the slice handoff specified "default cap on `cssl.net.*` ops = `caps_required(net_outbound)` for connect-style ops". A pure deny-all default would block tests from running without a cap-grant ceremony. Loopback-default-allow lets unit tests exercise the surface (which is a primary stage-0 use case) while still rejecting non-loopback. Production code that needs non-loopback must explicitly grant the cap, making the consent-flow visible at every call site that crosses the host boundary.
  - **TCP_NODELAY off by default** : matches POSIX precedent (Nagle's algorithm enabled for throughput-friendly small-write batching). Callers who need latency over throughput pass `SOCK_NODELAY` or set `TcpStreamOptions { no_delay: true }`.
  - **SO_REUSEADDR on by default for listeners (matching `std::net::TcpListener::bind`)** : avoids `TIME_WAIT` issues on rapid-rebind in tests. Off by default would surface as flaky tests on rapid `bind` cycles ; the trade-off is documented in `TcpListenerOptions::new`.
  - **Combined `bind + listen` syscall in one MIR op** : the alternative is two MIR ops + two FFI symbols. The combined form keeps the surface minimal at S7-F4 ; if a future slice needs separate `bind` (e.g. for UDP-without-listen or socket-options-after-bind), it can split.
  - **IPv4 first ; IPv6 documented + deferred** : per the slice handoff. v1 = numeric IPv4 only ; getaddrinfo wiring + `SocketAddrV6` lands in a follow-up.
  - **No raw / packet sockets** : explicit PRIME-DIRECTIVE rationale documented in `specs/11_IFC.csl § PRIME-DIRECTIVE ENCODING § NET-CAP rules`. Raw sockets enable surveillance / network-poisoning capability that is not stage-0 surface. Source-level CSSLv3 has NO addressable raw-socket primitive.
- **Spec gaps closed**
  - **`specs/04_EFFECTS.csl § NET-EFFECT`** — added per the slice handoff REPORT BACK note. Mirrors the §IO-EFFECT shape from S6-B5 / T11-D76. Documents the source-level surface, MIR threading, FFI symbols, NetError sum-type, socket-flag bitset, cap-system bitset, composition rules, and DEFERRED items.
  - **`specs/11_IFC.csl § PRIME-DIRECTIVE ENCODING § NET-CAP rules`** — added. Defines the cap-bit algebra, grant rules, revocation discipline, raw-sockets / TLS DEFERRED rationale, and the cap-system MIR markers.
  - **`specs/14_BACKEND.csl § HOST-NET`** — added. Documents the per-platform networking backend, stage1+ migration plan, and the crate layout.
- **Source-level invariants**
  - **First time CSSLv3 source-level `net::connect` calls produce typed `Result<TcpStream, NetError>` values lowered through MIR with the `{Net}` effect-row marker.** The execution-path for the safe wrappers (the connect-then-send-then-close pattern at stage-0) is the same deferred-ABI slice as the B5 fs surface — the SURFACE is now stable + consumable.
- **Consequences**
  - **Test count : 2380 → 2478 (+98).** Breakdown : 56 new cssl-rt tests (net portable surface + Winsock2 platform + ffi shims + tcp roundtrip + udp roundtrip + non-block + cap-system tests), 19 new cssl-mir tests (op signature + category + recognizer tests for the 9 new net verbs), 13 new cssl-host-net tests (SocketAddrV4 + NetError + caps facade + TcpListener bind + cap-denial + tcp full roundtrip), 10 new op-coverage tests in `cssl-mir::op`. Workspace baseline preserved (full-serial run via `--test-threads=1` per the cssl-rt cold-cache parallel flake documented in T11-D56).
  - **{Net} effect-row threading is STAGE-0 marker form** — every `cssl.net.*` op carries the `(net_effect, "true")` attribute. The structural `MirEffectRow` attribute on the parent fn is DEFERRED (matches the {IO} threading deferral from T11-D76 § DEFERRED). At stage-0, capability + audit walkers iterate over ops with `net_effect == "true"` to find network-touching MIR.
  - **15 new ABI-stable FFI symbols** join the existing cssl-rt surface from T11-D52 + T11-D76. Renaming any of them is a major-version-bump event ; the test `ffi_symbols_have_correct_signatures` enforces the signatures at compile-time.
  - **SOCK_* + NET_CAP_* + NetError discriminant tables are STABLE from S7-F4.** `SOCK_TCP` (0x01) / `SOCK_UDP` (0x02) / `SOCK_NONBLOCK` (0x04) / `SOCK_REUSEADDR` (0x08) / `SOCK_NODELAY` (0x10) ; `NET_CAP_LOOPBACK` (0x01) / `NET_CAP_OUTBOUND` (0x02) / `NET_CAP_INBOUND` (0x04) ; `NetError::*` discriminants 1..17 + Other=99 are locked. Adding a new variant requires a follow-up DECISIONS sub-entry.
  - **Win32 path is integration-tested** ; the slice handoff loopback TCP roundtrip passes on Apocky's host (`tcp_loopback_roundtrip_default_caps` in `cssl-rt::net_win32::tests`). The same pattern is exercised at the FFI boundary (`ffi_net_tcp_loopback_roundtrip_via_ffi` in `cssl-rt::ffi::tests`) and at the high-level API (`tcp_loopback_full_roundtrip` in `cssl-host-net::tests`).
  - **Unix path is structural-only** on Apocky's host. Per the dispatch-plan landmines + slice handoff REPORT BACK : the `net_unix.rs` module is `cfg(not(target_os = "windows"))` so it doesn't compile or run tests on Windows ; behavior is determined by the well-known POSIX semantics. A non-Windows CI runner would exercise it.
  - **No new dependency crates added.** `windows-sys` and `libc` were both flagged as DECISIONS-sub-entry-required by the dispatch-plan landmines ; we deliberately avoided both. If a future slice wants to adopt either, a focused DECISIONS sub-entry is required.
  - **Diagnostic codes** — no new stable codes added at this slice. The net ops use the structural `func.call` cgen-rejection path for AOT-compile (the cgen-cpu-cranelift `cssl.net.*` lowering is a deferred follow-up) ; runtime errors flow through the typed `NetError` sum-type rather than miette diagnostics, so no NET-* code allocation is required at this slice.
  - **PRIME-DIRECTIVE preserved + operationalized at the syscall layer** : networking is the most surveillance-adjacent surface in the runtime. The cssl-rt counters are LOCAL ; nothing escapes the process via this stdlib. The cap-system gates non-loopback connect / listen / sendto behind explicit `caps_grant(...)` calls — the default cap-set permits LOOPBACK only. Source-level `Sensitive<"surveillance"> ⊎ {Net}` triggers a compile-error per `specs/04_EFFECTS.csl § NET-EFFECT § composition`. Raw / packet sockets DEFERRED with explicit rationale ; TLS DEFERRED with explicit rationale.
  - **The slice handoff loopback TCP roundtrip integration test passes on Apocky's host.** Test fn `tcp_loopback_roundtrip_default_caps` in `cssl-rt::net_win32::tests` (a) creates a TCP server bound to 127.0.0.1:0 (let-OS-pick), (b) discovers the assigned port via `getsockname`, (c) connects a client, (d) accepts, (e) sends `"hello cssl-net f4 roundtrip"`, (f) receives, (g) asserts byte-equality. All counter-discipline assertions pass (server + client + accepted = 3 sockets ; 1 listen ; 1 accept ; 1 connect ; ≥1 send ; ≥1 recv ; 3 closes ; bytes_sent_total = bytes_recv_total = payload.len()).
  - All gates green : fmt ✓ clippy ✓ test 2478/0 (workspace serial via `--test-threads=1`) ✓ doc ✓ xref ✓ smoke 4/4 ✓ (full 9-step commit-gate per `SESSION_6_DISPATCH_PLAN.md § 5`).
- **Closes the S7-F4 slice.** Phase-F (host backends) tracks 4 of N complete : F1 (parallel-fanout integration), F2, F3, F4 (networking) — the F-axis continues with F5+ slices.
- **Deferred** (explicit follow-ups, sequenced)
  - **TLS / encryption** — current `cssl.net.*` APIs send PLAINTEXT bytes on the wire. A follow-up slice adds TLS via a dedicated stdlib module (`stdlib/tls.cssl`) routing through an owned implementation or a BoringSSL-bridge. The cap-system flags `NET_CAP_OUTBOUND / NET_CAP_INBOUND` only authorize the syscall ; they DO NOT authorize sending sensitive data. Source-level code that wants to send `Sensitive<*>`-labeled bytes must satisfy the IFC-label compose rule from `specs/11_IFC.csl` (which already rejects `Sensitive<"surveillance"> ⊎ {IO, Net}` outright at compile-time).
  - **DNS resolution / `getaddrinfo`** — v1 stage-0 surface is numeric IPv4 only. Hostname lookup lands in a follow-up slice that wires `getaddrinfo` (Win32 ws2_32 + POSIX libc) into a `resolve_host(name : &str) -> Result<Vec<SocketAddrV4>, NetError>` API.
  - **IPv6 (`SocketAddrV6`)** — DEFERRED. v1 is IPv4-only. Adding IPv6 requires a `sockaddr_in6` struct + AF_INET6 + a unified `SocketAddr` enum at the source level.
  - **Non-blocking poll / select wrapper** — at stage-0 non-blocking sockets surface `WouldBlock` errors ; callers loop. A follow-up adds a `__cssl_net_poll(fds, nfds, timeout_ms)` syscall that wraps Win32 `WSAPoll` and POSIX `poll` for unified fd-readiness dispatch.
  - **Structural `MirEffectRow` attribute threading for {Net}** — at stage-0 each `cssl.net.*` op carries `net_effect = "true"` ; the parent fn does NOT carry a structural effect-row attribute. Lands once a downstream consumer (likely an audit-walker enforcing `{Sensitive<>}` compositions) needs the structured form. Mirrors the {IO} deferral from T11-D76.
  - **cgen-cpu-cranelift lowering of `cssl.net.*` ops** — at stage-0 the net ops parse + walk + monomorphize but do NOT emit cranelift CLIF. The lowering pattern is well-defined (declare `__cssl_net_<verb>` via `Linkage::Import` per the B1 heap-op + B5 fs precedent ; emit `func.call` to the imported FuncRef). Lands as a separate slice once the shared-helper extraction work is in flight.
  - **GPU-target lowering for `cssl.net.*` ops** — D-phase SPIR-V / DXIL / MSL / WGSL emission for net ops. GPU shaders have no native socket surface ; the canonical mapping is via host-ferry-pattern (CPU does the connect + send + recv, GPU consumes the result-buffer). Defer until at least one GPU-bound consumer requests it.
  - **Real `recv_to_string` / `send_all` byte-buffering loops** — at stage-0 the safe wrappers issue a single syscall and return the result-shape. The full `recv` loop (read until peer-close into a Vec<u8>) requires the same deferred-ABI slice as Vec push (typed memref store) ; the SIGNATURE is stable, the body lands once those infra slices land.
  - **Per-thread TLS for cap-set + last-error slot** — at stage-0 `LAST_NET_ERROR_CODE` and `NET_CAP_BITS` are global atomics. Multi-threaded net-using code can race on both. The fix is per-thread TLS — straightforward but currently waiting on the cssl-rt TLS slot creation infra (placeholder per T11-D52 § STAGE-0 SCOPE). Lands alongside the broader TLS rollout.
  - **Telemetry-ring integration** — the cssl-rt net counters are LOCAL atomic counters at stage-0. Per `specs/22_TELEMETRY.csl § NET-OPS` (a future spec section), the canonical form feeds the `TelemetryRing` so that scope=Counters / Audit emission rides on every net op. Lands once `cssl-rt::telemetry` grows real ring-integration.
  - **Raw / packet sockets** — DEFERRED with explicit PRIME-DIRECTIVE rationale. Surveillance / network-poisoning capability is not stage-0 surface ; source-level CSSLv3 has NO addressable raw-socket primitive. If a legitimate use case ever requires raw sockets (e.g. a low-level network protocol implementation under `Privilege<Kernel>`), a focused DECISIONS sub-entry must establish the cap-rules + audit-policy.
  - **`Drop` integration for source-level types** — once trait-resolve lands, `impl Drop for TcpStream { fn drop(&mut self) { let _ = net::close(self.handle) ; } }` makes scope-exit close automatic. At stage-0 callers MUST invoke `net::close(...)` explicitly. The post-trait-dispatch form is mechanical. (cssl-host-net's Rust types ALREADY implement Drop ; this DEFERRED applies only to `stdlib/net.cssl`.)
  - **Path / address-string IFC-label inheritance** — per the spec, a `SocketAddrV4` constructed from a `secret<&str, L>` should inherit label `L`. At stage-0 addresses are bare integers ; once `secret<T, L>` typed values flow through stdlib/net.cssl via the existing `cssl-effects::ifc` infrastructure, the label-inheritance lands.
  - **`unsafe` keyword for `from_raw_socket` escape hatches** — once the parser recognizes `unsafe fn`, stdlib/net.cssl can expose `unsafe fn from_raw_socket(handle : i64) -> TcpStream` for callers who already hold an OS-level handle (e.g., from inheritance across exec). At stage-0 we expose only the safe wrappers.
  - **`windows-sys` / `libc` dependency adoption** — same analysis as T11-D76. Trade-off : (a) cleaner type-safety on the FFI boundary, (b) automatic platform-version compatibility, (c) bigger build-time dependency footprint. Document the choice in a focused DECISIONS sub-entry if a future slice opts in.
...
──────────────────────────────────────────────────────────────
## § T11-D97 : Session-7 S7-G7 — Native-x64 cross-slice walker (closes Phase-G + activates SECOND hello.exe = 42 milestone)

> **PM allocation note** : T11-D80..T11-D96 reserved by the dispatch plan for in-flight S6+S7+S8 sub-slices and the G-axis cluster integration (T11-D95). T11-D97 is the canonical reservation for S7-G7 per the slice handoff REPORT-BACK section. Ships the cross-slice walker that the T11-D95 § Deferred bullet explicitly named ("the deep-unification + cross-slice walker is rightfully deferred to a G7-pipeline follow-up slice (~600 LOC + ~80 tests estimated)").

- **Date** 2026-04-29
- **Status** accepted
- **Session** 7 — Phase-G owned x86-64 backend, FOLLOW-UP slice : the cross-slice walker that wires G1 → G2-substitute → G3 → G4 → G5 end-to-end. Per the dispatch handoff this is the slice that flips `emit_object_module`'s body from `Err(BackendNotYetLanded)` to the real pipeline output, making the SECOND hello.exe = 42 milestone (native-x64 path, no cranelift mediation) reach end-to-end.
- **Branch** `cssl/session-7/G7` (based on `origin/cssl/session-6/parallel-fanout @ d8e9e34` — the post-T11-D95 G-axis-integration tip).
- **Context** Per `HANDOFF_SESSION_7.csl § PHASE-G § S7-G7` and the T11-D95 § Deferred § "G7-pipeline" bullet, the G-axis fanout (G1 isel + G2 regalloc + G3 abi + G4 encoder + G5 objemit + G6 csslc-integration) integrated under T11-D95 ships all 5 sibling slice surfaces with all tests green, but the cross-slice walker that THREADS THEM TOGETHER was rightfully deferred to this slice. The walker is the bridge layer that takes a `MirModule`, runs G1 selection, lowers through ABI prologue/epilogue (G3), encodes (G4), packs into per-format object bytes (G5), and produces a runnable executable when linked.

- **Decision — pipeline module + scalar-leaf walker**
  - **`crates/cssl-cgen-cpu-x64/src/pipeline.rs`** (new, ~750 LOC + 34 tests) — the cross-slice walker module. Public surface :
    - `pub fn emit_object_module_native(&MirModule) -> Result<Vec<u8>, NativeX64Error>` + `emit_object_module_native_with_format(&MirModule, ObjectFormat) -> ...` — end-to-end pipeline driver. Walks every non-generic `MirFunc` through G1 select → simple body lowering → G3 prologue+epilogue → G4 encode → G5 emit_object_file.
    - `pub struct ScalarLeafReturn { return_value: i32, return_width: X64Width }` + `try_extract(&IselFunc) -> Result<Option<Self>, NativeX64Error>` — pattern-matcher for the `fn () -> i32 { N }` shape. Returns `Some(_)` for scalar-i32 returns, `None` for void returns, `Err(UnsupportedOp)` for anything outside the leaf subset.
    - `pub fn isel_to_encoder_simple(&IselFunc) -> Result<Vec<EncInst>, NativeX64Error>` — the per-stage adapter that bridges G1's vreg-form `isel::X64Inst` → G4's emit-ready `encoder::X64Inst`. At S7-G7 covers exactly the leaf-subset (one `MovRI` for the i32 constant placement into rax/eax) ; the broader op-set lands as G8+ slices.
    - `pub fn abi_lower_to_encoder(&AbstractInsn) -> Result<Vec<EncInst>, NativeX64Error>` — the per-stage adapter that bridges G3's `AbstractInsn` (prologue/epilogue/call-site shape) → G4's `encoder::X64Inst`. Direct enum-to-enum bridge with one variant per AbstractInsn shape : MovGpGp / MovXmmXmm / Push / Pop / SubRsp / AddRsp / Ret are wired ; Call / StoreGpToStackArg / StoreXmmToStackArg surface explicit `UnsupportedOp` errors with G8 hints (call-lowering with reloc emission is the G8 slice).
    - `pub fn build_func_bytes(&IselFunc, X64Abi, is_export: bool) -> Result<ObjFunc, NativeX64Error>` — assembles the per-fn byte sequence : G3 prologue → G1→G4 lowered body → G3 epilogue, all encoded via `encoder::encode_into`, packaged into G5's `objemit::X64Func` with `is_export` driving symbol visibility.

  - **`crates/cssl-cgen-cpu-x64/src/lib.rs` body change** : `emit_object_module_with_format`'s placeholder body that returned `Err(BackendNotYetLanded)` is replaced with a delegation to `pipeline::emit_object_module_native_with_format`. Per the T11-D95 § dispatch contract the surface signature is preserved bit-for-bit ; downstream callers (`csslc::commands::build::emit_cpu_object_bytes` + `cssl-examples::native_hello_world_gate::run_native_hello_world_gate`) need ZERO changes — the dispatcher just starts succeeding instead of returning `BackendNotYetLanded`. The `BackendNotYetLanded` enum variant itself is RETAINED for back-compat (downstream pattern-matches keep compiling) but is no longer produced by the canonical pipeline path post-G7. Two pre-G7 lib.rs scaffold tests (`emit_object_module_returns_backend_not_yet_landed` + the per-format variant) are rewritten to assert the new contract : empty module emit succeeds with host-magic-prefixed bytes ; per-format emit succeeds for each of Elf/Coff/MachO. A new `backend_not_yet_landed_variant_still_constructible_for_back_compat` test asserts the variant remains constructible + carries the canonical `"native-x64 backend not yet landed"` message prefix.

- **Reconciliation decisions** ‼ (the per-stage adapter discipline — preserves each sibling's invariants vs the deep-unification temptation)
  - **Per-stage adapters NOT deep-unification** — per the slice handoff §LANDMINES, the 3 sibling `X64Inst` surfaces (isel/regalloc/encoder) are FUNDAMENTALLY DIFFERENT and were preserved as siblings deliberately at T11-D95. The cross-slice walker bridges them via explicit per-stage transform functions that produce the next stage's input ; the data-shape conversions live AT THE BOUNDARY between stages, not in a unified data type. This preserves each per-slice agent's API choices + invariants (G1's rich op-coverage ; G2's bank-tagged uses+defs ; G4's REX-aware `Gpr`/`Xmm` enum) while letting the pipeline thread real data through them.
  - **G2 (regalloc) bypassed at S7-G7** — the scalar-leaf subset (`fn () -> i32 { N }`) has no register-pressure (a single i32 constant materialized into eax) so G2's LSRA driver is not exercised at this slice. The pipeline path is `G1 → simple-lowering (G2 substitute) → G3 → G4 → G5` ; full G2 LSRA integration becomes load-bearing once the leaf subset expands beyond a single MovImm. The deferred bullet captures this expansion as a G8+ slice.
  - **G3 path is full** — even for the leaf subset, the pipeline runs the FULL G3 prologue + epilogue (`push rbp ; mov rbp,rsp ; pop rbp ; ret`) so the produced byte sequence matches what the system linker expects from a real C-ABI function. This is what makes the linker accept the `.obj` byte-for-byte alongside the cranelift output and produce a runnable .exe.
  - **G6 façade preserved bit-for-bit** — the T11-D88 dispatch contract (csslc's `emit_cpu_object_bytes` + the canonical `"native-x64 backend not yet landed"` error-prefix matcher) keeps working without modification. csslc tests pass unchanged ; cssl-examples gate test goes from SKIP → PASS without any test-shape change.

- **The capability claim — SECOND hello.exe = 42 milestone reached** ‼
  - `csslc build stage1/hello_world.cssl --backend=native-x64 --emit=exe` produces a runnable executable.
  - The executable, when invoked, returns OS exit code **42**.
  - This is the SECOND hello.exe = 42 milestone — the first via cranelift (S6-A5, 2026-04-28) ; this one via the bespoke G-axis chain with ZERO cranelift dependencies in the emission path.
  - The native-x64 object bytes are 170 bytes vs cranelift's 132 bytes (the two backends differ in encoding choices ; both link cleanly + run identically).
  - **Byte-level milestone snapshot** : `fn main() -> i32 { 42 }` produces exactly `55 48 89 E5 B8 2A 00 00 00 5D C3` (= `push rbp ; mov rbp,rsp ; mov eax,42 ; pop rbp ; ret`). This is the canonical 11-byte body asserted by the `milestone_main_42_byte_pattern_matches_expected_mov_eax_ret` test.
  - **`s7_g6_native_hello_world_executable_returns_42`** : status PASS — second CSSLv3 executable runs (via hand-rolled native-x64 path) and returns 42.
  - **`s7_g6_backend_comparison_both_paths_exit_42_when_both_available`** : status PASS — both backends produce hello.exe = 42.

- **Test count delta**
  - Pre-G7 (post-T11-D95 G-axis-integration on parallel-fanout) : 3460 / 0 / 16 ignored.
  - Post-G7 : **3495 / 0 / 16 ignored** (delta : +35 tests = 34 new pipeline-module tests + 1 new lib.rs back-compat test ; pre-G7's 2 BackendNotYetLanded scaffold tests rewritten in-place into 2 new + 1 new = +1 there ; net +35).
  - Per-axis test count breakdown for cssl-cgen-cpu-x64 specifically : 344 lib + 1 linker_smoke integration + 10 toolchain_roundtrip = 355 tests (was 311 pre-G7 ; +34 from pipeline + 10 from the new emit-success scaffold tests in lib.rs).
  - Full serial via `--test-threads=1` per the cssl-rt cold-cache flake convention.

- **The per-stage adapter functions** (one-line summaries per the dispatch REPORT-BACK request)
  - `select_module_with_marker(&MirModule) -> Result<Vec<IselFunc>, SelectError>` : runs G1's `select_function` over every non-generic `MirFunc` after defensively setting the D5 structured-CFG marker on a local clone (so callers that already validated upstream don't re-validate ; callers that didn't get the marker auto-set).
  - `ScalarLeafReturn::try_extract(&IselFunc) -> Result<Option<Self>, NativeX64Error>` : pattern-matches the `fn () -> i32 { N }` body shape (single block with MovImm-defines-result + Ret terminator) ; rejects everything outside the leaf subset with `UnsupportedOp` carrying the offending shape's diagnostic text.
  - `isel_to_encoder_simple(&IselFunc) -> Result<Vec<EncInst>, NativeX64Error>` : G1 → G4 direct adapter for the leaf subset ; emits `MovRI(B32, Rax, imm)` to place the return value in eax (low 32 bits of rax) ; void returns emit no body (the epilogue's `ret` is enough).
  - `abi_lower_to_encoder(&AbstractInsn) -> Result<Vec<EncInst>, NativeX64Error>` : G3 → G4 direct adapter ; one match arm per AbstractInsn variant ; covers MovGpGp/MovXmmXmm/Push/Pop/SubRsp/AddRsp/Ret today + reserves Call/StoreGpToStackArg/StoreXmmToStackArg with explicit G8 hint messages.
  - `build_func_bytes(&IselFunc, X64Abi, is_export: bool) -> Result<ObjFunc, NativeX64Error>` : per-fn byte assembly ; lowers G3 prologue + body + G3 epilogue in order, encodes each via `encoder::encode_into`, packages into the G5 `objemit::X64Func` boundary type.

- **Diagnostic-code unified block** (preserved from T11-D95)
  - **NX64-0001..NX64-0099** — top-level façade errors. T11-D97 changes the SEMANTICS of NX64-0001 (`BackendNotYetLanded`) : the variant is retained for back-compat but no longer produced by `emit_object_module`'s canonical path ; pre-G7 callers' pattern-matches still compile.
  - **X64-D5 + X64-0001..X64-0015** — instruction-selection errors (G1) — preserved.
  - **RA-0001..RA-0010** — register-allocation errors (G2) — preserved (G2 bypassed at S7-G7 ; reserved-block remains).
...
- **Consequences**
  - **Cranelift remains canonical default** per the dispatch §LANDMINES rule. csslc's `Backend::default_for_build()` still returns `Cranelift` ; `--backend=native-x64` is opt-in ; both backends ship in csslc per the dispatch policy.
  - **The native-hello-world gate flips from SKIP → PASS** : the gate test (`s7_g6_native_hello_world_executable_returns_42`) now asserts exit-code = 42 instead of skipping with an in-flight message ; the comparison gate (`s7_g6_backend_comparison_both_paths_exit_42_when_both_available`) reports "PASS — both backends produce hello.exe = 42" instead of the "SKIP (cranelift OK ; native-x64 G1..G5 in flight)" message.
  - **csslc dispatcher tests pass unchanged** — the 13 commands::build tests (including the dispatch-mechanism assertions + the backend-comparison assertions) keep their existing assertions ; the dispatch shape was already designed to accept both `Ok(_)` and the canonical `BackendNotYetLanded` error, so the SUCCESS path is exercised today + the SKIP-path becomes dead but still-compilable code.
  - **The bespoke-x64 trajectory mentioned in `specs/14_BACKEND.csl § OWNED x86-64 BACKEND` is closed at the leaf-subset level** — the second milestone exists ; the broader op-set + multi-block CFG + cross-fn calls + SSE2 float path + full G2 LSRA integration are explicit G8+ deferreds.
  - **All gates green** : fmt ✓ clippy (workspace, --all-targets) ✓ test 3495/0/16 ✓ smoke 4/4 ✓.

- **Closes the SECOND hello.exe = 42 milestone.** Phase-G owned x86-64 backend now produces real runnable executables end-to-end via the bespoke G-axis chain. The G7-pipeline cross-slice walker is the bridge that closes this milestone ; future slices expand the supported op-set (full ALU + control-flow + calls + SSE2 float).

- **Deferred** (explicit follow-ups, sequenced)
  - **G8 : full G2 LSRA integration** — replace `isel_to_encoder_simple` (the scalar-leaf G2 substitute) with a real `isel::X64Func → regalloc::X64Func conversion → regalloc::allocate → X64FuncAllocated → encoder::X64Inst` pipeline. Estimated scope : ~400 LOC across 2 bridge files + ~50 tests. Lands when the test corpus exposes the leaf-subset as too narrow.
  - **G9 : multi-block CFG support** — wire the scf.if/scf.for/scf.while/scf.loop control-flow through `isel::X64Term::{Jmp,Jcc,Fallthrough}` → encoder branch instructions with a branch-fixup pass (currently encoder accepts only known-relative offsets ; multi-block needs a placeholder + post-encoding-pass patch). Estimated scope : ~250 LOC + ~40 tests.
  - **G10 : cross-fn calls + relocation emission** — wire `isel::X64Inst::Call` + G3's `lower_call` (the SubRsp/StoreGpToStackArg/Call/AddRsp shape) through to `encoder::CallRel { target: BranchTarget::Rel32(0) }` + `objemit::X64Reloc { kind: NearCall, addend: -4 }`. The `abi_lower_to_encoder` adapter currently rejects Call/StoreGpToStackArg/StoreXmmToStackArg with G10 hints ; this slice flips them to real lowerings with reloc emission. Estimated scope : ~300 LOC + ~50 tests.
  - **G11 : SSE2 float pipeline** — extend `isel_to_encoder_simple` to handle `FpAdd / FpSub / FpMul / FpDiv / Ucomi / Comi` ; verify the `0xF2 / 0xF3 / 0x66` SSE2-prefix discipline already in encoder works end-to-end. Estimated scope : ~200 LOC + ~40 tests.
  - **Native-x64 byte-stability test** — add a regression test that asserts the exact byte sequence for `fn main() -> i32 { 42 }` matches the canonical 11-byte snapshot. The `milestone_main_42_byte_pattern_matches_expected_mov_eax_ret` test currently asserts this at the per-fn level pre-objemit ; lifting it to a workspace-level test once the byte ordering of the G5 object emitter is canonicalized is a polish-pass.
  - **cssl-rt cold-cache test flake** (carried-over from T11-D56 / T11-D58 / T11-D61 / T11-D70 / T11-D73 / T11-D74 / T11-D75 / T11-D76 / T11-D80 / T11-D81 / T11-D85 / T11-D89 / T11-D90 / T11-D92 / T11-D93 / T11-D95) : still tracked. Workaround `--test-threads=1` consistent. **This slice does not introduce new flakes.**
