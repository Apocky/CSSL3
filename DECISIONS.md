...
## § T11-D64 : Session-6 S6-C5 — closures with environment capture (Lambda env-capture)

> **PM allocation note** : T11-D60..D63 are reserved for parallel-fanout slices that may merge in any order ; T11-D64 is the next-available slot per `SESSION_6_DISPATCH_PLAN.md § 4` for S6-C5. PM may renumber on integration to keep DECISIONS commit-order monotonic.

- **Date** 2026-04-28
- **Status** accepted (PM may renumber on merge)
- **Session** 6 — Phase-C control-flow + JIT enrichment, slice 5 of 5
- **Branch** `cssl/session-6/C5`
...
- **The capability claim**
  ...
  - Runtime : 22 unit tests pass (15 mir + 2 cap_check + 5 cgen) ; the closure surface lowers end-to-end through the object backend with the heap-FFI imports correctly threaded.
  - **First time CSSLv3 source can mint closures with environment capture that flow end-to-end through the pipeline.** Phase-C scope-5 success-gate met.
- **Consequences**
  - Test count : 1778 → 1800 (+22 net : 15 mir + 2 cap_check + 5 cgen). Workspace baseline preserved.
  - **Spec-hole closed** : `specs/02_IR.csl § CLOSURE-ENV` is now canonical for closure-value layout, free-var analysis, capture rules, env-layout, escape analysis, iso ownership, MIR shape, and indirect-call deferred-path documentation. `specs/09_SYNTAX.csl § lambda` carries a cross-reference for the surface-to-IR pointer.
  ...
  - All gates green : fmt ✓ clippy ✓ test 1800/0 ✓ doc ✓ xref ✓ smoke 4/4 ✓.
- **Closes the S6-C5 slice.** Phase-C is now complete (C1 ✓ C2 ✓ C3 ✓ C4 ✓ C5 ✓). Phase-C scope-5 success-gate met.
- **Deferred** (explicit follow-ups, sequenced)
  ...
  - **GPU-side closure lowering** : not currently planned for stage-0 ; closures are CPU-only until a future Phase-D slice grows function-pointer or per-call-site inlining infrastructure on the GPU emitters.
