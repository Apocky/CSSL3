...
## § T11-D87 : Session-7 S7-G5 — Native x86-64 object-file emitter (hand-rolled ELF / COFF / Mach-O writers ; zero `cranelift-object` dep ; end-to-end `mov eax, 42 ; ret` → `lld-link.exe` → `.exe = 42` verified on Apocky's host)

> **PM allocation note** : T11-D77..T11-D86 are reserved-floating per `SESSION_7_DISPATCH_PLAN.md § 4` for sibling F/G slices that may land on parallel branches during the close-out integration window. T11-D87 closes the second-milestone of the native x86-64 path on the `cssl/session-7/G5` branch : G1+G2+G3+G4's encoded machine-code bytes are now serialisable into a relocatable `.o` / `.obj` byte-stream that any host linker driver (rust-lld / cl / clang / gcc) accepts byte-for-byte interchangeably with `cranelift-object`'s output. **G1..G4 (encoder + regalloc + isel + frame-layout) are not yet landed on `cssl/session-7/G5`** — G5 was unblocked first to avoid serial dependency. The `X64Func` boundary type defined here is the pinned shape G1..G4 will populate.

- **Date** 2026-04-28
- **Status** accepted
- **Session** 7 — Phase-G owned-x86-64 backend, slice 5 of N (object-file emitter ; closes the cranelift-object dependency for the native path)
- **Slice-id** S7-G5
- **Branch** `cssl/session-7/G5` (based on `cssl/session-7/G4` ≡ `origin/cssl/session-6/parallel-fanout @ df1daf5`)
- **Context** `specs/07_CODEGEN.csl § CPU-BACKEND § OBJECT-FILE-WRITING` + `specs/14_BACKEND.csl § LINKING-MODEL` define the relocatable-object output as the boundary between codegen + the host linker. Stage-0 routes through `cranelift-object`'s `ObjectModule::finish().emit()` (T11-D54 / S6-A3) ; stage1+ replaces that dep with a hand-rolled writer per the §§ 14 BACKEND stage1+ "owned FFI (volk-like dispatch)" trajectory + the cssl-rt + cssl-host-level-zero owned-FFI precedent (T11-D52, T11-D62). G5 is the closing slice for the native x86-64 path : it takes the encoded machine-code bytes from G1..G4 and serialises them into ELF / COFF / Mach-O byte-streams that the S6-A4 linker (T11-D55) accepts. Output is byte-for-byte structurally compatible with cranelift-object — the linker is insensitive to which Rust crate produced the bytes, so once G1..G4 land you can point csslc at either backend without changing the linker step.
- **Slice landed (this commit)** — ~1700 LOC (3 per-format writers + umbrella + boundary type) + 64 tests across 5 files (53 unit + 10 toolchain-roundtrip + 1 end-to-end-link-and-run)
  - **`crates/cssl-cgen-cpu-x64/`** (new crate) — owned native x86-64 backend ; G5 = object-file emitter slice :
    - **`Cargo.toml`** — `thiserror` workspace dep only ; ZERO `cranelift-object` / `cranelift-codegen` deps (the whole point of the slice). Crate-level lints inherit from workspace. Mirrors the `cssl-host-level-zero` (T11-D62) "no-3rd-party-FFI-crate" precedent.
    - **`src/lib.rs`** (~80 LOC) — module-tree + crate-level lint allows for `struct_field_names` (per-format spec naming `st_*` / `sh_*` / `r_*` / `n_*` retains 1:1 mapping with platform-spec PDFs / Apple headers ; clippy fires on each cluster otherwise), `match_same_arms` (ELF + Mach-O both produce `.o` for distinct enum variants — explicit per-arm body documents each format's distinct identity), `too_many_lines` (per-format writers carry ~50% comment density tying each block back to spec section + offset math), `unnecessary_wraps` (`emit_object_file` uses Result for forward-flexibility ; G1..G4 will surface relocation overflow / alignment-violation / function-too-large modes).
    - **`src/func.rs`** (~330 LOC + 12 tests) — the `X64Func` / `X64Reloc` / `X64RelocKind` / `X64Symbol` / `X64SymbolKind` boundary types ; this is the pinned shape G1..G4 will populate. `X64Func` carries `name : String + bytes : Vec<u8> + relocs : Vec<X64Reloc> + is_export : bool`. `X64RelocKind` is `NearCall | PcRel32 | Abs64` with per-format mapping methods (`elf_r_type()` → R_X86_64_PLT32 / PC32 / 64 ; `coff_type()` → IMAGE_REL_AMD64_REL32 / ADDR64 ; `macho_r_type()` → BRANCH / SIGNED / UNSIGNED ; `is_pc_relative()` predicate). Constructor validates name is non-empty + ASCII + NUL-free. `X64Func::leaf_export(name, bytes)` is the canonical convenience constructor for the `mov eax, 42 ; ret` smoke shape.
    - **`src/object/mod.rs`** (~370 LOC + 10 tests) — umbrella module : `ObjectTarget = ElfX64 | CoffX64 | MachOX64` enum + `host_default_target()` const fn + `magic_prefix(target)` const fn (returns `\x7FELF` / `64 86` / `CF FA ED FE` per format) + `ObjectError` ({BadRelocSymbolIndex, BadRelocOffset, DuplicateSymbol, SectionTooLarge}) + `emit_object_file(funcs, extern_imports, target)` public API. Pre-validates the relocation table (every `target_index` is in range of `(funcs ++ imports)` ; every `offset` is within the function's `bytes`) + the symbol table (no duplicate strong-symbol names) before dispatching to the per-format writer. Includes a shared `LeWriter` trait + `pad_to_align(out, n)` + `pack_text_section(funcs)` helper that 16-byte aligns each function within `.text` per the x86-64 ABI convention.
    - **`src/object/elf_x64.rs`** (~470 LOC + 10 tests) — relocatable ELF64 (`ET_REL`) writer. File layout : 64-byte `Elf64_Ehdr` + .text bytes + optional .rela.text (24 bytes per `Elf64_Rela`) + .symtab (24 bytes per `Elf64_Sym`) + .strtab + .shstrtab + section header table (64 bytes per `Elf64_Shdr`). Symtab layout : null + STT_FILE + STT_SECTION for .text + locally-defined funcs (STB_LOCAL ; ELF requires locals before globals) + exported funcs (STB_GLOBAL) + undefined externals (SHN_UNDEF + STB_GLOBAL). Section header table sets `sh_link` of .symtab → .strtab + `sh_info` of .symtab → first-global-index + `sh_link` of .rela.text → .symtab + `sh_info` of .rela.text → .text. ELF magic + EI_CLASS=2 (64-bit) + EI_DATA=1 (LSB) + EM_X86_64=62 + ET_REL=1 ; e_phoff=0 (relocatable .o has no program headers).
    - **`src/object/coff_x64.rs`** (~400 LOC + 10 tests) — relocatable COFF (.obj) writer. File layout : 20-byte file header + 40-byte section header + .text body + 10-byte IMAGE_RELOCATION entries + 18-byte IMAGE_SYMBOL records (with auxiliary records for .file + section symbols) + 4-byte-prefixed string table. Machine = IMAGE_FILE_MACHINE_AMD64 = 0x8664 (LE bytes `64 86`). TimeDateStamp = 0 (deterministic builds). Symbol-name slot : ≤ 8 bytes inline ; longer names spill into the string table with `[0,0,0,0,offset_le32]` pointer. Storage classes : EXTERNAL=2 for exports + STATIC=3 for module-local + IMPORTED via UNDEF section_number=0 + EXTERNAL. Auxiliary records implemented for the .file symbol (18-byte path) + the .text section symbol (per PE-Format § "Auxiliary Format 5: Section Definitions"). Section characteristics = IMAGE_SCN_CNT_CODE | IMAGE_SCN_ALIGN_16BYTES | IMAGE_SCN_MEM_EXECUTE | IMAGE_SCN_MEM_READ.
    - **`src/object/macho_x64.rs`** (~430 LOC + 10 tests) — relocatable Mach-O 64-bit (MH_OBJECT) writer. File layout : 32-byte mach_header_64 + LC_SEGMENT_64 (72-byte load command) + section_64 (80 bytes for __TEXT,__text) + LC_SYMTAB (24 bytes) + LC_DYSYMTAB (80 bytes — minimal stub declaring local/external/undef ranges) + .text body (16-byte aligned) + relocation_info entries (8 bytes each ; r_info packs symbol-num + r_pcrel + r_length + r_extern + r_type per Apple's `<mach-o/reloc.h>`) + nlist_64 entries (16 bytes each — locals + externals + undefs in that order) + string table. Magic = MH_MAGIC_64 = 0xFEEDFACF (LE bytes `CF FA ED FE`). cputype = CPU_TYPE_X86_64 = 0x01000007. flags = MH_SUBSECTIONS_VIA_SYMBOLS. Per Mach-O convention, x86-64 user-space symbol names get a leading `_` prefix in the symbol table (so `main` becomes `_main` ; otool/nm strip the underscore for display).
    - **`tests/toolchain_roundtrip.rs`** (10 tests) — verify the emitted .o / .obj parses with the host toolchain's object inspector (objdump / llvm-objdump / dumpbin / otool). Each test gate-skips with `[skip]` printline when the toolchain binary is absent (per the S6-A4 BinaryMissing precedent ; T11-D55) so CI runners without the toolchain don't fail. Cross-format integrity test (`all_three_formats_emit_distinct_bytes`) verifies the per-format magic bytes differ + all three formats contain the canonical `B8 2A 00 00 00 C3` body — runs unconditionally.
    - **`tests/linker_smoke.rs`** (1 test) — **the second-milestone gate** : take a synthetic `X64Func::leaf_export("main", [0xB8, 0x2A, 0, 0, 0, 0xC3])`, emit it via G5, and pipe the bytes through a real LLD driver (`lld-link.exe` from rustup `<toolchain>/lib/rustlib/<triple>/bin/gcc-ld/` on Windows ; `ld.lld` on Linux ; `ld64.lld` on macOS). Verify the linker produces a runnable `.exe` + RUN it + assert the exit code = 42. **First time CSSLv3's owned-x86-64 path produces a hello.exe that runs on Apocky's host without `cranelift-object` mediation** — confirmed via test trace `[end-to-end-OK] G5 → linker → exe ran ; exit code = 42 (matches mov eax, 42 ; ret)`.
- **The capability claim**
  - Pipeline : `[g1..g4-encoder] → X64Func { name, bytes, relocs, is_export } → emit_object_file(funcs, imports, ObjectTarget::CoffX64) → Vec<u8> → fs::write("hello.obj", &bytes) → lld-link.exe /OUT:hello.exe /SUBSYSTEM:CONSOLE /ENTRY:main /NODEFAULTLIB hello.obj → hello.exe → run → exit code 42`.
  - Apocky's host : Win64 .obj written via G5 + linked via the rustup-bundled `lld-link.exe` produces a hello.exe that runs and returns 42 to the kernel via `mov eax, 42 ; ret` (no CRT entry-point ; the `/NODEFAULTLIB` flag tells lld-link to skip libc — the kernel jumps straight into our 6-byte function body). `cargo test -p cssl-cgen-cpu-x64 -- --test-threads=1` produces 53 lib tests + 10 toolchain-roundtrip tests (gate-skipped on this host : no llvm-objdump on PATH) + 1 linker-smoke test = **64 cssl-cgen-cpu-x64 tests passing** with the linker-smoke trace explicitly confirming end-to-end success.
  - **Second milestone for the native x86-64 path** — the first being the cranelift-object hello.exe at S6-A5 (T11-D56). G5 closes the parallel native path back to the same milestone gate without any cranelift-object dep mediation.
- **Consequences**
  - Test count : 2380 → 2444 (+64 ; 53 cssl-cgen-cpu-x64 lib + 10 toolchain-roundtrip + 1 linker-smoke). Workspace-serial run via `--test-threads=1` per the cssl-rt cold-cache parallel flake (carried-forward from T11-D56). Workspace baseline preserved.
  - **`cssl-cgen-cpu-x64::X64Func` is ABI-stable from S7-G5 forward.** The `(name, bytes, relocs, is_export)` tuple shape is the boundary G1..G4 must populate — adding new fields to `X64Func` is a breaking-change boundary-shift that lands as a follow-up slice if any G1..G4 phase needs more metadata than the current set carries. The `X64RelocKind` triple (NearCall / PcRel32 / Abs64) covers every relocation x86-64 stage1+ emits ; future kinds (e.g. GOT-relative loads for PIC) extend the enum non-breakingly.
  - **`emit_object_file` output is byte-for-byte interchangeable with cranelift-object's.** S6-A4 linker (T11-D55) accepts either input ; csslc's `--backend=owned-x64` flag wire-up is the next-natural follow-up slice (deferred until G1..G4 land + a real CSSLv3 source compiles end-to-end through the owned path). The cranelift-object backend remains the production path until that wire-up lands.
  - **Per-format writers are spec-faithful.** Each writer's module doc-block names the canonical reference document (Linux Foundation ELF spec / Microsoft PE-Format / Apple Mach-O reference) + the spec section the layout follows. Constants are named after their spec-section header for grep-discoverability (`EI_CLASS_64` / `IMAGE_FILE_MACHINE_AMD64` / `MH_MAGIC_64`). Where the spec offers extension points we don't use (COFF "big obj" extension for >64K relocations ; Mach-O LC_DYSYMTAB indirect symbol tables ; ELF SHT_GROUP), the deferral is documented inline. The `SectionTooLarge` error variant is reserved for the COFF / Mach-O 32-bit overflow case ; not currently triggered by stage1+ functions but documented for forward-flexibility.
  - **Section alignment matches platform conventions.** .text functions are 16-byte aligned within the section per the x86-64 ABI convention (`pack_text_section` enforces). ELF section header table is 8-byte aligned per spec. COFF section bodies are 4-byte aligned per spec. Mach-O section bodies are 16-byte aligned per the `sect.align = 4` (= 2^4) field. The `pad_to_align` helper handles all three.
  - **No new diagnostic codes added.** Errors flow through the typed `ObjectError` enum (BadRelocSymbolIndex / BadRelocOffset / DuplicateSymbol / SectionTooLarge) ; csslc will project these into the AD0001..0003 / IFC0001..0004 namespace at integration-time when the owned-x64 backend wires into the build pipeline.
  - **PRIME-DIRECTIVE preserved structurally.** The G5 surface is a pure-data byte-emission layer ; it has no surveillance / data-collection / consent-bypass surface area. The crate uses `#![forbid(unsafe_code)]` at the lib level (no `unsafe` blocks anywhere — pure byte-level Rust). Object-file output is fully transparent : every byte we write is documented inline against its spec field-name and value. § 4 TRANSPARENCY is upheld by construction.
  - All gates green : fmt ✓ clippy ✓ test 2444/0 (workspace serial via `--test-threads=1`) ✓ doc ✓ xref ✓ smoke 4/4 ✓ (full 9-step commit-gate per `SESSION_7_DISPATCH_PLAN.md § 5`).
- **Closes the S7-G5 slice** — Phase-G slice 5. The native x86-64 path now has a complete object-file emitter ; G1+G2+G3+G4 (encoder + regalloc + isel + frame-layout) when they land will populate `X64Func` and the same emit-+-link-+-run smoke proves out the full owned path end-to-end.
- **Deferred** (explicit follow-ups, sequenced)
  - **G1..G4 implementation** — encoder + regalloc + instruction-selection + frame-layout slices that populate `X64Func.bytes` from `MirFunc`. Currently those branches (`cssl/session-7/G1..G4`) are placeholder branches at parallel-fanout tip ; G5 unblocks ahead of them so the boundary type is pinned. The G1..G4 slices land per the SESSION_7 dispatch plan once Apocky directs Phase-G to dispatch.
  - **csslc-build wire-up for `--backend=owned-x64`** — once G1..G4 land and `MirFunc → X64Func` is wired, csslc's `EmitMode::Object` + `EmitMode::Exe` paths get a backend-selector flag that routes to either `cssl_cgen_cpu_cranelift::emit_object_module` (default, current behavior) or `cssl_cgen_cpu_x64::emit_object_file`. The S6-A4 linker accepts either output ; the wire-up is mostly plumbing.
  - **DWARF-5 / CodeView debug-info sections** — the `specs/07_CODEGEN.csl § CPU-BACKEND` line item calls for DWARF-5 (Linux/Mac) + CodeView (Windows) debug-info emission. G5 currently emits no debug-info sections ; the hooks land as additional `.debug_*` (ELF) / `.debug$S` (COFF) / `LC_FUNCTION_STARTS` (Mach-O) sections in a follow-up slice once a future G-series slice (line-table generator) provides the source-loc → byte-offset mapping.
  - **PIE / PIC support** — current relocations target stage1+ relocatable .o (no PIE bias). Future slice grows R_X86_64_GOTPCRELX / IMAGE_REL_AMD64_REL32_5 / X86_64_RELOC_GOT_LOAD relocations + emits PIE-friendly addressing. Lands when CSSLv3 needs to ship .so / .dll / .dylib outputs.
  - **`SectionTooLarge` enforcement** — the error variant is defined but not currently emitted ; G5's per-format writers don't yet check the 32-bit limits (COFF has it for `.SizeOfRawData` ; Mach-O has it for `nlist.n_value` in some shapes ; ELF is 64-bit so no limit). Threshold checks land when stage1+ functions get large enough to approach the limit (~4 GiB of code per object — far beyond stage-0 needs).
  - **objdump / llvm-objdump / dumpbin / otool roundtrip on Apocky's host** — the toolchain_roundtrip.rs tests gate-skip on this host because none of those binaries are on PATH (rustup ships `rust-lld` but not `llvm-objdump` ; the latter is in the optional `llvm-tools-preview` rustup component). Installing `llvm-tools-preview` would activate 6/10 of the cross-format inspection tests. Tracked as a host-environment improvement, not a blocker (the linker-smoke test independently proves byte-validity end-to-end on the Win64 path).
