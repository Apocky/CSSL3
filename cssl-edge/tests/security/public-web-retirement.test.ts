...
const authenticatedRuntimeUrls = [
  ...
  'https://www.apocky.com/api/apocrypha/jobs',
  'https://www.apocky.com/api/apocrypha/worker/heartbeat',
  'https://www.apocky.com/api/admin/apocrypha/chat',
  'https://www.apocky.com/api/admin/apocrypha/jobs',
...
async function testRetiredRoutes(): Promise<void> {
  ...
  for (const url of authenticatedRuntimeUrls) {
    const request = new NextRequest(url, { method: url.includes('/api/') ? 'POST' : 'GET' });
    assert.equal(isRetiredWebRuntimeRequest(request), false, `${url} must reach its own authorization handler`);
    const response = middleware(request);
    assert.equal(response.status, 200, `${url} must pass middleware`);
    assert.match(response.headers.get('cache-control') ?? '', /no-store/, `${url} must stay private`);
    assert.match(response.headers.get('x-robots-tag') ?? '', /noindex/, `${url} must not be indexed`);
  }

  const serviceHeaders = {
    authorization: 'Bearer service-token-shape-only',
    'x-mneme-profile': 'apocrypha-worker',
    'x-mneme-capability': 'mneme.recall',
  };
  const serviceRequest = new NextRequest('https://www.apocky.com/api/mneme/apocrypha-worker/recall', {
    method: 'POST',
    headers: serviceHeaders,
  });
  assert.equal(isUnbrokeredPrivateRuntimeRequest(serviceRequest), false, 'well-shaped service recall reaches exact handler authentication');
  assert.equal(middleware(serviceRequest).status, 200, 'well-shaped service recall passes middleware');
  for (const headers of [
    {},
    { ...serviceHeaders, 'x-mneme-profile': 'different-profile' },
    { ...serviceHeaders, 'x-mneme-capability': 'mneme.export' },
  ]) {
    const denied = new NextRequest('https://www.apocky.com/api/mneme/apocrypha-worker/recall', { method: 'POST', headers });
    assert.equal(isUnbrokeredPrivateRuntimeRequest(denied), true, 'missing or mismatched service broker headers fail closed');
    assert.equal(middleware(denied).status, 404, 'unbrokered service request stays opaque');
  }
