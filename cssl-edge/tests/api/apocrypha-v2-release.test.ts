...
async function main(): Promise<void> {
  ...
  for (let index = 0; index < 2; index += 1) {
    const turn = reqRes('POST', {
      body: {
        ...
        request_id: clientRequestIds[index],
      },
    });
    ...
    equal(response.request_id, clientRequestIds[index], 'client request UUID echoed exactly');
    equal(
    ...
    equal(response.duplicate_commit_protection, 'blocked', 'retry deduplication gap is explicit');
  }
  equal(turnCalls.length, 2, 'two turns reached the body');
  ...
  assert(!('request_id' in (turnCalls[0]?.body ?? {})), 'unsupported request identity is not invented upstream');
  assert(!('idempotency_key' in (turnCalls[0]?.body ?? {})), 'unsupported idempotency field is not invented upstream');
  ...
  assert(threadSource.includes('backend_turn_contract_has_no_idempotency_field'), 'duplicate-commit blocker is explicit');
