...
async function main(): Promise<void> {
  ...
  try {
    ...
    const system = String((hostedBody.messages as Array<{ content: string }>)[0]?.content ?? '');
    assert(system.includes('ATTACHMENT_MARKER') && system.includes('name="notes.txt"'), 'attachment text did not reach the system prompt');
