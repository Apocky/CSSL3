...
assert.match(composer, /Compile writes nothing/);
...
assert.match(sigils, /VISIBLE GEOMETRY · NO HIDDEN PAYLOAD/);
assert.match(spellbook, /Import verified JSON/);
...
for (const id of ['oracle', 'spellcraft', 'sigils', 'spellbook']) {
...
assert.match(atlas, /One field\. Four readable views\./);
