...
for (const token of ['ChatThread', "'owner'", '/api/admin/check', '/api/apocrypha/presence']) {
  assert(page.includes(token), `owner-direct chat contract missing: ${token}`);
}
for (const source of [page, thread, rest, retiredStream, history, presence]) {
...
assert(page.includes('No avatar is authorized for this interaction.'), 'hidden-presence explanation missing');
...
assert(thread.includes('backend_turn_contract_has_no_idempotency_field'), 'duplicate-commit blocker missing');
...
console.log('chat-presence.test : OK · V2-only REST, scoped continuity, hidden avatar, and capability truth passed');
