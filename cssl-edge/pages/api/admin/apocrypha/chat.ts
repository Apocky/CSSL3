...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  ...
  if (!text) {
    res.status(400).json({
      error: `text must contain 1-${MAX_TEXT_CHARS} non-whitespace characters`,
      ...envelope(),
    });
    return;
  }
  ...
  if (!committedEnvelope) {
    res.status(502).json({
      ...
      duplicate_commit_protection: 'blocked',
      duplicate_commit_blocker: DUPLICATE_COMMIT_BLOCKER,
      ...envelope(),
    });
    return;
  }
