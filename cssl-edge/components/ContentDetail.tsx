...
//
// Surfaces:
//   - title · author (revocable-pubkey-tag · ⊘ glyph)
//   - description · screenshots
//   - install-button (deep-link or download)
//   - ratings summary + distribution histogram
//   - remixes list + attribution chain (oldest → newest)
//   - cosmetic-axiom-attestation pill
//
// Sovereignty:
//   - cap-revocability glyph clickable · routes to /docs/sovereign-cap
//   - cosmetic-axiom-attestation prominently displayed
//   - "why am I seeing this" : detail-page = direct nav, no rationale
//   - NO ¬ engagement-tracking · NO ¬ scroll-depth
  ...
  return (
    <article style={{ paddingBottom: '4rem' }}>
      {stubMode && (
        ...
        >
          <strong>◐ stub-mode</strong> · publish-detail-API (sibling W12-5) not yet wired ·
          placeholder rendered to demonstrate layout
        </div>
      )}
      ...
      <header style={{ marginBottom: '2rem' }}>
        ...
        >
          ...
          {detail.cosmetic_axiom_attested && (
            <span
              title="creator has attested cosmetic-axiom-compliance · no pay-for-power · no gameplay-altering content"
              style={{
                padding: '0.15rem 0.6rem',
          ...
          {detail.cap_revocable && (
            <a
              ...
              title="Σ-mask : creator capability is unilaterally revocable"
              style={{
                padding: '0.15rem 0.6rem',
        ...
        >
          ...
          {detail.tags.length > 0 && (
            <span>· {detail.tags.slice(0, 6).join(' · ')}</span>
          )}
        </p>
      </header>
          ...
          >
            {detail.screenshots.map((src, idx) => (
              <img
                key={src}
                src={src}
                alt={`screenshot ${idx + 1} of ${detail.title}`}
                loading="lazy"
                style={{
                  width: '100%',
                  height: 'auto',
      ...
      {/* RATINGS · histogram-style with SVG-free CSS bars */}
      <section style={{ marginBottom: '2.5rem' }}>
        <h2
          style={{
            fontSize: '0.75rem',
        ...
        >
          § Ratings · {totalRatings} total
        </h2>
        {totalRatings === 0 ? (
          <p style={{ color: '#5a5a6a', fontSize: '0.85rem', margin: 0 }}>
            ○ no ratings yet · ¬ aggregate available
          </p>
        ) : (
          <div
            style={{
              ...
              {[5, 4, 3, 2, 1].map((star) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ color: '#5a5a6a', minWidth: '2rem', textAlign: 'right' }}>
                      {count}
                    </span>
                  </div>
                );
      ...
      {/* ATTRIBUTION CHAIN · oldest → newest */}
      {detail.attribution_chain.length > 1 && (
        <section style={{ marginBottom: '2.5rem' }}>
          ...
          >
            § Attribution Chain · {detail.attribution_chain.length} generations
          </h2>
          <ol
            style={{
              listStyle: 'none',
          ...
          >
            {detail.attribution_chain.map((link: AttributionLink, idx) => (
              <li
                key={link.slug}
                style={{
                  marginBottom: idx === detail.attribution_chain.length - 1 ? 0 : '0.75rem',
                  fontSize: '0.85rem',
                  color: '#cdd6e4',
                }}
              >
                ...
                <a
                  href={`/content/${encodeURIComponent(link.slug)}`}
                  style={{ color: '#7dd3fc', textDecoration: 'none' }}
                >
                  {link.title}
                </a>
                <span style={{ color: '#5a5a6a', marginLeft: '0.5rem', fontSize: '0.78rem' }}>
                  by {displayAuthor({ author_pubkey: link.author_pubkey })}
                </span>
              </li>
            ))}
          </ol>
      ...
      {detail.remix_slugs.length > 0 && (
        <section style={{ marginBottom: '2.5rem' }}>
          ...
          >
            § Remixes · {detail.remix_slugs.length} downstream
          </h2>
          <ul
            style={{
              listStyle: 'none',
