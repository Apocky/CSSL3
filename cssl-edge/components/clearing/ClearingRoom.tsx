        ...
        <div className={styles.layout}>
          ...
          <section className={styles.chat} aria-label="Conversation">
            ...
            <div className={styles.stream} aria-live="polite">
              {props.messages.length === 0 && <div className={styles.emptyStream}><span aria-hidden="true">◇</span><p>{props.liveState === 'loading' ? 'Opening the room…' : props.liveState === 'unavailable' || props.liveState === 'reconnecting' ? 'Waiting for the room to reconnect.' : 'No messages yet.'}</p><small>{props.liveState !== 'live' ? 'Messages will appear here when the room is ready.' : props.session === 'signed-in' ? 'Start a conversation when you’re ready.' : 'Sign in to be one of the first to post here.'}</small></div>}
