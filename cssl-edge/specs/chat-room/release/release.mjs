...
async function baseline(scope) {
  ...
  check(manifest.sourceFiles.length === 727 && body.files.length === 727, 'Historical source count changed.');
  ...
  check(same(scope.overlays.map(x => x.file), OWNED), 'Only the seven reviewed files may be overlaid.');
  ...
  check(replaced === 4 && added === 3 && payloads.size === 730, 'Release denominator changed.');
...
async function prepare(scope, scopeSha256, frozen) {
  const stage = await fs.mkdtemp(path.join(os.tmpdir(), 'apocky-chat-room-'));
  for (const [file, data] of frozen.payloads) await exclusive(path.join(stage, 'app', file), data);
  const manifest = { schema: 'apocky.chat-room.candidate.v1', createdAt: stamp(), scopeSha256,
...
async function loadStage(scope, scopeSha256, frozen, opts) {
  ...
  check(inside(temp, real) && path.basename(real).startsWith('apocky-chat-room-') && real === await fs.realpath(path.dirname(stage)).then(p => path.join(p, path.basename(stage))), 'Candidate must be an owned temporary directory.');
  const bytes = await read(stage, 'manifest.json');
  check(hash(bytes) === opts['manifest-sha256'], 'Candidate manifest seal changed.');
  const manifest = JSON.parse(bytes);
  check(manifest.schema === 'apocky.chat-room.candidate.v1' && manifest.scopeSha256 === scopeSha256 &&
...
async function gitProof(scope, prepared, commit) {
  ...
  for (const file of ['release.mjs', 'scope.json', 'RELEASE.csl']) rows.push({ file: 'specs/chat-room/release/' + file, data: await read(HERE, file) });
  const comparisons = [];
  for (const row of rows) {
...
async function candidate(scope, prepared) {
  ...
  check(current.projectId === scope.project && current.meta?.chatRoomSourceSet === prepared.sourceSetSha256 && current.meta?.chatRoomManifest === prepared.manifestSha256 && current.meta?.chatRoomGitCommit === receipt.gitCommit, 'Candidate identity changed.');
...
async function deploy(scope, prepared, opts) {
  ...
  if (opts.promote) {
    ...
    check(qa.deploymentId === target.id && qa.sourceSetSha256 === prepared.sourceSetSha256 && qa.passed === true &&
      ['desktop', 'mobile', 'keyboard', 'tool-output', 'explicit-insertion', 'auth-history'].every(k => qa.checks?.[k] === true), 'Candidate visual and interaction acceptance is incomplete.');
  ...
  body.meta = { ...body.meta, sourceDeployment: scope.baseline.id, chatRoomManifest: prepared.manifestSha256, chatRoomSourceSet: prepared.sourceSetSha256, chatRoomGitCommit: proof.commit };
