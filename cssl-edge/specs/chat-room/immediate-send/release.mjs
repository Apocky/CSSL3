...
const OWNED = ['components/brain/BrainExperience.tsx', 'tests/e2e/brain.spec.ts'];
...
async function baseline(scope) {
  ...
  check(same(scope.overlays.map(x => x.file), OWNED), 'Only the two reviewed files may be overlaid.');
  ...
  check(replaced === 2 && added === 0 && payloads.size === 730, 'Release denominator changed.');
...
async function prepare(scope, scopeSha256, frozen) {
  const stage = await fs.mkdtemp(path.join(os.tmpdir(), 'apocky-immediate-send-'));
  for (const [file, data] of frozen.payloads) await exclusive(path.join(stage, 'app', file), data);
  const manifest = { schema: 'apocky.immediate-send.candidate.v1', createdAt: stamp(), scopeSha256,
...
async function loadStage(scope, scopeSha256, frozen, opts) {
  ...
  check(inside(temp, real) && path.basename(real).startsWith('apocky-immediate-send-') && real === await fs.realpath(path.dirname(stage)).then(p => path.join(p, path.basename(stage))), 'Candidate must be an owned temporary directory.');
  const bytes = await read(stage, 'manifest.json');
  check(hash(bytes) === opts['manifest-sha256'], 'Candidate manifest seal changed.');
  const manifest = JSON.parse(bytes);
  check(manifest.schema === 'apocky.immediate-send.candidate.v1' && manifest.scopeSha256 === scopeSha256 &&
...
async function gitProof(scope, prepared, commit) {
  ...
  for (const file of ['release.mjs', 'scope.json', 'RELEASE.csl']) rows.push({ file: 'specs/chat-room/immediate-send/' + file, data: await read(HERE, file) });
  const comparisons = [];
  for (const row of rows) {
...
async function candidate(scope, prepared) {
  ...
  check(current.projectId === scope.project && current.meta?.immediateSendSourceSet === prepared.sourceSetSha256 && current.meta?.immediateSendManifest === prepared.manifestSha256 && current.meta?.immediateSendGitCommit === receipt.gitCommit, 'Candidate identity changed.');
...
async function deploy(scope, prepared, opts) {
  ...
  if (opts.promote) {
    ...
    check(qa.deploymentId === target.id && qa.sourceSetSha256 === prepared.sourceSetSha256 && qa.passed === true &&
      ['desktop', 'mobile', 'online-send', 'retry', 'reconnect', 'history-preserved'].every(k => qa.checks?.[k] === true), 'Candidate visual and interaction acceptance is incomplete.');
  ...
  body.meta = { ...body.meta, sourceDeployment: scope.baseline.id, immediateSendManifest: prepared.manifestSha256, immediateSendSourceSet: prepared.sourceSetSha256, immediateSendGitCommit: proof.commit };
