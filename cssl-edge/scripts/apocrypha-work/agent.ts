...
function buildSystemPrompt(workspace: Workspace, config: WorkConfig): string {
  ...
  return [
    ...
    '',
    '- RECALL before re-deriving. You have run_command: one federated read-only entry point',
    '  covers every memory region here --',
    '    python C:/Users/Apocky/source/repos/anamnesis/unirecall.py --tiers <t> -n 5 "<topic>"',
    '  tiers: l1 (mempalace), vault (the Obsidian vault, ~8000 notes), graph (graphify AST),',
    '  l2, l34, transport (3MNEME), metaharness. Also anamnesis.py state for the ledger and the',
    '  open owes. SCOPE BY TIER -- unscoped, "palworld crash" returns tarot card text.',
