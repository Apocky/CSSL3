-- 0057 · Apocrypha member chat: threads, flagship lane, attachments, consented analytics,
-- telemetry projection. Owner decisions 2026-09-25 (apocky.com front door = Apocrypha; sidebar
-- with history / new / pin / archive; plus-icon uploads; premium plan on Stripe with a flagship
-- switch; admin telemetry with conversation metadata and tracking that only collects with
-- consent).
--
-- The 0049 identity binding is untouched: a member still has exactly one conversation
-- (conversation_id = auth user id). THREADS live inside it. Every request row now names its
-- thread, the lane it ran on, and the attachments it carried; the job request JSON carries the
-- same so the worker routes and prompts from the record, not from the caller.
...
CREATE TABLE public.apocrypha_member_chat_thread (
    ...
    CONSTRAINT apocrypha_member_chat_thread_principal_fk
        FOREIGN KEY (tenant_id, principal_id)
        REFERENCES public.apocrypha_principal(tenant_id, id) ON DELETE CASCADE,
    CONSTRAINT apocrypha_member_chat_thread_conversation_fk
        FOREIGN KEY (tenant_id, principal_id, conversation_id)
        REFERENCES public.apocrypha_member_chat_conversation(tenant_id, principal_id, conversation_id)
        ON DELETE CASCADE,
    CONSTRAINT apocrypha_member_chat_thread_scope_unique UNIQUE (tenant_id, principal_id, id),
    CONSTRAINT apocrypha_member_chat_thread_title_shape
        CHECK (title = btrim(title) AND char_length(title) BETWEEN 1 AND 120)
);
CREATE INDEX apocrypha_member_chat_thread_list
    ON public.apocrypha_member_chat_thread (tenant_id, principal_id, archived_at, pinned_at DESC, last_active_at DESC);
ALTER TABLE public.apocrypha_member_chat_thread ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.apocrypha_member_chat_request
    ADD COLUMN thread_id      uuid,
    ADD COLUMN engine_lane    text   NOT NULL DEFAULT 'local',
    ADD COLUMN attachment_ids uuid[] NOT NULL DEFAULT '{}'::uuid[],
    ADD CONSTRAINT apocrypha_member_chat_request_lane_shape CHECK (engine_lane IN ('local', 'flagship')),
    ADD CONSTRAINT apocrypha_member_chat_request_thread_fk
        FOREIGN KEY (tenant_id, principal_id, thread_id)
        REFERENCES public.apocrypha_member_chat_thread(tenant_id, principal_id, id) ON DELETE SET NULL;
CREATE INDEX apocrypha_member_chat_request_thread_history
    ON public.apocrypha_member_chat_request (tenant_id, principal_id, thread_id, turn_sequence DESC);

-- ─── attachments (the plus icon) ─────────────────────────────────────────────
-- Bytes live in a private storage bucket; the row keeps the text the model may read (bounded)
-- and the mime so images can ride as images. A row is bound to the thread it was uploaded into.
CREATE TABLE public.apocrypha_member_chat_attachment (
    id              uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id       uuid        NOT NULL,
    principal_id    uuid        NOT NULL,
    ...
    created_at      timestamptz NOT NULL DEFAULT now(),
    used_by_job_id  uuid,
    CONSTRAINT apocrypha_member_chat_attachment_thread_fk
        FOREIGN KEY (tenant_id, principal_id, thread_id)
        REFERENCES public.apocrypha_member_chat_thread(tenant_id, principal_id, id) ON DELETE CASCADE,
    CONSTRAINT apocrypha_member_chat_attachment_scope_unique UNIQUE (tenant_id, principal_id, id),
    CONSTRAINT apocrypha_member_chat_attachment_size CHECK (byte_size BETWEEN 1 AND 26214400),
    CONSTRAINT apocrypha_member_chat_attachment_name_shape CHECK (char_length(file_name) BETWEEN 1 AND 255),
    CONSTRAINT apocrypha_member_chat_attachment_text_bound CHECK (extracted_text IS NULL OR octet_length(extracted_text) <= 65536)
);
ALTER TABLE public.apocrypha_member_chat_attachment ENABLE ROW LEVEL SECURITY;
INSERT INTO storage.buckets (id, name, public, file_size_limit)
VALUES ('apocrypha-attachments', 'apocrypha-attachments', false, 26214400)
ON CONFLICT (id) DO NOTHING;

-- ─── consented analytics ─────────────────────────────────────────────────────
-- Collection is gated in the database: an event for a member without analytics consent is
-- dropped by the recording function, not by the caller's good manners.
CREATE TABLE public.apocrypha_member_consent (
    auth_user_id uuid        PRIMARY KEY,
    analytics    boolean     NOT NULL DEFAULT false,
    updated_at   timestamptz NOT NULL DEFAULT now()
);
ALTER TABLE public.apocrypha_member_consent ENABLE ROW LEVEL SECURITY;
CREATE TABLE public.apocrypha_analytics_event (
    id           bigserial   PRIMARY KEY,
    auth_user_id uuid,
    kind         text        NOT NULL,
    props        jsonb       NOT NULL DEFAULT '{}'::jsonb,
    created_at   timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT apocrypha_analytics_event_kind_shape CHECK (kind ~ '^[a-z][a-z0-9_.]{1,63}$')
);
CREATE INDEX apocrypha_analytics_event_time ON public.apocrypha_analytics_event (created_at DESC, kind);
ALTER TABLE public.apocrypha_analytics_event ENABLE ROW LEVEL SECURITY;
...
REVOKE ALL ON FUNCTION public.apocrypha_set_member_consent(uuid, boolean) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_record_analytics_event(uuid, text, jsonb) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_member_has_flagship(uuid) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_ensure_member_thread(uuid, uuid) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_create_member_thread(uuid, text) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_list_member_threads(uuid, boolean) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_update_member_thread(uuid, uuid, text, boolean, boolean) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_register_member_attachment(uuid, uuid, text, text, integer, text, text) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_enqueue_member_chat_v3(uuid, uuid, uuid, text, text, text, text, text, uuid, text, uuid[]) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_list_member_chat_history_v3(uuid, uuid, uuid, text, integer) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_admin_telemetry(integer) FROM PUBLIC, authenticated;
REVOKE ALL ON FUNCTION public.apocrypha_admin_analytics(integer) FROM PUBLIC, authenticated;
